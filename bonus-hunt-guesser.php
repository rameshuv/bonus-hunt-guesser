 ...
 * Description: Bonus hunt management with tournaments, leaderboards, user guesses, affiliates, and translations.
 * Version: 8.0.14
...
add_action( 'wp_head', 'bhg_output_style_panel_css' );
...
/**
 * Output dynamic frontend styles configured in the admin panel.
 *
 * @return void
 */
function bhg_output_style_panel_css() {
	if ( is_admin() ) {
			return;
	}

	if ( ! function_exists( 'bhg_build_style_panel_css' ) ) {
			return;
	}

		$css = trim( bhg_build_style_panel_css() );

	if ( '' === $css ) {
			return;
	}

		echo '<style id="bhg-style-panel-css">' . $css . '</style>';
}
...
function bhg_handle_settings_save() {
	...
	if ( isset( $_POST['shortcode_visibility'] ) && is_array( $_POST['shortcode_visibility'] ) ) {
			$visibility = array();
		foreach ( $_POST['shortcode_visibility'] as $slug => $value ) {
				$slug                = sanitize_key( wp_unslash( $slug ) );
				$visibility[ $slug ] = (int) ( '1' === (string) wp_unslash( $value ) || 1 === (int) $value );
		}
			$settings['shortcode_visibility'] = $visibility;
	}

	if ( isset( $_POST['style_panel'] ) && is_array( $_POST['style_panel'] ) ) {
			$style_panel = array();
		foreach ( $_POST['style_panel'] as $section => $values ) {
				$section = sanitize_key( wp_unslash( $section ) );
			if ( ! is_array( $values ) ) {
				continue;
			}
				$style_panel[ $section ] = array();
			foreach ( $values as $key => $raw_value ) {
					$key   = sanitize_key( wp_unslash( $key ) );
					$value = is_string( $raw_value ) ? wp_unslash( $raw_value ) : $raw_value;
				if ( function_exists( 'bhg_sanitize_css_value' ) ) {
					$value = bhg_sanitize_css_value( $value );
				} else {
						$value = sanitize_text_field( $value );
				}
						$style_panel[ $section ][ $key ] = $value;
			}
		}
		if ( ! empty( $style_panel ) ) {
				$settings['style_panel'] = $style_panel;
		}
	}
