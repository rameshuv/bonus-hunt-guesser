...
function bhg_handle_settings_save() {
	...
	$ads_enabled_value       = isset( $_POST['bhg_ads_enabled'] ) ? absint( wp_unslash( $_POST['bhg_ads_enabled'] ) ) : 0;
	...
	if ( isset( $_POST['bhg_post_submit_redirect'] ) ) {
		$redirect_raw = sanitize_text_field( wp_unslash( $_POST['bhg_post_submit_redirect'] ) );
		$redirect     = trim( $redirect_raw );
		if ( '' === $redirect ) {
...
function bhg_handle_submit_guess() {
	...
	if ( isset( $_POST['redirect_to'] ) ) {
		$requested_redirect_raw = sanitize_text_field( wp_unslash( $_POST['redirect_to'] ) );
		$requested_redirect     = trim( $requested_redirect_raw );
