...
/* phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare,WordPress.DB.PreparedSQLPlaceholders.ReplacementsMismatch,WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber */

if ( ! function_exists( 'bhg_normalize_int_list' ) ) {
	/**
	 * Normalize a list of IDs into unique positive integers.
	 *
	 * @param int|int[] $ids ID or list of IDs to normalize.
	 * @return int[] Normalized list of IDs.
	 */
...
if ( ! function_exists( 'bhg_get_hunt_tournament_ids' ) ) {
	 ...
	 * @return int[] Tournament IDs associated with the hunt.
	 */
...
if ( ! function_exists( 'bhg_get_tournament_hunt_ids' ) ) {
	/**
	 * Retrieve hunt IDs linked to a tournament.
	 *
	 * @param int $tournament_id Tournament ID.
	 * @return int[] Hunt IDs associated with the tournament.
	 */
...
if ( ! function_exists( 'bhg_sync_legacy_hunt_tournament_column' ) ) {
	/**
	 * Sync the legacy tournament_id column for a hunt.
	 *
	 * @param int   $hunt_id    Hunt ID.
	 * @param int[] $known_ids  Known tournament IDs for the hunt.
	 * @return void
	 */
...
if ( ! function_exists( 'bhg_set_hunt_tournaments' ) ) {
	/**
	 * Update tournament relationships for a hunt.
	 *
	 * @param int   $hunt_id        Hunt ID.
	 * @param int[] $tournament_ids Tournament IDs to associate.
	 * @return void
	 */
...
if ( ! function_exists( 'bhg_set_tournament_hunts' ) ) {
	/**
	 * Update hunt relationships for a tournament.
	 *
	 * @param int   $tournament_id Tournament ID.
	 * @param int[] $hunt_ids      Hunt IDs to associate.
	 * @return void
	 */
...
if ( ! function_exists( 'bhg_get_hunt' ) ) {
	/**
	 * Retrieve a hunt record.
	 *
	 * @param int $hunt_id Hunt ID.
	 * @return object|null Hunt object or null when not found.
	 */
...
if ( ! function_exists( 'bhg_get_latest_closed_hunts' ) ) {
	/**
	 * Fetch the latest closed hunts.
	 *
	 * @param int $limit Number of hunts to retrieve.
	 * @return object[] List of hunts.
	 */
...
if ( ! function_exists( 'bhg_get_top_winners_for_hunt' ) ) {
	/**
	 * Retrieve the top winners for a hunt.
	 *
	 * @param int $hunt_id       Hunt ID.
	 * @param int $winners_limit Optional. Number of winners to return.
	 * @return object[] Winner records with guess and diff.
	 */
	function bhg_get_top_winners_for_hunt( $hunt_id, $winners_limit = 3 ) {
		...
		if ( $winners_limit ) {
			...
			$limit = 3;
				...
				$sql = $wpdb->prepare(
					sprintf(
						'SELECT g.user_id, g.guess, ABS(%%f - g.guess) AS diff FROM `%s` g WHERE g.hunt_id = %%d ORDER BY ABS(%%f - g.guess) ASC LIMIT %%d',
						$t_g
					),
...
if ( ! function_exists( 'bhg_get_all_ranked_guesses' ) ) {
	/**
	 * Retrieve all guesses for a hunt ordered by proximity.
	 *
	 * @param int $hunt_id Hunt ID.
	 * @return object[] Guess records with difference values.
	 */
	function bhg_get_all_ranked_guesses( $hunt_id ) {
		...
		if ( ! $hunt || null === $hunt->final_balance ) {
				...
				$sql = $wpdb->prepare(
					sprintf(
						'SELECT g.id, g.user_id, g.guess, ABS(%%f - g.guess) AS diff FROM `%s` g WHERE g.hunt_id = %%d ORDER BY ABS(%%f - g.guess) ASC',
						$t_g
					),
					(float) $hunt->final_balance,
					(int) $hunt_id,
...
if ( ! function_exists( 'bhg_get_hunt_participants' ) ) {
	/**
	 * Retrieve paginated participants for a hunt.
	 *
	 * @param int $hunt_id  Hunt ID.
	 * @param int $paged    Page number.
	 * @param int $per_page Items per page.
	 * @return array{rows: object[], total: int} Participant data.
	 */
...
/* phpcs:enable WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare,WordPress.DB.PreparedSQLPlaceholders.ReplacementsMismatch,WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber */
