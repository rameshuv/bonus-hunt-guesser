...
if ( ! function_exists( 'bhg_get_top_winners_for_hunt' ) ) {
	function bhg_get_top_winners_for_hunt( $hunt_id, $winners_limit = 3 ) {
		...
		if ( $winners_limit ) {
                ...
                $sql = $wpdb->prepare(
                        sprintf(
                                'SELECT g.user_id, g.guess, ABS(%%f - g.guess) AS diff FROM `%s` g WHERE g.hunt_id = %%d ORDER BY diff ASC LIMIT %%d',
                                $t_g
                        ),
...
if ( ! function_exists( 'bhg_get_all_ranked_guesses' ) ) {
	function bhg_get_all_ranked_guesses( $hunt_id ) {
		...
		if ( ! $hunt || null === $hunt->final_balance ) {
                ...
                $sql = $wpdb->prepare(
                        sprintf(
                                'SELECT g.id, g.user_id, g.guess, ABS(%%f - g.guess) AS diff FROM `%s` g WHERE g.hunt_id = %%d ORDER BY diff ASC',
                                $t_g
                        ),
                        (float) $hunt->final_balance,
                        (int) $hunt_id
