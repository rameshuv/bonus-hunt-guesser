...
class BHG_DB {
	...
	KEY eligible (eligible)
				...
				$sql[] = "CREATE TABLE `{$prize_cats_table}` (
        id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
        name VARCHAR(190) NOT NULL,
        slug VARCHAR(190) NOT NULL,
        link_url VARCHAR(255) NULL,
...
css_background VARCHAR(40) NULL,
	...
	PRIMARY KEY  (id),
		...
		try {
				// Prize categories adjustments.
				$cat_columns = array(
					'link_target' => "ADD COLUMN link_target VARCHAR(20) NOT NULL DEFAULT '_self' AFTER link_url",
					'show_link'   => 'ADD COLUMN show_link TINYINT(1) NOT NULL DEFAULT 0 AFTER link_target',
				);

				foreach ( $cat_columns as $column => $alter ) {
					if ( ! $this->column_exists( $prize_cats_table, $column ) ) {
								// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.SchemaChange, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
								$wpdb->query( "ALTER TABLE `{$prize_cats_table}` {$alter}" );
					}
				}

				if ( ! $this->index_exists( $prize_cats_table, 'slug' ) ) {
						// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.SchemaChange, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
						$wpdb->query( "ALTER TABLE `{$prize_cats_table}` ADD UNIQUE KEY slug (slug)" );
				}

				if ( ! $this->index_exists( $prize_cats_table, 'name' ) ) {
						// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.SchemaChange, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
...
WHERE tournament_id IS NOT NULL AND tournament_id > 0",
			...
			if ( function_exists( 'error_log' ) ) {
				...
				$this->seed_prize_categories( $prize_cats_table, $prizes_table );
	}
	...
	UNIQUE KEY slug_locale (slug, locale)
		...
		/**
		 * Seed default prize categories and ensure slugs referenced by prizes exist.
		 *
		 * @param string $categories_table Prize categories table name.
		 * @param string $prizes_table     Prizes table name.
		 * @return void
		 */
	private function seed_prize_categories( $categories_table, $prizes_table ) {
			global $wpdb;

		if ( empty( $categories_table ) || ! $this->table_exists( $categories_table ) ) {
				return;
		}

			$now = current_time( 'mysql' );

			// Build lookup of existing slugs to avoid duplicates.
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery
			$existing_rows = $wpdb->get_col( "SELECT slug FROM `{$categories_table}`" );
			$existing      = array();

		foreach ( (array) $existing_rows as $slug ) {
			...
			if ( class_exists( 'BHG_Prizes' ) && method_exists( 'BHG_Prizes', 'get_default_category_definitions' ) ) {
					$defaults = BHG_Prizes::get_default_category_definitions();
			}
			...
			if ( empty( $prizes_table ) || ! $this->table_exists( $prizes_table ) ) {
					return;
			}

			// Seed any legacy categories stored on prizes.
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery
			$legacy_slugs = $wpdb->get_col( "SELECT DISTINCT category FROM `{$prizes_table}` WHERE category <> ''" );

			foreach ( (array) $legacy_slugs as $legacy_slug ) {
					$slug = sanitize_key( (string) $legacy_slug );
			...
			if ( class_exists( 'BHG_Prizes' ) && method_exists( 'BHG_Prizes', 'flush_category_cache' ) ) {
					BHG_Prizes::flush_category_cache();
			}
	}
	 ...
	 */
		...
		if ( $wpdb->last_error || empty( $exists ) ) {
			$wpdb->last_error = '';
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
