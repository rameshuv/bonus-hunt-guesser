...
public function hunts_shortcode( $atts ) {
	...
	$params = array_merge( array( $h, $aff_table ), $params );
	$sql    = $wpdb->prepare( $sql . $order_clause, ...$params );
	// db call ok; no-cache ok.
...
public function leaderboards_shortcode( $atts ) {
	...
	if ( empty( $fields_arr ) ) {
		...
		// db call ok; no-cache ok.
	$sql  = $wpdb->prepare(
		'SELECT r.user_id, u.user_login, SUM(r.wins) AS total_wins, (SELECT AVG(hw.position) FROM %i hw WHERE hw.user_id = r.user_id) AS avg_rank FROM %i r INNER JOIN %i u ON u.ID = r.user_id GROUP BY r.user_id, u.user_login ORDER BY total_wins DESC, u.user_login ASC LIMIT %d',
		$hw,
		$r,
		$u,
		$ranking
	...
	foreach ( $rows as $row ) {
		...
		$last_sql                          = $wpdb->prepare(
			'SELECT t.title AS tournament_title, w.name AS site_name FROM %i r INNER JOIN %i t ON t.id = r.tournament_id LEFT JOIN %i w ON w.id = t.affiliate_site_id WHERE r.user_id = %d ORDER BY r.last_win_date DESC LIMIT 1',
			$r,
			$t,
			$w,
			$row->user_id
		...
		$hunt_sql                    = $wpdb->prepare(
			'SELECT h.title FROM %i hw INNER JOIN %i h ON h.id = hw.hunt_id WHERE hw.user_id = %d ORDER BY hw.created_at DESC LIMIT 1',
			$hw,
			$h,
			$row->user_id
		);
...
public function tournaments_shortcode( $atts ) {
	...
	$params = array_merge( array( $t ), $args );
	$query  = $wpdb->prepare( $query, ...$params );
	$rows   = $wpdb->get_results( $query ); // db call ok; no-cache ok.
	if ( ! $rows ) {
...
public function best_guessers_shortcode( $atts ) {
	...
	foreach ( $periods as $key => $info ) {
		if ( $info['type'] ) {
			...
			$sql    = 'SELECT u.ID as user_id, u.user_login, SUM(r.wins) as total_wins FROM %i r INNER JOIN %i u ON u.ID = r.user_id INNER JOIN %i t ON t.id = r.tournament_id WHERE ' . $where . ' GROUP BY u.ID, u.user_login ORDER BY total_wins DESC, u.user_login ASC LIMIT 50';
			$params = array_merge( array( $wins_tbl, $users_tbl, $tours_tbl ), $params );
			// db call ok; no-cache ok.
			$sql             = $wpdb->prepare( $sql, ...$params );
			$results[ $key ] = $wpdb->get_results( $sql );
			$sql             = $wpdb->prepare(
				'SELECT u.ID as user_id, u.user_login, SUM(r.wins) as total_wins FROM %i r INNER JOIN %i u ON u.ID = r.user_id GROUP BY u.ID, u.user_login ORDER BY total_wins DESC, u.user_login ASC LIMIT 50',
				$wins_tbl,
				$users_tbl
			);
			// db call ok; no-cache ok.
