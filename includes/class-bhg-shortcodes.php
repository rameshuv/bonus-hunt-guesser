...
if (!class_exists("BHG_Shortcodes")) {
    ...
    class BHG_Shortcodes
    {
         ...
         * @var string[]
         */
        private const LEADERBOARD_DEFAULT_FILTERS = [
            "timeline",
            "tournament",
            "site",
            "affiliate",
        ...
        private function resolve_affiliate_site_id($raw_site)
        {
            ...
            $table = esc_sql(
                $this->sanitize_table($wpdb->prefix . "bhg_affiliate_websites")
            );
            if (!$table) {
                return 0;
            }
        ...
        private function user_matches_affiliate_site($user_id, $site_id)
        {
            ...
            if (function_exists("bhg_is_user_affiliate_for_site")) {
                return (bool) bhg_is_user_affiliate_for_site(
                    $user_id,
                    $site_id
                );
            }

            $user_site_id = (int) get_user_meta(
                $user_id,
                "affiliate_site_id",
                true
            );
            ...
            $user_site = (string) get_user_meta(
                $user_id,
                "affiliate_site",
                true
            );
            ...
            if (!isset($site_lookup[$site_id])) {
                ...
                if ($table) {
                    $site_lookup[$site_id] = $wpdb->get_row(
            ...
            if ($site_row) {
                $slug_matches =
                    isset($site_row->slug) &&
                    strtolower((string) $site_row->slug) ===
                        strtolower($user_site);
                $name_matches =
                    isset($site_row->name) &&
                    strtolower((string) $site_row->name) ===
                        strtolower($user_site);
        ...
        public function register_shortcodes()
        {
            ...
            add_shortcode("bhg_winner_notifications", [
                $this,
                "winner_notifications_shortcode",
            ]);
            add_shortcode("bhg_user_profile", [
                $this,
                "user_profile_shortcode",
            ]);

            // Addons.
            add_shortcode("bhg_best_guessers", [
                $this,
                "best_guessers_shortcode",
            ]);
            add_shortcode("bhg_user_guesses", [
                $this,
                "user_guesses_shortcode",
            ]);
            add_shortcode("bhg_hunts", [$this, "hunts_shortcode"]);
            add_shortcode("bhg_leaderboards", [
                $this,
                "leaderboards_shortcode",
            ]);
            add_shortcode("bhg_prizes", [$this, "prizes_shortcode"]);
            add_shortcode("my_bonushunts", [$this, "my_bonushunts_shortcode"]);
            add_shortcode("my_tournaments", [
                $this,
                "my_tournaments_shortcode",
            ]);
            add_shortcode("my_prizes", [$this, "my_prizes_shortcode"]);
            add_shortcode("my_rankings", [$this, "my_rankings_shortcode"]);
            add_shortcode("bhg_jackpot_current", [
                $this,
                "jackpot_current_shortcode",
            ]);
            add_shortcode("bhg_jackpot_ticker", [
                $this,
                "jackpot_ticker_shortcode",
            ]);
            add_shortcode("bhg_jackpot_winners", [
                $this,
                "jackpot_winners_shortcode",
            ]);
            add_shortcode("bhg_button", [$this, "button_shortcode"]);
            add_shortcode("bhg_latest_winners_list", [
                $this,
                "latest_winners_list_shortcode",
            ]);
            add_shortcode("bhg_leaderboard_list", [
                $this,
                "leaderboard_list_shortcode",
            ]);
            add_shortcode("bhg_tournament_list", [
                $this,
                "tournament_list_shortcode",
            ]);
            add_shortcode("bhg_bonushunt_list", [
                $this,
                "bonushunt_list_shortcode",
            ]);

            // Back-compat aliases for early adopters without the bhg_ prefix.
            add_shortcode("latest-winners-list", [
                $this,
                "latest_winners_list_shortcode",
            ]);
            add_shortcode("leaderboard-list", [
                $this,
                "leaderboard_list_shortcode",
            ]);
            add_shortcode("tournament-list", [
                $this,
                "tournament_list_shortcode",
            ]);
            add_shortcode("bonushunt-list", [
                $this,
                "bonushunt_list_shortcode",
            ]);

            // Legacy/aliases.
            add_shortcode("bonus_hunt_leaderboard", [
                $this,
                "leaderboard_shortcode",
            ]);
            add_shortcode("bonus_hunt_login", [$this, "login_hint_shortcode"]);
            add_shortcode("bhg_active", [$this, "active_hunt_shortcode"]);
        ...
        /**
         * Return an admin-only notice and stay silent for non-admin users.
         *
         * @param string $message Notice content.
         * @return string HTML to render for admins or an empty string for others.
         */
        ...
        private function get_timeline_range($timeline)
        {
            ...
            $timeline = str_replace([" ", "-"], "_", $timeline);
            ...
            switch ($canonical) {
                ...
                case "this_year":
                    $start_dt = $now
                        ...
                        ->setTime(0, 0, 0);
                    $end_dt = $now
                        ->setDate((int) $now->format("Y"), 12, 31)
                        ->setTime(23, 59, 59);
                    break;
        ...
        private function run_leaderboard_query($args)
        {
            ...
            if ($tournament_id > 0 && $hunt_id <= 0) {
                ...
                if (
                    is_array($tournament_rows) &&
                    empty($tournament_rows["error"]) &&
                    isset($tournament_rows["total"]) &&
                    $tournament_rows["total"] >= 1
                ) {
                    return $tournament_rows;
                }
            }

            if (!in_array($direction_key, ["asc", "desc"], true)) {
                $direction_key = "desc";
            }
            ...
            $r = esc_sql(
                $this->sanitize_table($wpdb->prefix . "bhg_tournament_results")
            );
            $u = esc_sql($this->sanitize_table($wpdb->users));
            $t = esc_sql(
                $this->sanitize_table($wpdb->prefix . "bhg_tournaments")
            );
            $w = esc_sql(
                $this->sanitize_table($wpdb->prefix . "bhg_affiliate_websites")
            );
            $hw = esc_sql(
                $this->sanitize_table($wpdb->prefix . "bhg_hunt_winners")
            );
            $h = esc_sql(
                $this->sanitize_table($wpdb->prefix . "bhg_bonus_hunts")
            );
            $ht = esc_sql(
                $this->sanitize_table($wpdb->prefix . "bhg_tournaments_hunts")
            );
            ...
            if (!$r || !$u || !$t || !$w || !$hw || !$h || !$um || !$ht) {
                return [
                    ...
                    "limit" => 0,
                    "error" => bhg_t(
                        "notice_no_data_available",
                        "No data available."
                    ),
                ];
            }

            $aff_yes_values = ["1", "yes", "true", "on"];
            $aff_yes_sql = [];
            ...
            $where = ["hw.eligible = 1", "h.status = %s"];
            $prep_where = ["closed"];
            ...
            if ($tournament_id > 0) {
                ...
                $where[] =
                    "(ht.tournament_id = %d OR (ht.hunt_id IS NULL AND h.tournament_id = %d))";
                $prep_where[] = $tournament_id;
                $prep_where[] = $tournament_id;
            }

            if (in_array($aff_filter, ["yes", "true", "1"], true)) {
                $joins[] =
                    "INNER JOIN {$um} um_aff_filter ON um_aff_filter.user_id = u_filter.ID AND um_aff_filter.meta_key = '" .
                    esc_sql("bhg_is_affiliate") .
                    "'";
                $where[] = "CAST(um_aff_filter.meta_value AS CHAR) IN ({$aff_yes_list})";
            } elseif (in_array($aff_filter, ["no", "false", "0"], true)) {
                $joins[] =
                    "LEFT JOIN {$um} um_aff_filter ON um_aff_filter.user_id = u_filter.ID AND um_aff_filter.meta_key = '" .
                    esc_sql("bhg_is_affiliate") .
                    "'";
            ...
            $base_sql =
                "SELECT " .
                implode(", ", $base_select_parts) .
                " FROM {$hw} hw {$joins_sql}{$where_sql} GROUP BY hw.user_id, hw.hunt_id";
            ...
            if ($allow_union) {
                ...
                $participant_joins = [
                    "LEFT JOIN {$t} t_union ON t_union.id = tr.tournament_id",
                ];
                $participant_where = ["tr.tournament_id = %d"];
                $participant_params = [$tournament_id];
                ...
                if (in_array($aff_filter, ["yes", "true", "1"], true)) {
                    $participant_joins[] =
                        "INNER JOIN {$um} um_part ON um_part.user_id = tr.user_id AND um_part.meta_key = '" .
                        esc_sql("bhg_is_affiliate") .
                        "'";
                    $participant_where[] = "CAST(um_part.meta_value AS CHAR) IN ({$aff_yes_list})";
                } elseif (in_array($aff_filter, ["no", "false", "0"], true)) {
                    $participant_joins[] =
                        "LEFT JOIN {$um} um_part ON um_part.user_id = tr.user_id AND um_part.meta_key = '" .
                        esc_sql("bhg_is_affiliate") .
                        "'";
                ...
                if ($range) {
                    $participant_where[] =
                        "(COALESCE( NULLIF( tr.last_win_date, '0000-00-00 00:00:00' ), tr.last_win_date ) BETWEEN %s AND %s)";
                ...
                $participant_sql =
                    "SELECT " .
                    implode(", ", $participant_select) .
                    " FROM {$r} tr " .
                    implode(" ", $participant_joins);
                if (!empty($participant_where)) {
                    $participant_sql .=
                        " WHERE " . implode(" AND ", $participant_where);
                }

                $prepared_participant_sql = $wpdb->prepare(
                    $participant_sql,
                    ...$participant_params
                );
                $base_union_sql =
            ...
            $aggregate_sql =
                "SELECT " .
                implode(", ", $aggregate_parts) .
                " FROM " .
                $filtered_wrapper_sql .
                " fw GROUP BY fw.user_id";
            ...
            $select_parts = ["wins.user_id", "u.user_login", "wins.total_wins"];
            ...
            if ($need_avg_tournament || "avg_tournament" === $orderby_request) {
                ...
                $rank_query =
                    "SELECT tr.user_id, tr.tournament_id, (SELECT 1 + COUNT(*) FROM " .
                    $r .
                    " tr2 WHERE tr2.tournament_id = tr.tournament_id AND (tr2.wins > tr.wins OR (tr2.wins = tr.wins AND tr2.user_id < tr.user_id))) AS rank_position FROM " .
                    $r .
                    " tr" .
                    $rank_join_sql .
                    $rank_where_sql;
            ...
            $latest_hunt_subquery =
                "(SELECT fw_inner.hunt_id FROM " .
                $filtered_wrapper_sql .
                " fw_inner WHERE fw_inner.user_id = wins.user_id AND fw_inner.hunt_id > 0 ORDER BY fw_inner.win_date DESC, fw_inner.hunt_id DESC LIMIT 1)";

            if ($need_site_details) {
                $select_parts[] =
                    "(SELECT h2.affiliate_site_id FROM " .
                    $h .
                    " h2 WHERE h2.id = " .
                    $latest_hunt_subquery .
                    " LIMIT 1) AS site_id";
                if ($need_site) {
                    $select_parts[] =
                        ...
                        " w2 INNER JOIN " .
                        $h .
                        " h2 ON h2.affiliate_site_id = w2.id WHERE h2.id = " .
                        $latest_hunt_subquery .
            ...
            if ($need_hunt_name) {
                $select_parts[] =
                    "(SELECT h2.title FROM " .
                    $h .
                    " h2 WHERE h2.id = " .
                    $latest_hunt_subquery .
                    " LIMIT 1) AS hunt_title";
            }

            if ($need_tournament_name) {
                $select_parts[] =
                    "(SELECT COALESCE(t2.title, t2_legacy.title) FROM " .
                    $h .
                    " h2 LEFT JOIN " .
                    $ht .
                    " ht2 ON ht2.hunt_id = h2.id LEFT JOIN " .
                    $t .
                    " t2 ON t2.id = ht2.tournament_id LEFT JOIN " .
                    $t .
                    " t2_legacy ON t2_legacy.id = h2.tournament_id WHERE h2.id = " .
                    $latest_hunt_subquery .
            ...
            $direction = isset($direction_map[$direction_key])
                ? $direction_map[$direction_key]
                : $direction_map["desc"];
            $orderby = isset($orderby_map[$orderby_request])
                ? $orderby_map[$orderby_request]
                : $orderby_map["wins"];

            $order_clauses = [sprintf("%s %s", $orderby, $direction)];
        ...
        private function run_tournament_results_leaderboard($args)
        {
            ...
            $tournament_affiliate_site = get_post_meta(
                $tournament_id,
                "affiliate_site",
                true
            );
            $tournament_affiliate_site_id = (int) get_post_meta(
                $tournament_id,
                "affiliate_site_id",
                true
            );
            if ($tournament_affiliate_site_id <= 0 && $t) {
                // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
                $tournament_affiliate_site_id = (int) $wpdb->get_var(
                    $wpdb->prepare(
                        "SELECT affiliate_site_id FROM " .
                            $t .
                            " WHERE id = %d",
                        $tournament_id
                    )
            ...
            $needs_affiliate_site_filter =
                "" !== $tournament_affiliate_site ||
                $tournament_affiliate_site_id > 0;
            ...
            if (
                $need_site_details &&
                (!$can_use_hunt_meta || ($need_site && !$has_sites))
            ) {
            ...
            $count_joins = ["INNER JOIN {$u} u ON u.ID = tr.user_id"];
            ...
            if ($site_filter_active) {
                $site_where = [
                    "hw_scope.eligible = 1",
                    "h_scope.affiliate_site_id = %d",
                ];
                $site_params = [$website_id];
                $site_joins = $has_ht
                    ? " LEFT JOIN " .
                        $ht .
                        " ht_scope ON ht_scope.hunt_id = hw_scope.hunt_id"
                    : "";

                if ($has_ht) {
                    $site_where[] =
                        "(ht_scope.tournament_id = %d OR (ht_scope.hunt_id IS NULL AND h_scope.tournament_id = %d))";
                ...
                if ($range) {
                    $site_where[] =
                        "(COALESCE( NULLIF( h_scope.closed_at, '0000-00-00 00:00:00' ), NULLIF( hw_scope.created_at, '0000-00-00 00:00:00' ), NULLIF( h_scope.created_at, '0000-00-00 00:00:00' ) ) BETWEEN %s AND %s)";
                ...
                $count_joins[] =
                    "INNER JOIN (" .
                    $site_scope_query .
                    ") site_scope ON site_scope.user_id = tr.user_id";
                $select_joins[] =
                    "INNER JOIN (" .
                    $site_scope_query .
                    ") site_scope ON site_scope.user_id = tr.user_id";
            ...
            if (in_array($aff_filter, ["yes", "true", "1"], true)) {
                ...
                $join_clause =
                    "INNER JOIN {$um} um_aff_filter ON um_aff_filter.user_id = tr.user_id AND um_aff_filter.meta_key = '" .
                    esc_sql("bhg_is_affiliate") .
                    "'";
                ...
                $join_clause =
                    "LEFT JOIN {$um} um_aff_filter ON um_aff_filter.user_id = tr.user_id AND um_aff_filter.meta_key = '" .
                    esc_sql("bhg_is_affiliate") .
                    "'";
                $count_joins[] = $join_clause;
                $select_joins[] = $join_clause;
            ...
            $count_sql =
                "SELECT COUNT(DISTINCT tr.user_id) FROM " .
                $r .
                " tr " .
                implode(" ", $count_joins);
            if (!empty($where)) {
            ...
            if ($can_use_hunt_meta && !$site_filter_active) {
                ...
                $win_where = ["hw_count.eligible = 1"];
                $win_params = [];
                $win_joins = $has_ht
                    ? " LEFT JOIN " .
                        $ht .
                        " ht_count ON ht_count.hunt_id = hw_count.hunt_id"
                    : "";

                if ($has_ht) {
                    $win_where[] =
                        "(ht_count.tournament_id = %d OR (ht_count.hunt_id IS NULL AND h_count.tournament_id = %d))";
                ...
                $win_count_sql = strtr(
                    ...
                    [
                        "{hw}" => $hw,
                        "{h}" => $h,
                        "{joins}" => $win_join_segment,
                        "{where}" => implode(" AND ", $win_where),
                    ]
                );

                if (!empty($win_params)) {
                    $win_count_sql = $wpdb->prepare(
                        $win_count_sql,
                        ...$win_params
                    );
                }
                ...
                $select_joins[] =
                    "LEFT JOIN (" .
                    $win_count_sql .
                    ") win_totals ON win_totals.user_id = tr.user_id";
            ...
            if ($need_avg_hunt) {
                $avg_scope_condition = $has_ht
                    ? sprintf(
                        '(ht_scope.tournament_id = %1$d OR (ht_scope.hunt_id IS NULL AND h_scope.tournament_id = %1$d))',
                        $tournament_id
                    )
                ...
                $avg_ht_join = $has_ht
                    ? " LEFT JOIN " .
                        $ht .
                        " ht_scope ON ht_scope.hunt_id = hw_avg.hunt_id"
                    : "";
                ...
                $select_joins[] =
                    "LEFT JOIN (" .
                    $avg_hunt_query .
                    ") hunt_stats ON hunt_stats.user_id = tr.user_id";
            }
            ...
            if ($need_avg_tournament || "avg_tournament" === $orderby_request) {
                ...
                $select_joins[] =
                    "LEFT JOIN (" .
                    $rank_query .
                    ") tour_rank ON tour_rank.user_id = tr.user_id";
            }
            ...
            if ($need_site_details || $need_hunt_name) {
                $latest_scope = $has_ht
                    ? sprintf(
                        '(ht_inner.tournament_id = %1$d OR (ht_inner.hunt_id IS NULL AND h_inner.tournament_id = %1$d))',
                        $tournament_id
                    )
                    : sprintf("h_inner.tournament_id = %d", $tournament_id);
                $latest_ht_join = $has_ht
                    ? " LEFT JOIN " .
                        $ht .
                        " ht_inner ON ht_inner.hunt_id = hw_inner.hunt_id"
                    : "";
                $win_expr_inner =
                    "COALESCE( NULLIF( h_inner.closed_at, '0000-00-00 00:00:00' ), NULLIF( hw_inner.created_at, '0000-00-00 00:00:00' ), NULLIF( h_inner.created_at, '0000-00-00 00:00:00' ) )";
            ...
            if ($needs_affiliate_site_filter) {
                ...
                $rows_all = BHG_Tournaments_Controller::filter_results_by_affiliate(
                    $tournament_id,
                    (array) $rows_all,
                    $tournament_affiliate_site_id,
                    $tournament_affiliate_site
                );

                if ($apply_affiliate_site_filter) {
                    $rows_all = array_values(
                        array_filter($rows_all, function ($row_obj) use (
                            $website_id
                        ) {
                            $uid = isset($row_obj->user_id)
                                ? (int) $row_obj->user_id
                            ...
                            return $this->user_matches_affiliate_site(
                                $uid,
                                $website_id
                            );
                        })
        ...
        private function normalize_prize_layout($layout)
        {
            ...
            return in_array($layout, ["grid", "carousel"], true)
                ? $layout
                : "grid";
        }
        ...
        {
            if (
                class_exists("BHG_Prizes") &&
                method_exists("BHG_Prizes", "get_display_settings")
            ) {
        ...
        private function normalize_prize_size($size)
        {
            ...
            return in_array($size, ["small", "medium", "big"], true)
                ? $size
                : "medium";
        }
         ...
         * @return string
         */
        private function resolve_responsive_prize_size(
            $requested_size,
            $layout,
            $visible_count,
            $total_count,
            ...
            $section_options = is_array($section_options)
                ? $section_options
                : [];

            $respect_manual = apply_filters(
                "bhg_prize_respect_manual_size",
                false,
                $requested_size,
                $layout,
                $visible_count,
         ...
         */
        private function build_prize_summary_entries(
            $prizes,
            $section_options = []
        ) {
            $entries = [];
            ...
            $section_option = is_array($section_options)
                ? $section_options
                : [];
        ...
        /**
         * Render a two-column summary for regular and premium prize sets.
         *
         * @param array $prize_sets      Map of prize type => prize rows.
         * @param array $section_options Rendering options passed to summary builder.
         * @param bool  $enabled         Whether summaries should be shown.
         * @return string HTML markup.
         */
        private function render_prize_summary_columns(
            $prize_sets,
            $section_options = [],
            $enabled = true
        ) {
            if (!$enabled || empty($prize_sets)) {
                return "";
            }

            $regular_prizes =
                isset($prize_sets["regular"]) &&
                is_array($prize_sets["regular"])
                    ? $prize_sets["regular"]
                    : [];
            $premium_prizes =
                isset($prize_sets["premium"]) &&
                is_array($prize_sets["premium"])
                    ? $prize_sets["premium"]
                    : [];

            $regular_summary = $this->build_prize_summary_entries(
                $regular_prizes,
                $section_options
            );
            $premium_summary = $this->build_prize_summary_entries(
                $premium_prizes,
                $section_options
            );

            if (empty($regular_summary) && empty($premium_summary)) {
                return "";
            }

            ob_start();
            echo '<div class="bhg-prize-summary bhg-info-block">';
            echo '<div class="bhg-prize-summary-heading">' .
                esc_html(bhg_t("label_prize_summary", "Prize Summary")) .
                "</div>";
            echo '<div class="bhg-prize-summary-grid">';

            if (!empty($regular_summary)) {
                echo '<div class="bhg-prize-summary-column">';
                echo '<h5 class="bhg-prize-summary-title">' .
                    esc_html(
                        bhg_t(
                            "regular_prize_summary_heading",
                            "Regular Prize Summary"
                        )
                    ) .
                    "</h5>";
                echo '<ol class="bhg-prize-summary-list">';
                foreach ($regular_summary as $entry) {
                    $text = isset($entry["text"])
                        ? (string) $entry["text"]
                        : "";
                    if ("" === $text) {
            ...
            if (!empty($premium_summary)) {
                echo '<div class="bhg-prize-summary-column">';
                echo '<h5 class="bhg-prize-summary-title">' .
                    esc_html(
                        bhg_t(
                            "premium_prize_summary_heading",
                            "Premium Prize Summary"
                        )
                    ) .
                    "</h5>";
                echo '<ol class="bhg-prize-summary-list">';
                foreach ($premium_summary as $entry) {
                    $text = isset($entry["text"])
                        ? (string) $entry["text"]
                        : "";
                    if ("" === $text) {
         ...
         */
        private function normalize_prize_maps_from_storage(
            $raw_prizes,
            $winners_count = 0
        ) {
            $maps = [
            ...
            if (is_array($decoded)) {
                if (isset($decoded["regular"]) || isset($decoded["premium"])) {
                    foreach (["regular", "premium"] as $type) {
                        if (
                            empty($decoded[$type]) ||
                            !is_array($decoded[$type])
                        ) {
                            continue;
                        }
        ...
        private function load_prize_sets_from_maps($prize_maps)
        {
            ...
            if (
                empty($prize_maps) ||
                !class_exists("BHG_Prizes") ||
                !method_exists("BHG_Prizes", "get_prizes_by_ids")
            ) {
                return $results;
            }
         ...
         * @return string
         */
        private function render_prize_sets_tabs(
            $prize_sets,
            $section_options = [],
            $display_overrides = [],
            $layout = "carousel",
            ...
            if (1 === count($available)) {
                $type = $available[0];
                $heading = isset($section_options["heading_text"])
                    ? $section_options["heading_text"]
                    : ("premium" === $type
                        ? bhg_t("label_premium_prize_set", "Premium Prize Set")
                        : bhg_t(
                            "label_regular_prize_set",
                            "Regular Prize Set"
                        ));
                $options = $section_options;
                $options["heading_text"] = $heading;
                return $this->render_prize_section(
                    $prize_sets[$type],
                    $layout,
                    $size,
                    $display_overrides,
            ...
            foreach ($available as $type) {
                ...
                $tab_label =
                    "premium" === $type
                        ? bhg_t("label_premium_prizes", "Premium Prizes")
                        : bhg_t("label_regular_prizes", "Regular Prizes");
                $tab_target = $tab_id . "-" . $type;
                echo '<button type="button" class="bhg-prize-tab' .
                    ($is_active ? " is-active" : "") .
                    '" role="tab" id="' .
                    esc_attr($tab_target) .
                    '" aria-selected="' .
            ...
            foreach ($available as $type) {
                ...
                $heading =
                    "premium" === $type
                        ? bhg_t("label_premium_prize_set", "Premium Prize Set")
                        : bhg_t("label_regular_prize_set", "Regular Prize Set");
                $options = $section_options;
                $options["heading_text"] = $heading;

                echo '<div class="bhg-prize-tab-panel' .
                    ($is_active ? " is-active" : "") .
                    '" role="tabpanel" id="' .
                    esc_attr($panel_id) .
                    '" aria-labelledby="' .
        ...
        private function normalize_yes_no_attr($value, $allow_inherit = true)
        {
            ...
            if (
                $allow_inherit &&
                in_array($value, ["inherit", "default", ""], true)
            ) {
                return "inherit";
            }

            if (in_array($value, ["no", "0", "false", "off"], true)) {
                return "0";
            }
        ...
        private function normalize_leaderboard_filters($filters_input)
        {
            ...
            if (in_array($keyword, ["all", "default", "inherit"], true)) {
                return self::LEADERBOARD_DEFAULT_FILTERS;
            }

            if (in_array($keyword, ["none", "no", "false", "0"], true)) {
                return [];
            }
        ...
        private function normalize_click_action_attr($value)
        {
            ...
            if (in_array($action, ["inherit", "default"], true)) {
                return "inherit";
            }
         ...
         */
        private function normalize_link_target_attr(
            $value,
            $allow_inherit = true
        ) {
            if (!class_exists("BHG_Prizes")) {
            ...
            if (
                $allow_inherit &&
                in_array($target, ["inherit", "default", ""], true)
            ) {
                return "inherit";
            }
        ...
        {
            $base_url = defined("BHG_PLUGIN_URL")
                ? BHG_PLUGIN_URL
        ...
        private function is_profile_section_enabled($section)
        {
            if (null === $this->profile_visibility_settings) {
                ...
                if (
                    isset($settings["profile_sections"]) &&
                    is_array($settings["profile_sections"])
                ) {
            ...
            if (!is_array($this->profile_visibility_settings)) {
                $this->profile_visibility_settings = [];
            }
            ...
            if (
                !array_key_exists($section, $this->profile_visibility_settings)
            ) {
                return true;
            }
         ...
         * @return string
         */
        private function render_prize_section(
            $prizes,
            $layout,
            $size,
            $display = [],
            ...
            $limit = isset($section_options["limit"])
                ? (int) $section_options["limit"]
                : 0;
            ...
            $count = count($prizes);
            $grid_columns = isset($section_options["grid_columns"])
                ? max(1, (int) $section_options["grid_columns"])
                : 3;
            $heading_text = isset($section_options["heading_text"])
                ? $section_options["heading_text"]
                : "";
            $hide_heading = !empty($section_options["hide_heading"]);
            $carousel_visible = isset($section_options["carousel_visible"])
                ? max(1, (int) $section_options["carousel_visible"])
                : 1;
            $carousel_autoplay = !empty($section_options["carousel_autoplay"]);
            $carousel_interval = isset($section_options["carousel_interval"])
                ? max(1000, (int) $section_options["carousel_interval"])
                : 5000;
            $carousel_pages = max(
                1,
                (int) ceil($count / max(1, $carousel_visible))
            );

            $visible_for_sizing =
                "carousel" === $layout
                    ? min($carousel_visible, $count)
                    : min(
                        $grid_columns,
                        $limit > 0 ? min($limit, $count) : $count
                    );
            $size = $this->resolve_responsive_prize_size(
                $size,
                $layout,
                $visible_for_sizing,
                $count,
            ...
            $summary_label = isset($section_options["summary_heading"])
                ? (string) $section_options["summary_heading"]
                : bhg_t("prize_summary_heading", "Prize Summary");
            $summary_entries = [];
            if ($summary_enabled) {
                $summary_entries = $this->build_prize_summary_entries(
                    $prizes,
                    $section_options
                );
            ...
            $card_renderer = [$this, "render_prize_card"];
            $context = [
                "layout" => $layout,
                ...
                "display_defaults" => class_exists("BHG_Prizes")
                    ? BHG_Prizes::get_display_defaults()
                    : [],
            ];
            ...
            ob_start();
            echo '<div class="bhg-prizes-block bhg-prizes-layout-' .
                esc_attr($layout) .
                " size-" .
                esc_attr($size) .
                '">';
            if (!$hide_heading && "" !== $heading_text) {
                echo '<h4 class="bhg-prizes-title">' .
            ...
            if ("carousel" === $layout) {
                ...
                echo '<div class="bhg-prize-carousel" data-count="' .
                    (int) $count .
                    '" data-visible="' .
                    esc_attr($carousel_visible) .
                    '" data-pages="' .
                    esc_attr($carousel_pages) .
                    '" data-autoplay="' .
                    ($carousel_autoplay ? "1" : "0") .
                    '" data-interval="' .
                    esc_attr($carousel_interval) .
                    '">';
                if ($show_nav) {
                    echo '<button type="button" class="bhg-prize-nav bhg-prize-prev" aria-label="' .
                        esc_attr(bhg_t("previous", "Previous")) .
                        '"><span aria-hidden="true">&lsaquo;</span></button>';
                }
                echo '<div class="bhg-prize-track-wrapper"><div class="bhg-prize-track">';
                foreach ($prizes as $prize) {
                    echo $this->render_prize_card(
                        $prize,
                        $size,
                        $display_options,
                        $context
                ...
                echo "</div></div>";
                if ($show_nav) {
                    echo '<button type="button" class="bhg-prize-nav bhg-prize-next" aria-label="' .
                        esc_attr(bhg_t("next", "Next")) .
                        '"><span aria-hidden="true">&rsaquo;</span></button>';
                    echo '<div class="bhg-prize-dots" role="tablist">';
                    for ($i = 0; $i < $carousel_pages; $i++) {
                        $active = 0 === $i ? " active" : "";
                        echo '<button type="button" class="bhg-prize-dot' .
                            esc_attr($active) .
                            '" data-index="' .
                            esc_attr($i) .
                            '" aria-label="' .
                ...
                echo '<div class="bhg-prizes-grid" data-columns="' .
                    esc_attr($grid_columns) .
                    '" style="--bhg-prize-grid-columns:' .
                    esc_attr($grid_columns) .
                    ';">';
                foreach ($prizes as $prize) {
                    echo $this->render_prize_card(
        ...
        public function button_shortcode($atts, $content = "")
        {
            ...
            $target = in_array($atts["link_target"], ["_self", "_blank"], true)
                ? $atts["link_target"]
                : "_self";
            $size = in_array($atts["size"], ["small", "medium", "big"], true)
                ? $atts["size"]
                : "medium";
            $classes = ["bhg-button-shortcode", "bhg-button-size-" . $size];

            $style = [];
            if ($atts["background"]) {
                $style[] =
                    "background-color:" .
                    sanitize_hex_color($atts["background"]);
            }
            if ($atts["text_color"]) {
            ...
            if ($atts["border_color"]) {
                $style[] =
                    "border-color:" . sanitize_hex_color($atts["border_color"]);
            }
            ...
            if ($atts["background_hover"]) {
                $hover_rules[] =
                    "--bhg-button-bg-hover:" .
                    sanitize_hex_color($atts["background_hover"]);
            }
            if ($atts["text_hover"]) {
                $hover_rules[] =
                    "--bhg-button-text-hover:" .
                    sanitize_hex_color($atts["text_hover"]);
            }

            $style_attr = $style
                ? ' style="' .
                    esc_attr(implode(";", $style)) .
                    ($hover_rules ? ";" . implode(";", $hover_rules) : "") .
                    '"'
                : ($hover_rules
                    ? ' style="' . esc_attr(implode(";", $hover_rules)) . '"'
                    : "");
        ...
        private function button_visibility_allows($visible_to)
        {
            switch ($visible_to) {
                ...
                case "affiliates":
                    return is_user_logged_in() &&
                        function_exists("bhg_is_user_affiliate") &&
                        bhg_is_user_affiliate(get_current_user_id());
        ...
        private function has_open_hunt()
        {
            ...
            return (bool) $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT(1) FROM {$hunts_table} WHERE status = %s",
                    "open"
                )
            ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        }
        ...
        private function has_open_tournament()
        {
            ...
            return (bool) $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT(1) FROM {$tours_table} WHERE status = %s",
                    "open"
                )
            ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        }
        ...
        private function render_prize_tabs($tabs)
        {
            ...
            foreach ($tabs as $tab) {
                ...
                $nav_markup .=
                    '<button type="button" class="bhg-prize-tab' .
            ...
            return '<div class="bhg-prize-tabset" data-bhg-prize-tabs="1"><div class="bhg-prize-tabs" role="tablist">' .
                $nav_markup .
                '</div><div class="bhg-prize-tab-panels">' .
                $panel_markup .
                "</div></div>";
        }
         ...
         */
        private function parse_field_tokens(
            $raw_fields,
            array $allowed_fields,
            array $default_fields
        ) {
            $raw_fields = is_array($raw_fields)
                ? $raw_fields
                : explode(",", (string) $raw_fields);
         ...
         */
        private function build_field_parts_from_tokens(
            array $tokens,
            array $value_map
        ) {
            $parts = [];
            ...
            foreach ($tokens as $token) {
                ...
                if (isset($token["literal"])) {
                    ...
                    $value = isset($value_map[$field])
                        ? (string) $value_map[$field]
                        : "";
                    $prefix =
                        isset($token["prefix"]) && "" !== $token["prefix"]
                            ? esc_html((string) $token["prefix"])
                            : "";
                    $suffix =
                        isset($token["suffix"]) && "" !== $token["suffix"]
                            ? esc_html((string) $token["suffix"])
                            : "";
        ...
        private function format_username_with_badges($label, $user_id = 0)
        {
            ...
            if (
                $user_id > 0 &&
                function_exists("bhg_format_user_with_badges")
            ) {
	...
	*/
        private function get_user_bonus_hunt_rows($user_id, $limit = 50)
        {
            ...
            $guesses_tbl = esc_sql(
                $this->sanitize_table($wpdb->prefix . "bhg_guesses")
            );
            $winners_tbl = esc_sql(
                $this->sanitize_table($wpdb->prefix . "bhg_hunt_winners")
            );
            ...
            $order_expression =
                "COALESCE(h.closed_at, h.updated_at, h.created_at, g.created_at)";
            $select_columns =
                "h.id, h.title, h.status, h.final_balance, h.closed_at, h.winners_count, g.guess, g.created_at AS guess_date";
            ...
            $sql .=
                " WHERE g.user_id = %d ORDER BY " .
                $order_expression .
                " DESC, h.id DESC LIMIT %d";
            $args[] = $user_id;
            $args[] = $limit;

            $prepared = call_user_func_array(
                [$wpdb, "prepare"],
                array_merge([$sql], $args)
            );
            ...
            foreach ($rows as $row) {
                ...
                if (
                    isset($row->final_balance) &&
                    "" !== $row->final_balance &&
                    null !== $row->final_balance
                ) {
                ...
                $winners_limit = isset($row->winners_count)
                    ? (int) $row->winners_count
                    : 0;
                if ($winners_limit <= 0) {
                    $winners_limit = 1;
                }
                ...
                $guess_date = isset($row->guess_date)
                    ? (string) $row->guess_date
                    : "";
        ...
        private function get_user_tournament_rows($user_id)
        {
            ...
            $has_total_points = $this->table_has_column(
                $results_tbl,
                "total_points"
            );
            $total_points_field = $has_total_points
                ? "r.total_points"
                : "r.points";
            ...
            if (
                $has_total_points &&
                $wpdb->last_error &&
                false !== stripos($wpdb->last_error, "total_points")
            ) {
                ...
                $rows = $wpdb->get_results(
                    $wpdb->prepare($fallback_sql, $user_id)
                );
            }
            ...
            if (!empty($tournament_ids)) {
                $placeholders = implode(
                    ", ",
                    array_fill(0, count($tournament_ids), "%d")
                );
                $args = array_values($tournament_ids);
                $total_points_order = $has_total_points
                    ? "total_points"
                    : "points";
            ...
            if (!empty($rank_rows)) {
                ...
                foreach ($rank_rows as $rank_row) {
                    $tid = isset($rank_row->tournament_id)
                        ? (int) $rank_row->tournament_id
                        : 0;
                    $uid = isset($rank_row->user_id)
                        ? (int) $rank_row->user_id
                        : 0;
            ...
            foreach ($rows as $row) {
                ...
                $formatted[] = [
                    ...
                    "status" => isset($row->status)
                        ...
                        : "",
                    "points" => isset($row->points) ? (int) $row->points : 0,
                    "total_points" => isset($row->total_points)
                        ? (int) $row->total_points
                        : 0,
                    "wins" => isset($row->wins) ? (int) $row->wins : 0,
                    "last_win_date" => isset($row->last_win_date)
                        ? (string) $row->last_win_date
                        : "",
                    "rank" => $rank,
                ];
            }
        ...
        private function get_user_prize_rows($user_id)
        {
            ...
            $relation_tbl = esc_sql(
                $this->sanitize_table($wpdb->prefix . "bhg_hunt_prizes")
            );
            $prizes_tbl = esc_sql(
                $this->sanitize_table($wpdb->prefix . "bhg_prizes")
            );
            ...
            foreach ($rows as $row) {
                $formatted[] = [
                    "prize_id" => isset($row->prize_id)
                        ? (int) $row->prize_id
                        : 0,
                    "title" => isset($row->prize_title)
                        ? (string) $row->prize_title
                        : "",
                    "category" => isset($row->category)
                        ? (string) $row->category
                        : "",
                    "hunt_title" => isset($row->hunt_title)
                        ? (string) $row->hunt_title
                        : "",
                    "closed_at" => isset($row->closed_at)
                        ? (string) $row->closed_at
                        : "",
         ...
         * @return string
         */
        private function render_prize_card(
            $prize,
            $size,
            $display = [],
            $context = []
            ...
            if (
                isset($context["display_defaults"]) &&
                is_array($context["display_defaults"])
            ) {
            ...
            $image_id = class_exists("BHG_Prizes")
                ? BHG_Prizes::get_image_id_for_size($prize, $size)
                : 0;
            $category = isset($prize->category)
            ...
            $show_image = BHG_Prizes::resolve_display_flag(
                ...
                array_key_exists("show_image", $defaults)
                    ? $defaults["show_image"]
                    : null
            );

            $action_override = isset($display["click_action"])
                ? $display["click_action"]
                : "inherit";
            $prize_action = isset($prize->click_action)
                ? $prize->click_action
                : "link";
            $default_click = isset($defaults["click_action"])
                ? $defaults["click_action"]
                : "inherit";
            $click_action = BHG_Prizes::resolve_click_action(
                $action_override,
                $prize_action,
                $default_click
            );
            ...
            $target = "_self";
            $rel_attr = "";
            if (in_array($click_action, ["link", "new"], true)) {
                ...
                if ("" === $link_href) {
                    ...
                    if ("new" === $click_action) {
                        ...
                        $default_link_target = isset($defaults["link_target"])
                            ? $defaults["link_target"]
                            : "inherit";
                        $target = BHG_Prizes::resolve_link_target(
                            isset($display["link_target"])
                                ? $display["link_target"]
                                : "inherit",
                            isset($prize->link_target)
                                ? $prize->link_target
                                : "_self",
                            $default_link_target
                        );
            ...
            $classes = ["bhg-prize-card"];
            if ($style_attr) {
                $classes[] = "bhg-prize-card--custom";
            }
            if (
                !empty($prize->css_border) ||
                !empty($prize->css_border_color)
            ) {
            ...
            if ("none" !== $click_action && "" !== $link_href) {
                ...
                if ("image" === $click_action) {
                    $attributes .=
                        ' data-bhg-prize-popup="image" data-bhg-prize-alt="' .
                        esc_attr($prize->title) .
                        '"';
                }
            }
            ...
            if ($show_image && $image_url) {
                $img_attr = "";
                $srcset = $image_id
                    ? wp_get_attachment_image_srcset($image_id, $wp_size)
                    : "";
                ...
                $sizes = $image_id
                    ? wp_get_attachment_image_sizes($image_id, $wp_size)
                    : "";
                if (!$sizes && "bhg_prize_big" === $wp_size) {
                    $sizes = $image_id
                        ? wp_get_attachment_image_sizes($image_id, "large")
                        : "";
                }
                if ($srcset) {
                ...
                echo '<div class="bhg-prize-image"><img src="' .
                    esc_url($image_url) .
                    '" alt="' .
                    esc_attr($prize->title) .
                    '"' .
            ...
            if ($show_title) {
                echo '<h5 class="bhg-prize-title">' .
                    esc_html($prize->title) .
                    "</h5>";
            }
            if ($show_category && $category) {
                ...
                $category_link_data = BHG_Prizes::get_category_link_data(
                    $prize
                );

                $link_allowed = BHG_Prizes::resolve_display_flag(
                    isset($display["category_links"])
                        ? $display["category_links"]
                        : null,
                    $category_link_data["enabled"],
                    array_key_exists("category_links", $defaults)
                        ? $defaults["category_links"]
                        : null
                );
                ...
                if (
                    $link_allowed &&
                    $category_link_data["enabled"] &&
                    $category_link_data["url"]
                ) {
                    $category_target = BHG_Prizes::resolve_link_target(
                        isset($display["category_target"])
                            ? $display["category_target"]
                            : "inherit",
                        $category_link_data["target"],
                        isset($defaults["category_target"])
                            ? $defaults["category_target"]
                            : "inherit"
                    );
                    $rel =
                        "_blank" === $category_target
                            ? ' rel="noopener noreferrer"'
                            : "";
                    $category_label =
                        '<a href="' .
                        esc_url($category_link_data["url"]) .
                        '" target="' .
                        esc_attr($category_target) .
                        '"' .
                        $rel .
                        ">" .
                        $category_label .
         ...
         */
        public function login_hint_shortcode($atts = [])
        {
            unset($atts); // Parameter unused but kept for shortcode signature.

            if (is_user_logged_in()) {
                return $this->shortcode_admin_notice(
                    "BHG login notice suppressed: user already logged in."
                );
            }
            ...
            $redirect = esc_url_raw(add_query_arg([], $base));

            $login_url = function_exists("bhg_get_login_url")
                ? bhg_get_login_url($redirect)
                : wp_login_url($redirect);

            return "<p>" .
                esc_html(
                    bhg_t(
                        "notice_login_to_continue",
                ...
                esc_url($login_url) .
                '">' .
                esc_html(bhg_t("button_log_in", "Log in")) .
                "</a></p>";
        }
        ...
        public function active_hunt_shortcode($atts)
        {
            ...
            $prize_layout = $this->normalize_prize_layout(
                isset($atts["prize_layout"])
                    ? $atts["prize_layout"]
                    : $default_layout
            );
            $prize_size = $this->normalize_prize_size(
                isset($atts["prize_size"]) ? $atts["prize_size"] : "medium"
            );
            ...
            if (false === $hunts) {
                ...
                $hunts = $wpdb->get_results($wpdb->prepare($sql, "open")); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
                wp_cache_set($cache_key, $hunts, "bhg", 300);
            }

            if (empty($hunts)) {
                return '<div class="bhg-active-hunt"><p>' .
                    esc_html(
                        bhg_t(
                            "notice_no_active_hunts",
                            "No active bonus hunts at the moment."
                        )
                    ) .
                    "</p></div>";
            }
            ...
            if (!class_exists("BHG_Bonus_Hunts") && defined("BHG_PLUGIN_DIR")) {
                $hunts_class =
                    BHG_PLUGIN_DIR . "includes/class-bhg-bonus-hunts.php";
            ...
            $selected_hunt_id = 0;
            if (isset($_GET["bhg_hunt"])) {
                // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Viewing data.
                $selected_hunt_id = absint(wp_unslash($_GET["bhg_hunt"])); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            }

            if (
                $selected_hunt_id <= 0 ||
                !isset($hunts_map[$selected_hunt_id])
            ) {
            ...
            if (
                !class_exists("BHG_Buttons") &&
                file_exists(BHG_PLUGIN_DIR . "includes/class-bhg-buttons.php")
            ) {
            ...
            $active_default = function_exists("bhg_get_shortcode_rows_per_page")
                ? bhg_get_shortcode_rows_per_page(30)
                : 30;
            $per_page = (int) apply_filters(
                "bhg_active_hunt_per_page",
                $active_default
            );
            if ($per_page <= 0) {
            ...
            $current_page = 1;
            if (isset($_GET["bhg_hunt_page"])) {
                // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Viewing data.
                $current_page = max(
                    1,
                    absint(wp_unslash($_GET["bhg_hunt_page"]))
                ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            }

            $guesses_table = esc_sql(
                $this->sanitize_table($wpdb->prefix . "bhg_guesses")
            );
            ...
            $final_balance = isset($selected_hunt->final_balance)
                ? $selected_hunt->final_balance
                : null;
            ...
            if ($has_final) {
                ...
                $rows = $wpdb->get_results(
                    $wpdb->prepare($sql, $selected_hunt_id, $per_page, $offset)
                ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            }
            ...
            $total_pages =
                $total_guesses > 0 ? (int) ceil($total_guesses / $per_page) : 1;

            $win_limit_notice = "";
            if (
                $has_final &&
                method_exists("BHG_Bonus_Hunts", "get_ineligible_winner_ids") &&
                function_exists("bhg_get_win_limit_config") &&
                function_exists("bhg_build_win_limit_notice")
            ) {
                $skipped_ids = BHG_Bonus_Hunts::get_ineligible_winner_ids(
                    $selected_hunt_id
                );
                if (!empty($skipped_ids)) {
                    $limit_config = bhg_get_win_limit_config("hunt");
                    $limit_count = isset($limit_config["count"])
                        ? (int) $limit_config["count"]
                        : 0;
                    $limit_period = isset($limit_config["period"])
                        ? $limit_config["period"]
                        : "none";
                    $notice_text = bhg_build_win_limit_notice(
                        "hunt",
                        $limit_count,
                        $limit_period,
                        count($skipped_ids)
            ...
            wp_enqueue_style(
                "bhg-shortcodes",
                (defined("BHG_PLUGIN_URL")
                    ? BHG_PLUGIN_URL
                    : plugins_url("/", __FILE__)) .
                    "assets/css/bhg-shortcodes.css",
                [],
            ...
            wp_enqueue_script(
                ...
                (defined("BHG_PLUGIN_URL")
                    ? BHG_PLUGIN_URL
                    : plugins_url("/", __FILE__)) .
                    "assets/js/bhg-shortcodes.js",
                [],
                defined("BHG_VERSION") ? BHG_VERSION : null,
                true
            );

            $hunt_site_id = isset($selected_hunt->affiliate_site_id)
            ...
            if (count($hunts) > 1) {
                ...
                if (!empty($_GET)) {
                    // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Preserving query vars.
                    foreach (wp_unslash($_GET) as $key => $value) {
                        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
                        if (
                            in_array($key, ["bhg_hunt", "bhg_hunt_page"], true)
                        ) {
                            continue;
                        }
                        if (is_array($value)) {
                ...
                echo '<label for="bhg-hunt-select">' .
                    esc_html(bhg_t("label_choose_hunt", "Choose a hunt:")) .
                    "</label>";
                ...
                foreach ($hunts as $hunt) {
                    ...
                    echo '<option value="' .
                        esc_attr($hunt_id) .
                        '"' .
                        selected($hunt_id, $selected_hunt_id, false) .
                        ">" .
                        esc_html($hunt->title) .
                        "</option>";
                }
                echo "</select>";
                echo '<noscript><button type="submit" class="button button-primary">' .
                    esc_html(bhg_t("button_apply", "Apply")) .
                    "</button></noscript>";
                echo "</form>";
            }

            $show_num_bonuses = isset($selected_hunt->num_bonuses_visible)
                ? (int) $selected_hunt->num_bonuses_visible
                : 1;
            $show_affiliate = isset($selected_hunt->affiliate_visible)
                ? (int) $selected_hunt->affiliate_visible
                : 1;
            $guess_url = function_exists("bhg_get_guess_submission_url")
                ? bhg_get_guess_submission_url($selected_hunt_id)
                : add_query_arg(
                    ["bhg_hunt" => $selected_hunt_id],
                    get_permalink()
                );
            ...
            echo "<li><strong>" .
                esc_html(bhg_t("label_start_balance", "Starting Balance")) .
                ":</strong> " .
                esc_html(
                    bhg_format_money((float) $selected_hunt->starting_balance)
                ) .
                "</li>";
            if ($show_num_bonuses) {
                echo "<li><strong>" .
                    esc_html(
                        bhg_t("label_number_bonuses", "Number of Bonuses")
                    ) .
                    ":</strong> " .
                    (int) $selected_hunt->num_bonuses .
                    "</li>";
            }

            $opened_at = isset($selected_hunt->created_at)
                ? (string) $selected_hunt->created_at
                : "";
            if ("" !== $opened_at && "0000-00-00 00:00:00" !== $opened_at) {
                $opened_label = mysql2date(
                    get_option("date_format") . " " . get_option("time_format"),
                    $opened_at,
                    true
                );
                echo "<li><strong>" .
                    esc_html(bhg_t("label_opened", "Opened")) .
                    ":</strong> " .
                    esc_html($opened_label) .
                    "</li>";
            }

            $closed_at = isset($selected_hunt->closed_at)
                ? (string) $selected_hunt->closed_at
                : "";
            $closed_display = "-";
            if ("" !== $closed_at && "0000-00-00 00:00:00" !== $closed_at) {
                $closed_display = mysql2date(
                    get_option("date_format") . " " . get_option("time_format"),
                    $closed_at,
                    true
                );
            ...
            $status_value =
                "open" === $selected_hunt->status
                    ? bhg_t("status_active_guess_now", "Active: Guess Now")
                    : bhg_t("status_closed", "Closed");
            ...
            echo "<li><strong>" .
                ...
                ":</strong> " .
                wp_kses_post($status_value) .
                "</li>";

            $affiliate_name = isset($selected_hunt->affiliate_site_name)
                ? $selected_hunt->affiliate_site_name
                : "";
            if (
                "" === $affiliate_name &&
                isset($selected_hunt->affiliate_site_id) &&
                (int) $selected_hunt->affiliate_site_id > 0
            ) {
                $sites_table = esc_sql(
                    $this->sanitize_table(
                        $wpdb->prefix . "bhg_affiliate_websites"
            ...
            if ($show_affiliate) {
                ...
                echo "<li><strong>" .
                    esc_html(bhg_t("label_affiliate_site", "Affiliate Site")) .
                    ":</strong> " .
                    esc_html($affiliate_display) .
                    "</li>";
            }
            if (!empty($selected_hunt->prizes)) {
                echo "<li><strong>" .
                    esc_html(bhg_t("sc_prizes", "Prizes")) .
                    ":</strong> " .
                    wp_kses_post($selected_hunt->prizes) .
                    "</li>";
            }
            if ($has_final) {
                echo "<li><strong>" .
                    esc_html(bhg_t("label_final_balance", "Final Balance")) .
                    ":</strong> " .
                    esc_html(bhg_format_money((float) $final_balance)) .
                    "</li>";
            }
            echo "</ul>";

            if ("" !== $win_limit_notice) {
                echo '<div class="bhg-win-limit-notice bhg-info-block">' .
                    esc_html($win_limit_notice) .
                    "</div>";
            }
            ...
            if (!empty($premium_prizes) && is_user_logged_in()) {
                if (
                    $hunt_site_id > 0 &&
                    function_exists("bhg_is_user_affiliate_for_site")
                ) {
                    $show_premium = bhg_is_user_affiliate_for_site(
                        get_current_user_id(),
                        $hunt_site_id
                    );
                } elseif (function_exists("bhg_is_user_affiliate")) {
                    $show_premium = bhg_is_user_affiliate(
                        get_current_user_id()
                    );
                } else {
                    $show_premium = (bool) get_user_meta(
                        get_current_user_id(),
                        "bhg_is_affiliate",
                        true
                    );
            ...
            $prize_section_options = class_exists("BHG_Prizes")
                ? BHG_Prizes::prepare_section_options([
                    "layout" => $prize_layout,
                    "show_summary" => true,
                ])
                : [
            ...
            if ($show_premium && !empty($premium_prizes)) {
                ...
                $premium_options["summary_heading"] = bhg_t(
                    "premium_prize_summary_heading",
                    "Premium Prize Summary"
                );
                ...
                if (!empty($regular_prizes)) {
                    ...
                    $regular_options["summary_heading"] = bhg_t(
                        "regular_prize_summary_heading",
                        "Regular Prize Summary"
                    );
                    ...
                    $tabs = [
                        [
                            "label" => bhg_t(
                                "premium_prizes_heading",
                                "Premium Prizes"
                            ),
                            "content" => $this->render_prize_section(
                                $premium_prizes,
                                $prize_layout,
                                $prize_size,
                                [],
                    ...
                    echo '<h4 class="bhg-prize-heading bhg-prize-heading-premium">' .
                        esc_html(
                            bhg_t("premium_prizes_heading", "Premium Prizes")
                        ) .
                        "</h4>";
                    echo $this->render_prize_section(
                        $premium_prizes,
                        $prize_layout,
                        $prize_size,
                        [],
            ...
            if (class_exists("BHG_Buttons")) {
                $buttons_markup = BHG_Buttons::render_for_placement(
                    "active_bonushunt"
                );
                if ("" !== $buttons_markup) {
                    echo '<div class="bhg-hunt-buttons">' .
                        wp_kses_post($buttons_markup) .
                        "</div>";
                }
            }
            ...
            if (empty($rows)) {
                echo '<div class="bhg-info-block bhg-info-block--muted">' .
                    esc_html(
                        bhg_t("notice_no_guesses_yet", "No Guesses Yet.")
                    ) .
                    "</div>";
            } else {
                ...
                echo "<thead><tr>";
                echo '<th scope="col">' .
                    esc_html(bhg_t("label_position", "Position")) .
                    "</th>";
                echo '<th scope="col">' .
                    esc_html(bhg_t("label_username", "User")) .
                    "</th>";
                echo '<th scope="col">' .
                    esc_html(bhg_t("label_guess", "Guess")) .
                    "</th>";
                if ($has_final) {
                    echo '<th scope="col">' .
                        esc_html(bhg_t("label_difference", "Difference")) .
                        "</th>";
                }
                echo "</tr></thead><tbody>";
                foreach ($rows as $index => $row) {
                    $position = $offset + $index + 1;
                    $user_login = !empty($row->display_name)
                        ? $row->display_name
                        : $row->user_login;
                    $user_label = $user_login
                        ? $this->format_username_label($user_login)
                        : bhg_t("label_unknown_user", "Unknown user");
                    $aff_dot = bhg_render_affiliate_dot(
                        (int) $row->user_id,
                        $hunt_site_id
                    );
                    $user_cell = function_exists("bhg_format_user_with_badges")
                        ? bhg_format_user_with_badges(
                            (int) $row->user_id,
                            $user_label
                        )
                        : esc_html($user_label);
                    ...
                    echo "<tr>";
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_position", "Position")) .
                        '">' .
                        (int) $position .
                        "</td>";
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_username", "User")) .
                        '">' .
                        wp_kses_post($user_cell . " " . $aff_dot) .
                        "</td>";
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_guess", "Guess")) .
                        '">' .
                        esc_html(bhg_format_money((float) $row->guess)) .
                        "</td>";
                    if ($has_final) {
                        $diff = isset($row->diff) ? (float) $row->diff : 0.0;
                        echo '<td data-label="' .
                            esc_attr(bhg_t("label_difference", "Difference")) .
                            '">' .
            ...
            if ($total_pages > 1) {
                ...
                if (!empty($pagination_links)) {
                    echo '<nav class="bhg-pagination" aria-label="' .
                        esc_attr(bhg_t("label_pagination", "Pagination")) .
                        '">';
                    echo '<ul class="bhg-pagination-list">';
                    foreach ($pagination_links as $link) {
                        $class =
                            false !== strpos($link, "current")
                                ? ' class="bhg-current-page"'
                                : "";
                        echo "<li" .
                            $class .
                            ">" .
        ...
        public function guess_form_shortcode($atts)
        {
            ...
            if (!is_user_logged_in()) {
                ...
                return "<p>" .
                    esc_html(
                        bhg_t(
                            "notice_login_to_guess",
                            "Please log in to submit your guess."
                        )
                    ) .
                    "</p>" .
                    '<p><a class="button button-primary" href="' .
            ...
            $settings = get_option("bhg_plugin_settings");
            $min = isset($settings["min_guess_amount"])
                ? (float) $settings["min_guess_amount"]
                : 0;
            $max = isset($settings["max_guess_amount"])
                ? (float) $settings["max_guess_amount"]
                : 100000;
            $redirect_target = !empty($settings["post_submit_redirect"])
                ? wp_validate_redirect($settings["post_submit_redirect"], "")
                : "";
            $button_label = $existing_id
                ? bhg_t("button_edit_guess", "Edit Guess")
                : bhg_t("button_submit_guess", "Submit Guess");
            ...
            ob_start();
            ?>
												<form class="bhg-guess-form" method="post" action="<?php echo esc_url(
                admin_url("admin-post.php")
                  ...
                  $redirect_target
					...
					<label for="bhg-hunt-select">
						<?php echo esc_html(bhg_t("label_choose_hunt", "Choose a hunt:")); ?>
</label>
					<select id="bhg-hunt-select" name="hunt_id" required>
						<option value="">
						<?php echo esc_html(bhg_t("label_select_hunt", "Select a hunt")); ?>
</option>
												<?php foreach ($open_hunts as $oh): ?>
														<option value="<?php echo esc_attr((int) $oh->id); ?>" <?php selected(
    $hunt_id,
    (int) $oh->id
); ?>>
				...
				<?php echo esc_html(
        bhg_t("label_guess_final_balance", "Your guess (final balance):")
    ); ?>
</label>
				<input type="number" step="0.01" min="<?php echo esc_attr(
        $min
    ); ?>" max="<?php echo esc_attr($max); ?>"
					id="bhg-guess" name="guess" value="<?php echo esc_attr(
         $existing_guess
     ); ?>" required>
				<div class="bhg-error-message"></div>
								<button type="submit" class="bhg-submit-btn button button-primary"><?php echo esc_html(
            $button_label
        ); ?></button>
			</form>
        ...
        public function leaderboard_shortcode($atts)
        {
            ...
            if ($hunt_id <= 0) {
                ...
                if ($hunt_id <= 0) {
                    return "<p>" .
                        esc_html(
                            bhg_t("notice_no_hunts_found", "No hunts found.")
                        ) .
                        "</p>";
                }
            }
            ...
            if (function_exists("bhg_get_shortcode_rows_per_page")) {
                $per_page_default = bhg_get_shortcode_rows_per_page(
                    $per_page_default
                );
            }
            ...
            $allowed_field = ["position", "user", "guess"];
            $fields = array_values(
                array_intersect(
                    $allowed_field,
            ...
            $total_cache =
                "bhg_leaderboard_total_" . $hunt_id . "_" . md5($search);
            ...
            if ($total < 1) {
                return '<div class="bhg-info-block bhg-info-block--muted">' .
                    esc_html(
                        bhg_t("notice_no_guesses_yet", "No guesses yet.")
                    ) .
                    "</div>";
            ...
            $base_url = remove_query_arg(
                ["bhg_page", "bhg_orderby", "bhg_order"],
                $current_url
            );
            if ("" === $search) {
            ...
            $toggle = function ($field) use (
                ...
                $dir =
                    $orderby_key === $field && "asc" === $direction_key
                        ? "desc"
                        : "asc";
                $args = [
                    "bhg_orderby" => $field,
            ...
            $header_class = static function ($key) use (
                $orderby_key,
                $direction_key
            ) {
                $classes = ["sortable"];
                if ($orderby_key === $key) {
                    $classes[] =
                        "desc" === strtolower((string) $direction_key)
                            ? "desc"
                            : "asc";
                }
            ...
            $sort_icon_markup = static function ($field, $label_text) use (
                $orderby_key,
                $direction_key
            ) {
                $state = "";
                ...
                return '<span class="bhg-sort-icon" aria-hidden="true">' .
                    esc_html($icon) .
            ...
            foreach ($_GET as $raw_key => $v) {
                ...
                echo '<input type="hidden" name="' .
                    esc_attr($key) .
                    '" value="' .
                    esc_attr(is_array($v) ? reset($v) : wp_unslash($v)) .
                    '">';
            }
            echo '<input type="text" name="bhg_search" value="' .
                esc_attr($search) .
                '">';
            echo '<button type="submit">' .
                esc_html(bhg_t("button_search", "Search")) .
                "</button>";
            echo "</form>";
            echo '<div class="bhg-table-wrapper">';
            ...
            foreach ($fields as $field) {
                if ("position" === $field) {
                    ...
                    echo '<th class="' .
                        esc_attr(implode(" ", $classes)) .
                        '" data-column="position"><a href="' .
                        esc_url($toggle("position")) .
                        '">' .
                        esc_html($label) .
                        $sort_icon_markup("position", $label) .
                        "</a></th>";
                } elseif ("user" === $field) {
                    $classes = ["sortable"];
                    if ("user" === $orderby_key) {
                    ...
                    echo '<th class="' .
                        esc_attr(implode(" ", $classes)) .
                        '" data-column="user"><a href="' .
                        esc_url($toggle("user")) .
                        '">' .
                        esc_html($label) .
                        $sort_icon_markup("user", $label) .
                        "</a></th>";
                } elseif ("guess" === $field) {
                    $classes = ["sortable"];
                    ...
                    echo '<th class="' .
                        esc_attr(implode(" ", $classes)) .
                        '" data-column="guess"><a href="' .
                        esc_url($toggle("guess")) .
                        '">' .
                        esc_html($label) .
                        $sort_icon_markup("guess", $label) .
                        "</a></th>";
                }
            }
            echo "</tr></thead><tbody>";
            ...
            foreach ($rows as $r) {
                if ($need_user) {
                    $site_id = isset($r->affiliate_site_id)
                        ? (int) $r->affiliate_site_id
                        : 0;
                    $aff_dot = bhg_render_affiliate_dot(
                        (int) $r->user_id,
                        $site_id
                ...
                foreach ($fields as $field) {
                    if ("position" === $field) {
                        echo '<td data-column="position">' .
                            (int) $pos .
                            "</td>";
                    } elseif ("user" === $field) {
                        echo '<td data-column="user">' .
                            wp_kses_post($user_label . " " . $aff_dot) .
                            "</td>";
                    } elseif ("guess" === $field) {
                        echo '<td data-column="guess">' .
                            esc_html(bhg_format_money((float) $r->guess)) .
                            "</td>";
                    }
                }
                echo "</tr>";
        ...
        public function user_guesses_shortcode($atts)
        {
            ...
            $fields_raw = explode(",", (string) $a["fields"]);
            $allowed_field = ["hunt", "guess", "final", "user", "site"];
            $fields_arr = array_values(
                array_unique(
                    array_intersect(
                        ...
                        array_map(
                            "sanitize_key",
                            array_map("trim", $fields_raw)
                        )
                    )
                )
            );
            if (empty($fields_arr)) {
                $fields_arr = ["hunt", "user", "guess", "final"];
            }

            $need_site = in_array("site", $fields_arr, true);
            $need_users = in_array("user", $fields_arr, true);

            $paged = isset($_GET["bhg_paged"])
                ? max(1, (int) wp_unslash($_GET["bhg_paged"]))
                : max(1, (int) $a["paged"]);
            $search = isset($_GET["bhg_search"])
                ? sanitize_text_field(wp_unslash($_GET["bhg_search"]))
                : sanitize_text_field($a["search"]);
            $limit_default = function_exists("bhg_get_shortcode_rows_per_page")
                ? bhg_get_shortcode_rows_per_page(30)
                : 30;
            $limit = $limit_default;
            $offset = ($paged - 1) * $limit;
            $has_orderby_query = isset($_GET["bhg_orderby"]);
            $orderby_request = $has_orderby_query
                ? sanitize_key(wp_unslash($_GET["bhg_orderby"]))
                : sanitize_key($a["orderby"]);
            $has_order_query = isset($_GET["bhg_order"]);
            $order_request = $has_order_query
                ? sanitize_key(wp_unslash($_GET["bhg_order"]))
                : sanitize_key($a["order"]);
            $has_order_attribute =
                is_array($atts) && array_key_exists("order", $atts);

            $status_attr = sanitize_key((string) $a["status"]);
            if (!in_array($status_attr, ["open", "closed"], true)) {
                $status_attr = "";
            }
            $status_request = isset($_GET["bhg_status"])
                ? sanitize_key(wp_unslash($_GET["bhg_status"]))
                : "";
            $status_filter = in_array($status_request, ["open", "closed"], true)
                ? $status_request
                : $status_attr;
            ...
            $has_created_at = $wpdb->get_var(
                $wpdb->prepare("SHOW COLUMNS FROM {$h} LIKE %s", "created_at")
            );
            if (empty($has_created_at)) {
                error_log(
                    "Bonus Hunt Guesser: missing required column created_at in table " .
                        $h
                );
                return "<p>" .
                    esc_html(
                        bhg_t(
                            "notice_db_update_required",
                            "Database upgrade required. Please run plugin upgrades."
                        )
                    ) .
                    "</p>";
            }
            ...
            $hunt_has_final_balance =
                $hunt_context && null !== $hunt_context->final_balance;
            $is_open_hunt = !$hunt_has_final_balance;

            $aff_raw =
                is_array($atts) && array_key_exists("aff", $atts)
                    ? $atts["aff"]
                    : "";
            ...
            $count_joins = ["INNER JOIN {$h} h ON h.id = g.hunt_id"];
            $select_joins = $count_joins;
            ...
            if ("yes" === $aff_filter) {
                $count_joins[] =
                    "INNER JOIN {$um} um_aff ON um_aff.user_id = g.user_id AND um_aff.meta_key = '" .
                    esc_sql("bhg_is_affiliate") .
                    "'";
                $select_joins[] =
                    "INNER JOIN {$um} um_aff ON um_aff.user_id = g.user_id AND um_aff.meta_key = '" .
                ...
                $count_joins[] =
                    "LEFT JOIN {$um} um_aff ON um_aff.user_id = g.user_id AND um_aff.meta_key = '" .
                    esc_sql("bhg_is_affiliate") .
                    "'";
                $select_joins[] =
                    "LEFT JOIN {$um} um_aff ON um_aff.user_id = g.user_id AND um_aff.meta_key = '" .
            ...
            $timeline = isset($_GET["bhg_timeline"])
                ? sanitize_key(wp_unslash($_GET["bhg_timeline"]))
                : sanitize_key($a["timeline"]);
            ...
            $direction_key = isset($direction_map[$order_request_key])
                ? $order_request_key
                : $default_direction_key;
            ...
            $orderby_key = isset($orderby_map[$orderby_request_key])
                ? $orderby_request_key
                : $default_orderby_key;
            ...
            $select_join_sql = $select_joins
                ? " " . implode(" ", $select_joins) . " "
                : " ";
            $where_sql = implode(" AND ", $where);

            $sql =
                "SELECT g.guess, g.created_at, g.user_id, h.title, h.final_balance, h.affiliate_site_id, CASE WHEN h.final_balance IS NOT NULL THEN (h.final_balance - g.guess) END AS difference";
            if ($need_site) {
                $sql .= ", w.name AS site_name";
            }
            if ($need_users) {
                $sql .=
                    ", u.display_name AS user_display_name, u.user_login AS user_login";
            }
            ...
            $current_url = isset($_SERVER["REQUEST_URI"])
                ...
                : home_url("/");
            $base_url = remove_query_arg(
                ["bhg_orderby", "bhg_order", "bhg_paged"],
                $current_url
            );
            if ("" === $search) {
                $base_url = remove_query_arg("bhg_search", $base_url);
            ...
            if (!$rows) {
                return '<div class="bhg-info-block bhg-info-block--muted">' .
                    esc_html(
                        bhg_t("notice_no_guesses_found", "No Guesses Found.")
                    ) .
                    "</div>";
            }
            ...
            foreach ($_GET as $raw_key => $v) {
                $key = sanitize_key(wp_unslash($raw_key));
                if (in_array($key, ["bhg_search", "bhg_timeline"], true)) {
                    continue;
                }
            ...
            echo '<label for="bhg_user_guesses_search" class="screen-reader-text">' .
                esc_html(bhg_t("button_search", "Search")) .
                "</label>";
            echo '<input type="text" id="bhg_user_guesses_search" name="bhg_search" value="' .
                esc_attr($search) .
                '">';
            echo "</div>";
            ...
            echo '<label class="bhg-filter-label" for="bhg_user_guesses_timeline">';
            echo "<span>" .
                esc_html(bhg_t("label_timeline_colon", "Timeline:")) .
                "</span>";
            echo '<select name="bhg_timeline" id="bhg_user_guesses_timeline" class="bhg-filter-select">';
            $timeline_options = [
            ...
            foreach ($timeline_options as $key => $label) {
                echo '<option value="' .
                    esc_attr($key) .
                    '"' .
                    selected($selected_timeline, $key, false) .
                    ">" .
                    esc_html($label) .
                    "</option>";
            }
            ...
            echo '<th class="' .
                esc_attr($header_class("hunt")) .
                '"><a href="' .
                esc_url($toggle("hunt")) .
                '">' .
                esc_html(bhg_t("sc_hunt", "Hunt")) .
                "</a></th>";
            if ($need_users) {
                echo "<th>" . esc_html(bhg_t("label_user", "User")) . "</th>";
            }
            echo '<th class="' .
                esc_attr($header_class("guess")) .
                '"><a href="' .
                esc_url($toggle("guess")) .
                '">' .
                esc_html(bhg_t("sc_guess", "Guess")) .
                "</a></th>";
            if ($need_site) {
                echo "<th>" . esc_html(bhg_t("label_site", "Site")) . "</th>";
            }
            echo '<th class="' .
                esc_attr($header_class("final")) .
                '"><a href="' .
                esc_url($toggle("final")) .
                '">' .
                esc_html(bhg_t("sc_final", "Final")) .
                "</a></th>";
            echo '<th class="' .
                esc_attr($header_class("difference")) .
                '"><a href="' .
                esc_url($toggle("difference")) .
                '">' .
                esc_html(bhg_t("sc_difference", "Difference")) .
                "</a></th>";
            echo "</tr></thead><tbody>";
            ...
            foreach ($rows as $row) {
                ...
                if ($need_users) {
                    ...
                    if ($show_aff) {
                        $user_cell =
                            bhg_render_affiliate_dot(
                                (int) $row->user_id,
                                (int) $row->affiliate_site_id
                            ) .
                            " " .
                            $user_cell;
                    }
                ...
                echo "<td>" .
                    (isset($row->final_balance)
                        ? esc_html(
                            bhg_format_money((float) $row->final_balance)
                        )
                        : esc_html(bhg_t("label_emdash", "—"))) .
                    "</td>";
                echo "<td>" .
                    (isset($row->difference)
                        ? esc_html(bhg_format_money((float) $row->difference))
                        : esc_html(bhg_t("label_emdash", "—"))) .
                    "</td>";
                echo "</tr>";
            }
        ...
        public function latest_winners_list_shortcode($atts)
        {
            ...
            $field_config = $this->parse_field_tokens(
                $atts["fields"],
                $allowed_fields,
                ["date", "username", "prize", "bonushunt"]
            );
            ...
            $sql =
                "SELECT w.id, w.hunt_id, w.user_id, w.position, w.created_at, " .
            ...
            if (empty($rows)) {
                $empty_text =
                    "" !== $atts["empty"]
                        ? $atts["empty"]
                        : bhg_t(
                            "notice_no_winners_yet",
                            "There are no winners yet"
                        );

                return '<p class="bhg-latest-winners-empty">' .
                    esc_html($empty_text) .
                    "</p>";
            }
            ...
            if ($relation_tbl && $tournaments_tbl && !empty($hunt_ids)) {
                ...
                foreach ((array) $pairs as $pair) {
                    ...
                    $tid = isset($pair->tournament_id)
                        ? (int) $pair->tournament_id
                        : 0;
                    if ($hid > 0 && $tid > 0) {
                        if (!isset($hunt_tournaments[$hid])) {
                            $hunt_tournaments[$hid] = [];
                        }
            ...
            foreach ($rows as $row) {
                ...
                $legacy_tid = isset($row->legacy_tournament_id)
                    ? (int) $row->legacy_tournament_id
                    : 0;
            ...
            if ($tournaments_tbl && !empty($tournament_lookup_ids)) {
                ...
                foreach ((array) $results as $result) {
                    $tournament_titles[(int) $result->id] =
                        (string) $result->title;
                }
            }
            ...
            foreach ($rows as $row) {
                ...
                $hunt_title =
                    isset($row->hunt_title) && "" !== (string) $row->hunt_title
                        ? $this->format_title_label((string) $row->hunt_title)
                        : bhg_t("label_unnamed_hunt", "Untitled hunt");
                $created_at = isset($row->created_at) ? $row->created_at : "";
                $date_output = $created_at
                    ? mysql2date(get_option("date_format"), $created_at)
                    : "";
                $position = isset($row->position) ? (int) $row->position : 0;

                $user_label = "";
                if (
                    isset($row->display_name) &&
                ...
                if ($show_prize && $hunt_id > 0) {
                    if (!isset($prize_cache[$hunt_id])) {
                        ...
                        if (class_exists("BHG_Prizes")) {
                            $sets = BHG_Prizes::get_prizes_for_hunt($hunt_id, [
                                "grouped" => true,
                            ]);
                            foreach (["regular", "premium"] as $set_key) {
                                if (
                                    !empty($sets[$set_key]) &&
                                    is_array($sets[$set_key])
                                ) {
                                    foreach ($sets[$set_key] as $prize_row) {
                                        if (
                                            isset($prize_row->title) &&
                                            "" !== (string) $prize_row->title
                                        ) {
                                            $prize_cache[$hunt_id][$set_key][] =
                                                (string) $prize_row->title;
                                        }
                                    }
                                }
                    ...
                    $site_id = isset($row->affiliate_site_id)
                        ? (int) $row->affiliate_site_id
                        : 0;
                    $is_aff = false;
                    if ($user_id > 0) {
                        if (function_exists("bhg_is_user_affiliate_for_site")) {
                            $is_aff = bhg_is_user_affiliate_for_site(
                                $user_id,
                                $site_id
                            );
                        } elseif (function_exists("bhg_is_user_affiliate")) {
                ...
                if (
                    $show_tournament &&
                    $hunt_id > 0 &&
                    isset($hunt_tournaments[$hunt_id])
                ) {
                    foreach ($hunt_tournaments[$hunt_id] as $tid) {
                        if (isset($tournament_titles[$tid])) {
                            $tournament_names[] = $this->format_title_label(
                                $tournament_titles[$tid]
                            );
                        }
                    }
                }
                ...
                $field_values = [
                    "date" => $show_date
                        ? '<span class="bhg-winner-date">' .
                            esc_html($date_output) .
                            "</span>"
                        : "",
                    "position" =>
                        $show_position && $position > 0
                            ? '<span class="bhg-winner-position">#' .
                                esc_html(number_format_i18n($position)) .
                                "</span>"
                            : "",
                    "username" => $show_username
                        ? '<span class="bhg-winner-username">' .
                            wp_kses_post($user_label) .
                            "</span>"
                        : "",
                    "prize" => $show_prize
                        ? '<span class="bhg-winner-prize">' .
                            esc_html(
                                "" !== $prize_title
                                    ? $prize_title
                                    : bhg_t("label_emdash", "—")
                        ...
                        : "",
                    "bonushunt" => $show_hunt
                        ? '<span class="bhg-winner-hunt">' .
                            esc_html($hunt_title) .
                            "</span>"
                        : "",
                    "tournament" => $show_tournament
                        ? '<span class="bhg-winner-tournament">' .
                            esc_html(
                                !empty($tournament_names)
                                    ? implode(", ", $tournament_names)
                                    : bhg_t("label_emdash", "—")
                            ) .
                            "</span>"
                        : "",
                ...
                $parts = $this->build_field_parts_from_tokens(
                    $field_tokens,
                    $field_values
                );

                $items[] =
                    '<li class="bhg-latest-winner-item">' .
                    implode(
                        ' <span class="bhg-separator">&mdash;</span> ',
                        $parts
                    ) .
                    "</li>";
            }
            ...
            return '<ul class="bhg-latest-winners-list">' .
                implode("", $items) .
                "</ul>";
        }
        ...
        public function leaderboard_list_shortcode($atts)
        {
            $atts = shortcode_atts(
                [
                    "limit" => 5,
                    "fields" =>
                        "position,username,times_won,avg_hunt,avg_tournament",
                    "timeline" => "",
                    "tournament" => "",
                    "website" => "",
                    "aff" => "",
            ...
            $tournament_id = isset($atts["tournament"])
                ? (int) $atts["tournament"]
                : 0;
            $hunt_id = isset($atts["bonushunt"]) ? (int) $atts["bonushunt"] : 0;
            $website_id = isset($atts["website"])
                ? $this->resolve_affiliate_site_id($atts["website"])
                : 0;
            ...
            if (
                !in_array(
                    $orderby,
                    ["wins", "user", "avg_hunt", "avg_tournament"],
                    true
                )
            ) {
                $orderby = "wins";
            ...
            $query_result = $this->run_leaderboard_query([
                ...
                "need_avg_tournament" =>
                    $show_avg_tournament || "avg_tournament" === $orderby,
                "need_site" => false,
                "need_tournament_name" => false,
                "need_hunt_name" => false,
                "need_aff" => false,
            ...
            if (empty($query_result["rows"])) {
                ...
                return '<p class="bhg-leaderboard-list-empty">' .
                    esc_html($empty_text) .
                    "</p>";
            }

            $rows = $query_result["rows"];
            $offset = isset($query_result["offset"])
                ? (int) $query_result["offset"]
                : 0;
            ...
            foreach ($rows as $row) {
                ...
                $wins_value = isset($row->total_wins)
                    ? (int) $row->total_wins
                    : 0;
                $avg_hunt = isset($row->avg_hunt_pos)
                    ? number_format_i18n((float) $row->avg_hunt_pos, 0)
                    : bhg_t("label_emdash", "—");
                $avg_tournament = isset($row->avg_tournament_pos)
                    ? number_format_i18n((float) $row->avg_tournament_pos, 0)
                    : bhg_t("label_emdash", "—");

                $field_values = [
                    "position" => $show_position
                        ? '<span class="bhg-leaderboard-pos">#' .
                            esc_html(number_format_i18n($position_counter)) .
                            "</span>"
                        : "",
                    "username" => $show_username
                        ? '<span class="bhg-leaderboard-user">' .
                            esc_html($label) .
                            "</span>"
                        : "",
                    "times_won" => $show_times_won
                        ? '<span class="bhg-leaderboard-wins">' .
                            esc_html(number_format_i18n($wins_value)) .
                            "</span>"
                        : "",
                    "avg_hunt" => $show_avg_hunt
                        ? '<span class="bhg-leaderboard-avg-hunt">' .
                            esc_html($avg_hunt) .
                            "</span>"
                        : "",
                    "avg_tournament" => $show_avg_tournament
                        ? '<span class="bhg-leaderboard-avg-tournament">' .
                            esc_html($avg_tournament) .
                            "</span>"
                        : "",
                ];
                ...
                $items[] =
                    '<li class="bhg-leaderboard-list-item">' .
                    implode(
                        ' <span class="bhg-separator">&mdash;</span> ',
                        $parts
                    ) .
            ...
            return '<ul class="bhg-leaderboard-list">' .
                implode("", $items) .
                "</ul>";
        }
        ...
        public function tournament_list_shortcode($atts)
        {
            ...
            if (!in_array($status_filter, ["active", "closed", "all"], true)) {
            ...
            if ($range) {
                ...
                $where[] =
                    "( (start_date IS NULL OR start_date <= %s) AND (end_date IS NULL OR end_date >= %s) )";
                $params[] = $end;
                $params[] = $start;
            }
            ...
            $sql =
                "SELECT id, title, start_date, end_date, status FROM " .
                $table .
                $where_sql .
                " ORDER BY " .
                $orderby .
            ...
            $query_args = array_merge($params, [$limit]);
            ...
            if (empty($rows)) {
                $empty_text =
                    "" !== $atts["empty"]
                        ? $atts["empty"]
                        : bhg_t(
                            "notice_no_tournaments_found",
                            "No tournaments found."
                        );

                return '<p class="bhg-tournament-list-empty">' .
                    esc_html($empty_text) .
                    "</p>";
            }

            $details_base = function_exists("bhg_get_core_page_url")
                ? bhg_get_core_page_url("tournaments")
                : "";
            ...
            foreach ($rows as $row) {
                $title =
                    isset($row->title) && "" !== (string) $row->title
                        ? $this->format_title_label((string) $row->title)
                        : bhg_t(
                            "label_unnamed_tournament",
                            "Untitled tournament"
                        );
                ...
                $end_value =
                    isset($row->end_date) && $row->end_date
                        ? mysql2date(get_option("date_format"), $row->end_date)
                        : bhg_t("label_emdash", "—");
                $status_key = isset($row->status)
                ...
                $details_text =
                    "" !== $details_url
                        ? '<a href="' .
                            ...
                            '">' .
                            esc_html(
                                bhg_t("label_show_details", "Show details")
                            ) .
                            "</a>"
                        : esc_html(bhg_t("label_emdash", "—"));
                ...
                $field_values = [
                    "name" => $show_name
                        ? '<span class="bhg-tournament-name">' .
                            esc_html($title) .
                            "</span>"
                        : "",
                    "start_date" => $show_start
                        ? '<span class="bhg-tournament-start">' .
                            esc_html($start_value) .
                            "</span>"
                        : "",
                    "end_date" => $show_end
                        ? '<span class="bhg-tournament-end">' .
                            esc_html($end_value) .
                            "</span>"
                        : "",
                    "status" => $show_status
                        ? '<span class="bhg-tournament-status">' .
                            esc_html(bhg_t($status_key, ucfirst($status_key))) .
                            "</span>"
                        : "",
                    "details" => $show_details
                        ? '<span class="bhg-tournament-details">' .
                            $details_text .
                            "</span>"
                        : "",
                ];
                ...
                $items[] =
                    '<li class="bhg-tournament-list-item">' .
                    implode(
                        ' <span class="bhg-separator">&mdash;</span> ',
                        $parts
                    ) .
            ...
            return '<ul class="bhg-tournament-list">' .
                implode("", $items) .
                "</ul>";
        }
        ...
        public function bonushunt_list_shortcode($atts)
        {
            ...
            $atts = shortcode_atts(
                [
                    ...
                    "order" => "desc",
                    "fields" =>
                        "title,start_balance,final_balance,winners,status,details",
                    "empty" => "",
                ],
                $atts,
                "bhg_bonushunt_list"
            ...
            $sql =
                "SELECT id, title, starting_balance, final_balance, winners_count, status, guessing_enabled FROM " .
                $table .
                $where_sql .
                " ORDER BY " .
                $orderby .
            ...
            if (empty($rows)) {
                ...
                return '<p class="bhg-bonushunt-list-empty">' .
                    esc_html($empty_text) .
                    "</p>";
            }
            ...
            foreach ($rows as $row) {
                $title =
                    isset($row->title) && "" !== (string) $row->title
                        ? $this->format_title_label((string) $row->title)
                        : bhg_t("label_unnamed_hunt", "Untitled hunt");
                $start_balance = isset($row->starting_balance)
                    ? bhg_format_money((float) $row->starting_balance)
                    : bhg_t("label_emdash", "—");
                $final_balance =
                    isset($row->final_balance) && "" !== $row->final_balance
                        ? bhg_format_money((float) $row->final_balance)
                        : bhg_t("label_emdash", "—");
                $winners_count = isset($row->winners_count)
                    ? (int) $row->winners_count
                    : 0;
                $status_key = isset($row->status)
                    ? strtolower((string) $row->status)
                ...
                if ($show_details && $hunt_id > 0) {
                    ...
                    if (
                        "closed" === $status &&
                        function_exists("bhg_get_hunt_results_url")
                    ) {
                        ...
                        "open" === $status &&
                        function_exists("bhg_get_guess_submission_url")
                    ) {
                        $guess_enabled = isset($row->guessing_enabled)
                            ? (int) $row->guessing_enabled
                            : 1;
                        if ($guess_enabled > 0) {
                            ...
                            $details_markup =
                                "" !== $guess_url
                                    ? '<a href="' .
                                        ...
                                        '">' .
                                        esc_html(
                                            bhg_t("link_guess_now", "Guess Now")
                                        ) .
                                        "</a>"
                                    : $details_markup;
                        }
                ...
                $field_values = [
                    "title" => $show_title
                        ? '<span class="bhg-hunt-title">' .
                            esc_html($title) .
                            "</span>"
                        : "",
                    "start_balance" => $show_start
                        ? '<span class="bhg-hunt-start">' .
                            esc_html($start_balance) .
                            "</span>"
                        : "",
                    "final_balance" => $show_final
                        ? '<span class="bhg-hunt-final">' .
                            esc_html($final_balance) .
                            "</span>"
                        : "",
                    "winners" => $show_winners
                        ? '<span class="bhg-hunt-winners">' .
                            esc_html(number_format_i18n($winners_count)) .
                            "</span>"
                        : "",
                    "status" => $show_status
                        ? '<span class="bhg-hunt-status">' .
                            esc_html(bhg_t($status_key, ucfirst($status_key))) .
                            "</span>"
                        : "",
                    "details" => $show_details
                        ? '<span class="bhg-hunt-details">' .
                            $details_markup .
                            "</span>"
                        : "",
                ];
                ...
                $items[] =
                    '<li class="bhg-bonushunt-list-item">' .
                    implode(
                        ' <span class="bhg-separator">&mdash;</span> ',
                        $parts
                    ) .
            ...
            return '<ul class="bhg-bonushunt-list">' .
                implode("", $items) .
                "</ul>";
        }
        ...
        public function hunts_shortcode($atts)
        {
            ...
            if (empty($fields_arr)) {
                $fields_arr = ["title", "start", "final", "status"];
            }
            ...
            $timeline_default = in_array(
                ...
                ? $timeline_attr
                : "all_time";
            $timeline_request = isset($_GET["bhg_timeline"])
                ? sanitize_key(wp_unslash($_GET["bhg_timeline"]))
                : "";
            $timeline_ui_value = in_array(
                $timeline_request,
                $allowed_timeline_ui,
                true
            )
            ...
            $search = isset($_GET["bhg_search"])
                ? sanitize_text_field(wp_unslash($_GET["bhg_search"]))
                : sanitize_text_field($a["search"]);
            $show_search_form = $this->attribute_to_bool(
                isset($a["show_search"]) ? $a["show_search"] : "yes"
            );
            $limit_default = function_exists("bhg_get_shortcode_rows_per_page")
                ? bhg_get_shortcode_rows_per_page(30)
                : 30;
            $limit = $limit_default;
            $offset = ($paged - 1) * $limit;
            $orderby_request = isset($_GET["bhg_orderby"])
                ? sanitize_key(wp_unslash($_GET["bhg_orderby"]))
                : sanitize_key($a["orderby"]);
            $order_request = isset($_GET["bhg_order"])
                ? sanitize_key(wp_unslash($_GET["bhg_order"]))
                : sanitize_key($a["order"]);
            ...
            // Timeline handling.
            $timeline_for_range =
                "all_time" === $timeline_query_value
                    ? ""
                    : $timeline_query_value;
            ...
            if (
                "" === $default_orderby ||
                !isset($orderby_map[$default_orderby])
            ) {
            ...
            $orderby_key = isset($orderby_map[$orderby_request])
                ? $orderby_request
                : $default_orderby;
            $orderby = $orderby_map[$orderby_key];
            $order_sql = sprintf(" ORDER BY %s %s", $orderby, $direction);
            ...
            $select =
                "SELECT h.id, h.title, h.starting_balance, h.final_balance, h.winners_count, h.status, h.created_at, h.closed_at, h.guessing_enabled";
            $join = "";
            if ($need_site_field) {
            ...
            foreach ($_GET as $raw_key => $v) {
                ...
                if (
                    in_array(
                        $key,
                        ["bhg_search", "bhg_timeline", "bhg_status"],
                        true
                    )
                ) {
                    continue;
            ...
            if (!$show_search_form) {
                if ("" !== $search) {
                    echo '<input type="hidden" name="bhg_search" value="' .
                        esc_attr($search) .
                        '">';
                }
                if ("all_time" !== $timeline_ui_value) {
                    echo '<input type="hidden" name="bhg_timeline" value="' .
                        esc_attr($timeline_ui_value) .
                        '">';
                }
                if ("" !== $status_filter) {
                    echo '<input type="hidden" name="bhg_status" value="' .
            ...
            if ($show_search_form) {
                echo '<div class="bhg-search-control bhg-search-control--text">';
                echo '<label for="bhg_hunts_search" class="screen-reader-text">' .
                    esc_html(bhg_t("button_search", "Search")) .
                    "</label>";
                echo '<input type="text" id="bhg_hunts_search" name="bhg_search" value="' .
                    esc_attr($search) .
                    '">';
                echo "</div>";
            }
            ...
            echo '<div class="bhg-filter-controls">';
            echo '<label class="bhg-filter-label" for="bhg_hunts_timeline">' .
                esc_html(bhg_t("label_timeline", "Timeline")) .
                "</label>";
            ...
            foreach ($timeline_options as $value => $label) {
                echo '<option value="' .
                    esc_attr($value) .
                    '"' .
                    selected($timeline_ui_value, $value, false) .
                    ">" .
                    esc_html($label) .
                    "</option>";
            }
            ...
            echo '<label class="bhg-filter-label" for="bhg_hunts_status">' .
                esc_html(bhg_t("sc_status", "Status")) .
                "</label>";
            ...
            foreach ($status_options as $value => $label) {
                ...
                echo '<option value="' .
                    esc_attr($value) .
                    '"' .
                    selected($status_filter, $selected_status, false) .
                    ">" .
                    esc_html($label) .
                    "</option>";
            }
            ...
            echo '<th class="' .
                esc_attr($header_class("title")) .
                '"><a href="' .
                esc_url($toggle("title")) .
                '">' .
                esc_html($title_label) .
                $sort_icon_markup("title", $title_label) .
                "</a></th>";
            echo '<th class="' .
                esc_attr($header_class("start")) .
                '"><a href="' .
                esc_url($toggle("start")) .
                '">' .
                esc_html($start_label) .
                $sort_icon_markup("start", $start_label) .
                "</a></th>";
            echo '<th class="' .
                esc_attr($header_class("final")) .
                '"><a href="' .
                ...
                $sort_icon_markup("final", $final_label) .
                "</a></th>";
            echo '<th class="' .
                esc_attr($header_class("winners")) .
                '"><a href="' .
                esc_url($toggle("winners")) .
                '">' .
                esc_html($winners_label) .
                $sort_icon_markup("winners", $winners_label) .
                "</a></th>";
            echo '<th class="' .
                esc_attr($header_class("status")) .
                '"><a href="' .
                esc_url($toggle("status")) .
                '">' .
                esc_html($status_label) .
                $sort_icon_markup("status", $status_label) .
                "</a></th>";
            ...
            foreach ($rows as $row) {
                ...
                $guessing_enabled = isset($row->guessing_enabled)
                    ? (int) $row->guessing_enabled
                    : 1;
                $guess_url = function_exists("bhg_get_guess_submission_url")
                    ? bhg_get_guess_submission_url($hunt_id)
                    : "";
                $results_url = function_exists("bhg_get_hunt_results_url")
                    ? bhg_get_hunt_results_url($hunt_id)
                    : "";
                ...
                $title_markup =
                    "" !== $title_link
                        ? '<a class="bhg-hunt-title-link" href="' .
                            esc_url($title_link) .
                            '">' .
                            esc_html($row->title) .
                            "</a>"
                ...
                echo "<td>" .
                    esc_html(bhg_format_money((float) $row->starting_balance)) .
                    "</td>";
                echo "<td>" .
                    (isset($row->final_balance)
                        ? esc_html(
                        ...
                        : esc_html(bhg_t("label_emdash", "—"))) .
                    "</td>";
                $winners_display = isset($row->winners_count)
                    ? number_format_i18n((int) $row->winners_count)
                    : bhg_t("label_emdash", "—");
                ...
                if ("closed" === $status_key && "" !== $results_url) {
                    $details_html =
                        '<a class="bhg-hunt-link" href="' .
                        esc_url($results_url) .
                        '">' .
                        esc_html(bhg_t("link_show_results", "Show Results")) .
                        "</a>";
                    ...
                    if ($guessing_enabled > 0 && "" !== $guess_url) {
                        $details_html =
                            ...
                            esc_url($guess_url) .
                            '">' .
                            esc_html(bhg_t("link_guess_now", "Guess Now")) .
                            "</a>";
                    } else {
                        $details_html = esc_html(
                            bhg_t("label_guessing_closed", "Guessing Closed")
                        );
                    }
                } elseif ("" !== $results_url) {
                    $details_html =
        ...
        public function leaderboards_shortcode($atts)
        {
            ...
            $leaderboard_defaults = [
                "fields" => "pos,user,points,wins,avg_hunt,avg_tournament,aff",
                "ranking" => 0,
                "timeline" => "",
                "orderby" => "wins",
                "order" => "DESC",
                ...
                "aff" => "",
                "filters" =>
                    "timeline,tournament,affiliate_site,affiliate_status",
                "per_page" => 25,
                "paged" => 1,
                "show_search" => "yes",
            ...
            $summary_preference = strtolower(
                trim((string) $a["show_prize_summary"])
            );
            $show_prizes_attr = isset($a["show_prizes"])
                ? $a["show_prizes"]
                : "yes";
            $show_prizes = $this->attribute_to_bool($show_prizes_attr, true);

            $raw_fields = array_map(
                "trim",
                explode(",", (string) $a["fields"])
            );
            $allowed_fields = [
                "pos",
                "user",
            ...
            $filters_input =
                is_array($atts) && array_key_exists("filters", $atts)
                    ? $atts["filters"]
                    : null;
            $normalized_filters = $this->normalize_leaderboard_filters(
                $filters_input
            );
            ...
            $filter_timeline_enabled = in_array(
                "timeline",
                $enabled_filters,
                true
            );
            $filter_tournament_enabled = in_array(
                "tournament",
                $enabled_filters,
                true
            );
            $filter_site_enabled = in_array("site", $enabled_filters, true);
            $filter_affiliate_enabled = in_array(
                "affiliate",
                $enabled_filters,
                true
            );
            ...
            $search = isset($_GET["bhg_search"])
                ? sanitize_text_field(wp_unslash($_GET["bhg_search"]))
                : sanitize_text_field($a["search"]);
            $show_search_control = $this->attribute_to_bool(
                isset($a["show_search"]) ? $a["show_search"] : "yes"
            );
            ...
            $timeline_request =
                $filter_timeline_enabled && isset($_GET["bhg_timeline"])
                    ? sanitize_key(wp_unslash($_GET["bhg_timeline"]))
                    : "";
            $timeline =
                "" !== $timeline_request ? $timeline_request : $attr_timeline;
            ...
            $paged = isset($_GET["bhg_paged"])
                ? max(1, absint(wp_unslash($_GET["bhg_paged"])))
                : (int) $a["paged"];
            ...
            $default_per_page = function_exists(
                "bhg_get_shortcode_rows_per_page"
            )
                ? bhg_get_shortcode_rows_per_page(25)
                : 25;
            ...
            $shortcode_tournament = isset($a["tournament"])
                ? $a["tournament"]
                : "";
            $shortcode_bonushunt = isset($a["bonushunt"])
                ? $a["bonushunt"]
                : "";
            ...
            $raw_site =
                $filter_site_enabled && isset($_GET["bhg_site"])
                    ? wp_unslash($_GET["bhg_site"])
                    : $shortcode_site;
            $raw_aff =
                $filter_affiliate_enabled && isset($_GET["bhg_aff"])
                    ? wp_unslash($_GET["bhg_aff"])
                    : $shortcode_aff;
            ...
            if (in_array($summary_preference, ["yes", "true", "1"], true)) {
                $show_prize_summary = true;
            } elseif (in_array($summary_preference, ["auto", ""], true)) {
            ...
            if ($tournaments_table) {
                ...
                if ($tournament_filter_active) {
                    ...
                    if (!$has_selected) {
                        $sql = $wpdb->prepare(
                            "SELECT id, title FROM {$tournaments_table} WHERE id = %d",
                            $tournament_id
                        );
                    ...
                    if (
                        ...
                        if (
                            $tournament_meta &&
                            !empty($tournament_meta->prizes)
                        ) {
                            $status = strtolower(
                                (string) $tournament_meta->status
                            );
                            if ("active" === $status) {
                                $prize_maps = $this->normalize_prize_maps_from_storage(
                                    $tournament_meta->prizes,
                                    isset($tournament_meta->winners_count)
                                        ? (int) $tournament_meta->winners_count
                                        : 0
                                );
                                $prizes = $this->load_prize_sets_from_maps(
                                    $prize_maps
                                );

                                if (isset($tournament_meta->points_map)) {
                                    $decoded_points = json_decode(
                                        $tournament_meta->points_map,
                                        true
                                    );
                                    if (
                                        is_array($decoded_points) &&
                                        function_exists(
                                            "bhg_sanitize_points_map"
                                        )
                                    ) {
                                        $tournament_points_map = bhg_sanitize_points_map(
                                            $decoded_points
                                        );
                                    }
                                }
                                ...
                                if (
                                    empty($tournament_points_map) &&
                                    function_exists(
                                        "bhg_get_default_points_map"
                                    )
                                ) {
                                ...
                                if (
                                    !empty($prizes["regular"]) ||
                                    !empty($prizes["premium"])
                                ) {
                                    $section_options = class_exists(
                                        "BHG_Prizes"
                                    )
                                        ? BHG_Prizes::prepare_section_options([
                                    ...
                                    if ("" !== $section_html) {
                                        $leaderboard_prizes_markup =
                                            '<div class="bhg-tournament-prizes bhg-tournament-prizes--leaderboard">';

                                        $leaderboard_prizes_markup .= wp_kses_post(
                                            $section_html
                                        );
                                    ...
                                    $leaderboard_summary_markup = $this->render_prize_summary_columns(
                                        $prizes,
                                        $section_options,
                                        $show_prize_summary
                                    );
            ...
            if ($sites_table) {
                ...
                if (
                    !empty($site_limits) &&
                    $website_id > 0 &&
                    !in_array($website_id, $site_limits, true)
                ) {
                    $site_limits[] = $website_id;
                }
                $site_limits = array_values(
                    array_unique(array_filter($site_limits))
                );

                if (!empty($site_limits)) {
                    $placeholders = implode(
                        ",",
                        array_fill(0, count($site_limits), "%d")
                    );
                    $sql = "SELECT id, name FROM {$sites_table} WHERE id IN ({$placeholders}) ORDER BY name ASC";
                    $sites = $wpdb->get_results(
                        $wpdb->prepare($sql, ...$site_limits)
                    );
                } else {
                ...
                if ($website_id > 0) {
                    ...
                    if (!$has_selected_site) {
                        $sql = $wpdb->prepare(
                            "SELECT id, name FROM {$sites_table} WHERE id = %d",
                            $website_id
                        );
            ...
            $rows = isset($query_result["rows"]) ? $query_result["rows"] : [];
            $total = isset($query_result["total"])
                ? (int) $query_result["total"]
                : 0;
            $per_page = isset($query_result["per_page"])
                ? (int) $query_result["per_page"]
                : $per_page;
            $paged = isset($query_result["paged"])
                ? (int) $query_result["paged"]
                : $paged;
            $offset = isset($query_result["offset"])
                ? (int) $query_result["offset"]
                : $offset;
            $total_pages = isset($query_result["total_pages"])
                ? (int) $query_result["total_pages"]
                : 1;
            $orderby_key = isset($query_result["orderby_key"])
                ? sanitize_key($query_result["orderby_key"])
                : $orderby_request;
            $direction_key = isset($query_result["direction_key"])
                ? sanitize_key($query_result["direction_key"])
                : $direction_key;
            ...
            $show_timeline_filter = in_array(
                "timeline",
                $enabled_filters,
                true
            );
            $show_tournament_filter =
                in_array("tournament", $enabled_filters, true) &&
                !empty($tournaments);
            $show_site_filter =
                in_array("site", $enabled_filters, true) && !empty($sites);
            $show_affiliate_filter = in_array(
                "affiliate",
                $enabled_filters,
                true
            );
            $has_filter_dropdowns =
                $show_timeline_filter ||
                $show_tournament_filter ||
                $show_site_filter ||
                $show_affiliate_filter;
            ...
            if ($has_filter_dropdowns) {
                ...
                if ($show_timeline_filter) {
                    $timeline_options = [
                        "all_time" => bhg_t(
                            "option_timeline_all_time",
                            "Alltime"
                        ),
                        "today" => bhg_t("option_timeline_today", "Today"),
                        "this_week" => bhg_t(
                            "option_timeline_this_week",
                            "This Week"
                        ),
                        "this_month" => bhg_t(
                            "option_timeline_this_month",
                            "This Month"
                        ),
                        "this_quarter" => bhg_t(
                            "option_timeline_this_quarter",
                        ...
                        "this_year" => bhg_t(
                            "option_timeline_this_year",
                            "This Year"
                        ),
                        "last_year" => bhg_t(
                            "option_timeline_last_year",
                            "Last Year"
                        ),
                    ];

                    echo '<label class="bhg-filter-label">' .
                        esc_html(bhg_t("label_filter_timeline", "Timeline"));
                    echo '<select name="bhg_timeline" class="bhg-filter-select">';
                    foreach ($timeline_options as $value => $label) {
                        echo '<option value="' .
                            esc_attr($value) .
                            '"' .
                            selected($timeline, $value, false) .
                            ">" .
                            esc_html($label) .
                            "</option>";
                    }
                ...
                if ($show_tournament_filter) {
                    echo '<label class="bhg-filter-label">' .
                        esc_html(
                            bhg_t("label_filter_tournament", "Tournament")
                        );
                    echo '<select name="bhg_tournament" class="bhg-filter-select">';
                    echo '<option value="">' .
                        esc_html(
                            bhg_t("option_all_tournaments", "All tournaments")
                        ) .
                        "</option>";
                    foreach ($tournaments as $tournament) {
                        echo '<option value="' .
                ...
                if ($show_site_filter) {
                    echo '<label class="bhg-filter-label">' .
                        esc_html(bhg_t("label_filter_site", "Affiliate Site"));
                    echo '<select name="bhg_site" class="bhg-filter-select">';
                    echo '<option value="">' .
                        esc_html(
                            bhg_t("option_all_sites", "All affiliate sites")
                        ) .
                        "</option>";
                    foreach ($sites as $site) {
                        echo '<option value="' .
                            (int) $site->id .
                            '"' .
                            selected($website_id, (int) $site->id, false) .
                            ">" .
                            esc_html($site->name) .
                            "</option>";
                    }
                ...
                if ($show_affiliate_filter) {
                    echo '<label class="bhg-filter-label">' .
                        esc_html(
                            bhg_t("label_filter_affiliate", "Affiliate Status")
                        );
                    echo '<select name="bhg_aff" class="bhg-filter-select">';
                    echo '<option value="">' .
                        esc_html(bhg_t("option_aff_all", "All users")) .
                        "</option>";
                    echo '<option value="yes"' .
                        selected($aff_filter, "yes", false) .
                        ">" .
                        esc_html(bhg_t("option_aff_only", "Affiliates only")) .
                        "</option>";
                    echo '<option value="no"' .
                        selected($aff_filter, "no", false) .
                        ">" .
                        esc_html(
                            bhg_t("option_aff_none", "Non-affiliates only")
                        ) .
                        "</option>";
                    echo "</select></label>";
                }
            ...
            $heading_parts = [];
            if (
                $selected_tournament_row &&
                !empty($selected_tournament_row->title)
            ) {
                $heading_parts[] =
                    '<h2 class="bhg-leaderboard-heading bhg-leaderboard-heading--tournament">' .
                    esc_html($selected_tournament_row->title) .
                    "</h2>";
            }
            if (
                $selected_bonushunt_row &&
                !empty($selected_bonushunt_row->title)
            ) {
                $heading_parts[] =
                    '<h2 class="bhg-leaderboard-heading bhg-leaderboard-heading--bonushunt">' .
                    esc_html($selected_bonushunt_row->title) .
                    "</h2>";
            }
            if (!empty($heading_parts)) {
                echo '<div class="bhg-leaderboard-headings">' .
                    implode("", $heading_parts) .
                    "</div>";
            }
            ...
            if (
                ...
                echo '<div class="bhg-prize-summary-wrap">' .
                    wp_kses_post($leaderboard_summary_markup) .
                    "</div>";
            }
            ...
            foreach ($fields_arr as $field) {
                if ("pos" === $field) {
                    $label = bhg_t("sc_position", "Position");
                    echo '<th class="sortable"><a href="' .
                        esc_url($toggle("pos")) .
                        '">' .
                        esc_html($label) .
                        $sort_icon_markup("pos", $label) .
                        "</a></th>";
                } elseif ("user" === $field) {
                    $label = bhg_t("sc_user", "User");
                    echo '<th class="sortable"><a href="' .
                    ...
                    $label = bhg_t("label_points", "Points");
                    ...
                    echo '<th class="sortable"><a href="' .
                        ...
                        '">' .
                        esc_html($label) .
                        $sort_icon_markup("wins", $label) .
                        "</a></th>";
                } elseif ("avg_hunt" === $field) {
                    $label = bhg_t("sc_avg_rank", "Avg Hunt Pos");
                    echo '<th class="sortable"><a href="' .
                        esc_url($toggle("avg_hunt")) .
                        '">' .
                        esc_html($label) .
                        $sort_icon_markup("avg_hunt", $label) .
                        "</a></th>";
                } elseif ("avg_tournament" === $field) {
                    $label = bhg_t(
                        "sc_avg_tournament_pos",
                        "Avg Tournament Pos"
                    );
                    echo '<th class="sortable"><a href="' .
                        esc_url($toggle("avg_tournament")) .
                        '">' .
                        esc_html($label) .
                        $sort_icon_markup("avg_tournament", $label) .
                        "</a></th>";
                } elseif ("aff" === $field) {
                    echo "<th>" .
                        esc_html(bhg_t("label_affiliate_status", "Affiliate")) .
                        "</th>";
                } elseif ("site" === $field) {
                    echo "<th>" .
                        esc_html(bhg_t("label_site", "Site")) .
                        "</th>";
                } elseif ("hunt" === $field) {
                    echo "<th>" .
                        esc_html(bhg_t("label_hunt", "Hunt")) .
                        "</th>";
                } elseif ("tournament" === $field) {
                    echo "<th>" .
                        esc_html(bhg_t("label_tournament", "Tournament")) .
                        "</th>";
                }
            }
            echo "</tr></thead><tbody>";

            echo $this->render_leaderboard_rows(
                $rows,
                $fields_arr,
                $offset,
                $need_aff,
         ...
         */
        private function render_leaderboard_rows(
            $rows,
            $fields_arr,
            $offset,
            $need_aff,
            ...
            foreach ($rows as $row) {
                ...
                foreach ($fields_arr as $field) {
                    if ("pos" === $field) {
                        ...
                        echo "<td>" .
                            esc_html((string) $points_value) .
                            "</td>";
                    } elseif ("wins" === $field) {
                        echo "<td>" . (int) $row->total_wins . "</td>";
                    } elseif ("avg_hunt" === $field) {
                        echo "<td>" .
                            (isset($row->avg_hunt_pos)
                                ? esc_html(
                                    number_format_i18n(
                                        (float) $row->avg_hunt_pos,
                                        0
                                    )
                                )
                                : esc_html(bhg_t("label_emdash", "—"))) .
                        ...
                        echo "<td>" .
                            (isset($row->avg_tournament_pos)
                                ? esc_html(
                                    number_format_i18n(
                                        (float) $row->avg_tournament_pos,
                                        0
                                    )
                                )
                                : esc_html(bhg_t("label_emdash", "—"))) .
                        ...
                        $site_label = !empty($row->site_name)
                            ? $row->site_name
                            : bhg_t("label_emdash", "—");
                        echo "<td>" . esc_html($site_label) . "</td>";
                    } elseif ("hunt" === $field) {
                        $hunt_label = !empty($row->hunt_title)
                            ? $row->hunt_title
                            : bhg_t("label_emdash", "—");
                        echo "<td>" . esc_html($hunt_label) . "</td>";
                    } elseif ("tournament" === $field) {
                        $tournament_label = !empty($row->tournament_title)
        ...
        public function tournaments_shortcode($atts)
        {
            ...
            $show_prize_summary_attr = isset($atts["show_prize_summary"])
                ? $atts["show_prize_summary"]
                : "yes";
            $show_prize_summary_detail = $this->attribute_to_bool(
                $show_prize_summary_attr,
                true
            );
            $show_prizes_attr = isset($atts["show_prizes"])
                ? $atts["show_prizes"]
                : "yes";
            $show_prizes = $this->attribute_to_bool($show_prizes_attr, true);
            $default_prize_layout = $this->get_default_prize_layout();
            ...
            if ($details_id > 0) {
                ...
                $site_join = $include_sites
                    ? " LEFT JOIN {$s} s ON s.id = t.affiliate_site_id"
                    : "";
                ...
                $prize_maps = $this->normalize_prize_maps_from_storage(
                    $tournament->prizes,
                    isset($tournament->winners_count)
                        ? (int) $tournament->winners_count
                        : 0
                );
                $prizes = $this->load_prize_sets_from_maps($prize_maps);
                $prize_summary_markup = "";
                ...
                if (isset($_GET["bhg_tr_per_page"])) {
                    $per_page_override = max(
                        1,
                        absint(wp_unslash($_GET["bhg_tr_per_page"]))
                    );
                ...
                $per_page = (int) apply_filters(
                    "bhg_tournament_leaderboard_per_page",
                    $per_page,
                    $tournament
                );
                if ($per_page <= 0) {
                ...
                $site_filter = isset($_GET["bhg_site"])
                    ? $this->resolve_affiliate_site_id(
                        wp_unslash($_GET["bhg_site"])
                    )
                    : 0;

                $orderby = isset($_GET["orderby"])
                    ? strtolower(sanitize_key(wp_unslash($_GET["orderby"])))
                    : "position";
                $allowed_orders = ["asc", "desc"];
                ...
                $current_page = isset($_GET["bhg_tr_paged"])
                    ? max(1, absint(wp_unslash($_GET["bhg_tr_paged"])))
                    : 1;
                $offset = ($current_page - 1) * $per_page;
                ...
                switch ($orderby) {
                    ...
                    case "wins":
                        ...
                        $order_parts[] =
                            "r.points " . ("DESC" === $order ? "DESC" : "ASC");
                        $order_parts[] =
                            "r.last_win_date " .
                            ("DESC" === $order ? "ASC" : "DESC");
                        break;
                    case "points":
                        $order_parts[] = "r.points " . $order;
                        $order_parts[] =
                            "r.wins " . ("DESC" === $order ? "DESC" : "ASC");
                        $order_parts[] =
                            "r.last_win_date " .
                            ("DESC" === $order ? "ASC" : "DESC");
                        break;
                ...
                $tournament_affiliate = get_post_meta(
                    (int) $tournament->id,
                    "affiliate_site",
                    true
                );
                $tournament_affiliate_id = isset($tournament->affiliate_site_id)
                    ? (int) $tournament->affiliate_site_id
                    : 0;
                $manual_filtering =
                    $apply_site_filter ||
                    "" !== $tournament_affiliate ||
                    $tournament_affiliate_id > 0;

                if ($manual_filtering) {
                    $rows_all = $wpdb->get_results(
                        $wpdb->prepare($select_sql, $tournament->id)
                    ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
                    $rows_all = BHG_Tournaments_Controller::filter_results_by_affiliate(
                        (int) $tournament->id,
                        (array) $rows_all,
                    ...
                    foreach ((array) $rows_all as $row_obj) {
                        ...
                        if (
                            $apply_site_filter &&
                            !$this->user_matches_affiliate_site(
                                $uid,
                                $site_filter
                            )
                        ) {
                ...
                if (empty($rows)) {
                    echo "<p>" .
                        esc_html(
                            bhg_t("notice_no_results_yet", "No results yet.")
                        ) .
                        "</p>";
                    echo "</div>";
                    return ob_get_clean();
                ...
                if ($rows) {
                    ...
                    if ($hw_table && $hunts_tbl && $rel_table) {
                        ...
                        if (!empty($user_ids)) {
                            ...
                            $last_win_args = array_merge(
                                array_values($user_ids),
                                [$tournament->id, $tournament->id]
                            );
                            $win_rows = $wpdb->get_results(
                                $wpdb->prepare($last_win_sql, ...$last_win_args)
                            );
                            if ($win_rows) {
                                foreach ($win_rows as $win_row) {
                                    $uid = isset($win_row->user_id)
                                        ? (int) $win_row->user_id
                                        : 0;
                                    if (
                                        $uid > 0 &&
                                        !empty($win_row->last_win_date)
                                    ) {
                                        $last_win_map[$uid] =
                                            $win_row->last_win_date;
                                    }
                                }
                            }
                        }
                ...
                $base = remove_query_arg(["orderby", "order", "bhg_tr_paged"]);
                $toggle = function ($key) use ($orderby, $order, $base) {
                    $next =
                        $orderby === $key &&
                        "ASC" === strtoupper((string) $order)
                            ? "desc"
                            : "asc";
                ...
                echo '<p><a href="' .
                    esc_url(remove_query_arg("bhg_tournament_id")) .
                    '">&larr; ' .
                    esc_html(
                        bhg_t(
                    ...
                    "</a></p>";
                $heading = $tournament->title
                    ? $tournament->title
                    : bhg_t("label_tournament", "Tournament");
                ...
                if ("active" === $status_key && !empty($tournament->end_date)) {
                    try {
                        ...
                        $end_date_obj = new DateTimeImmutable(
                            $tournament->end_date,
                            $tz
                        );
                        $end_of_period = $end_date_obj->setTime(23, 59, 59);
                        $seconds_left =
                            $end_of_period->getTimestamp() -
                            $now->getTimestamp();
                        if ($seconds_left >= 0) {
                ...
                echo '<div class="bhg-tournament-header">';
                echo '<h3 class="bhg-tournament-title">' .
                    esc_html($heading) .
                    "</h3>";
                echo '<div class="bhg-tournament-meta">';
                echo '<div class="bhg-tournament-meta__item"><span class="bhg-tournament-meta__label">' .
                    esc_html(bhg_t("sc_start", "Start")) .
                    '</span><span class="bhg-tournament-meta__value">' .
                    esc_html(
                        mysql2date(
                            get_option("date_format"),
                ...
                if (!empty($tournament->description)) {
                    echo '<div class="bhg-tournament-description">' .
                        wp_kses_post(wpautop($tournament->description)) .
                        "</div>";
                }
                ...
                if (
                    !class_exists("BHG_Buttons") &&
                    file_exists(
                        BHG_PLUGIN_DIR . "includes/class-bhg-buttons.php"
                    )
                ) {
                    require_once BHG_PLUGIN_DIR .
                        "includes/class-bhg-buttons.php";
                }

                if (class_exists("BHG_Buttons")) {
                    $tournament_buttons = BHG_Buttons::render_for_placement(
                        "active_tournament"
                    );
                    if ("" !== $tournament_buttons) {
                        echo '<div class="bhg-tournament-buttons">' .
                            wp_kses_post($tournament_buttons) .
                            "</div>";
                    }
                }

                $affiliate_name = isset($tournament->affiliate_site_name)
                    ? (string) $tournament->affiliate_site_name
                    : "";
                $affiliate_url = isset($tournament->affiliate_website)
                    ? (string) $tournament->affiliate_website
                    : "";
                if (
                    "" === $affiliate_url &&
                    isset($tournament->affiliate_site_url)
                ) {
                    $affiliate_url = (string) $tournament->affiliate_site_url;
                }
                $show_affiliate_url = isset($tournament->affiliate_url_visible)
                    ? (int) $tournament->affiliate_url_visible
                    : 0;

                if ("" !== $affiliate_name || "" !== $affiliate_url) {
                    echo '<p class="bhg-tournament-affiliate"><strong>' .
                        esc_html(bhg_t("label_affiliate", "Affiliate")) .
                        ":</strong> ";
                ...
                if ($show_prizes) {
                    $section_options = class_exists("BHG_Prizes")
                        ? BHG_Prizes::prepare_section_options([
                            "layout" => $default_prize_layout,
                    ...
                    if ("" !== $prize_markup) {
                        echo '<div class="bhg-tournament-prizes">' .
                            wp_kses_post($prize_markup) .
                            "</div>";
                    }

                    $prize_summary_markup = $this->render_prize_summary_columns(
                        $prizes,
                        $section_options,
                        $show_prize_summary_detail
                    );
                    if ("" !== $prize_summary_markup) {
                        echo '<div class="bhg-prize-summary-wrap">' .
                            wp_kses_post($prize_summary_markup) .
                            "</div>";
                    }
                }
                ...
                if ($days_remaining > 0) {
                    $days_label =
                        1 === $days_remaining
                            ? bhg_t(
                                "tournament_closes_in_one_day",
                                "This tournament will close in 1 day."
                            )
                            : sprintf(
                                bhg_t(
                                    "tournament_closes_in_days",
                                    "This tournament will close in %s days."
                                ),
                                number_format_i18n($days_remaining)
                            );
                    echo '<div class="bhg-tournament-details bhg-countdown">';
                    echo '<div class="bhg-tournament-countdown">' .
                        esc_html($days_label) .
                        "</div>";
                    echo "</div>";
                }
                ...
                $points_label = bhg_t("label_points", "Points");
                $wins_label = bhg_t("sc_wins", "Times Won");
                $last_win_label = bhg_t("label_last_win", "Last win");
                echo '<th scope="col" class="sortable"><a href="' .
                    esc_url($toggle("position")) .
                    '">' .
                    esc_html($position_label) .
                    $sort_icon_markup("position", $position_label) .
                    "</a></th>";
                echo '<th scope="col" class="sortable"><a href="' .
                    esc_url($toggle("username")) .
                    '">' .
                    esc_html($username_label) .
                    $sort_icon_markup("username", $username_label) .
                    "</a></th>";
                echo '<th scope="col" class="sortable"><a href="' .
                    esc_url($toggle("points")) .
                    '">' .
                    esc_html($points_label) .
                    $sort_icon_markup("points", $points_label) .
                    "</a></th>";
                echo '<th scope="col" class="sortable"><a href="' .
                    esc_url($toggle("wins")) .
                    '">' .
                    esc_html($wins_label) .
                    $sort_icon_markup("wins", $wins_label) .
                    "</a></th>";
                echo '<th scope="col" class="sortable"><a href="' .
                    esc_url($toggle("last_win_at")) .
                    '">' .
                    esc_html($last_win_label) .
                    $sort_icon_markup("last_win_at", $last_win_label) .
                    "</a></th>";
                echo "</tr></thead><tbody>";

                $winner_limit_meta = get_post_meta(
                    (int) $tournament->id,
                    "number_of_winners",
                    true
                );
                $winner_limit_value = isset($tournament->winners_count)
                    ? (int) $tournament->winners_count
                    : 0;
                ...
                foreach ($rows as $index => $row) {
                    ...
                    $row_classes = ["bhg-tournament-row"];
                    ...
                    $class_attr =
                        ' class="' . esc_attr(implode(" ", $row_classes)) . '"';
                    ...
                    if (
                        isset($row->user_id) &&
                        isset($last_win_map[(int) $row->user_id])
                    ) {
                    ...
                    echo "<tr" . $class_attr . ">";
                    echo '<td data-label="' .
                        esc_attr($position_label) .
                        '">' .
                        (int) $position_number .
                        "</td>";
                    echo '<td data-label="' .
                        esc_attr($username_label) .
                        '">' .
                        wp_kses_post($user_label) .
                        "</td>";
                    $points_cell_value = isset($row->points)
                        ? (int) $row->points
                        : 0;
                    ...
                    if (
                        0 === $points_cell_value &&
                        isset($tournament_points_map[$position_number])
                    ) {
                        $points_cell_value =
                            (int) $tournament_points_map[$position_number];
                    }

                    echo '<td data-label="' .
                        esc_attr($points_label) .
                        '">' .
                        (int) $points_cell_value .
                        "</td>";
                    echo '<td data-label="' .
                        esc_attr($wins_label) .
                        '">' .
                ...
                if ($total_pages > 1) {
                    ...
                    if (!empty($_GET)) {
                        foreach ($_GET as $raw_key => $value) {
                            ...
                            $preserved_args[$key] = sanitize_text_field(
                                wp_unslash($value)
                            );
                        }
                    }

                    $pagination_args = array_merge($preserved_args, [
                        "orderby" => $orderby,
            ...
            $status = isset($_GET["bhg_status"])
                ? sanitize_key(wp_unslash($_GET["bhg_status"]))
                : sanitize_key($a["status"]);
            $timeline = isset($_GET["bhg_timeline"])
                ? sanitize_key(wp_unslash($_GET["bhg_timeline"]))
                : sanitize_key($a["timeline"]);
            ...
            $orderby_param = isset($_GET["bhg_orderby"])
                ? sanitize_key(wp_unslash($_GET["bhg_orderby"]))
                : sanitize_key($a["orderby"]);
            $order_param = isset($_GET["bhg_order"])
                ? sanitize_key(wp_unslash($_GET["bhg_order"]))
                : sanitize_key($a["order"]);
            $allowed_orderby = [
                "title" => "title",
            ...
            $orderby_column = isset($allowed_orderby[$orderby_param])
                ? $allowed_orderby[$orderby_param]
                : "start_date";
            $order_param = in_array(
                strtolower($order_param),
                ["asc", "desc"],
                true
            )
                ? strtoupper($order_param)
                : "DESC";
            ...
            $sql =
                ...
                " " .
                $order_param .
                " LIMIT %d OFFSET %d"; // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Order by clause sanitized via whitelist.
            $query_args = array_merge($params, [$limit, $offset]);
            ...
            $timeline_key = isset($_GET["bhg_timeline"])
                ? sanitize_key(wp_unslash($_GET["bhg_timeline"]))
                : $timeline_ui;
            ...
            foreach ($timeline_options as $key => $label) {
                echo '<option value="' .
                    esc_attr($key) .
                    '"' .
                    selected($timeline_key, $key, false) .
                    ">" .
                    esc_html($label) .
                    "</option>";
            }
            ...
            echo '<label class="bhg-filter-label">' .
                esc_html(bhg_t("status", "Status:")) .
                " ";
            echo '<select name="bhg_status" class="bhg-filter-select">';
            $statuses = [
                "active" => bhg_t("label_active", "Active"),
                "closed" => bhg_t("label_closed", "Closed"),
                "all" => bhg_t("label_all", "All"),
            ];
            $status_key = isset($_GET["bhg_status"])
                ? sanitize_key(wp_unslash($_GET["bhg_status"]))
                : $status;
            ...
            echo '<button class="bhg-filter-button" type="submit">' .
                esc_html(bhg_t("button_filter", "Filter")) .
                "</button>";
            echo "</div>";
            ...
            if ($show_search_control) {
                echo '<div class="bhg-search-control">';
                echo '<label class="bhg-filter-label">' .
                    esc_html(bhg_t("label_search", "Search")) .
                    ' <input type="text" name="bhg_search" value="' .
                    esc_attr($search) .
                    '"></label>';
                echo '<button class="bhg-filter-button" type="submit">' .
                    esc_html(bhg_t("button_search", "Search")) .
            ...
            $toggle = function ($key) use (
                ...
                $next =
                    $orderby_param === $key && "ASC" === $order_param
                        ? "desc"
                        : "asc";
                $args = [
                    "bhg_orderby" => $key,
            ...
            $header_class = static function ($key) use (
                ...
                if ($orderby_param === $key) {
                    $classes[] =
                        "DESC" === strtoupper((string) $order_param)
                            ? "desc"
                            : "asc";
                }
            ...
            echo '<th class="' .
                ...
                '">' .
                esc_html($name_label) .
                $sort_icon_markup("title", $name_label) .
                "</a></th>";
            echo '<th class="' .
                esc_attr($header_class("start_date")) .
                '"><a href="' .
                esc_url($toggle("start_date")) .
                '">' .
                esc_html($start_label) .
                $sort_icon_markup("start_date", $start_label) .
                "</a></th>";
            echo '<th class="' .
                esc_attr($header_class("end_date")) .
                '"><a href="' .
                esc_url($toggle("end_date")) .
                '">' .
                esc_html($end_label) .
                $sort_icon_markup("end_date", $end_label) .
                "</a></th>";
            echo '<th class="' .
                esc_attr($header_class("status")) .
                '"><a href="' .
            ...
            foreach ($rows as $row) {
                $detail_url = add_query_arg(
                    "bhg_tournament_id",
                    (int) $row->id,
                    remove_query_arg(["orderby", "order"], $current_url)
                );
                echo "<tr>";
                echo '<td data-label="' .
                    esc_attr(bhg_t("label_name", "Name")) .
                    '"><a href="' .
                    esc_url($detail_url) .
                    '">' .
                    esc_html(
                    ...
                    "</a></td>";
                echo '<td data-label="' .
                    esc_attr(bhg_t("sc_start", "Start")) .
                    '">' .
                    esc_html(
                        mysql2date(get_option("date_format"), $row->start_date)
                    ) .
                    "</td>";
                echo '<td data-label="' .
                    esc_attr(bhg_t("sc_end", "End")) .
                    '">' .
                    esc_html(
                        mysql2date(get_option("date_format"), $row->end_date)
                    ) .
                ...
                $status_key = strtolower((string) $row->status);
                echo '<td data-label="' .
                    esc_attr(bhg_t("sc_status", "Status")) .
                    '">' .
                    esc_html(bhg_t($status_key, ucfirst($status_key))) .
                    "</td>";
                echo '<td data-label="' .
                    esc_attr(bhg_t("label_details", "Details")) .
                    '"><a href="' .
                    esc_url($detail_url) .
                    '">' .
                    esc_html(bhg_t("label_show_details", "Show details")) .
                    "</a></td>";
                echo "</tr>";
            }
        ...
        public function prizes_shortcode($atts)
        {
            ...
            $active = isset($atts["active"])
                ? strtolower((string) $atts["active"])
                : "yes";
            if (in_array($active, ["yes", "no", "1", "0"], true)) {
                $args["active"] = in_array($active, ["yes", "1"], true) ? 1 : 0;
            }

            $layout = $this->normalize_prize_layout(
                isset($atts["design"])
                    ? $atts["design"]
                    : $this->get_default_prize_layout()
            );
            $size = $this->normalize_prize_size(
                isset($atts["size"]) ? $atts["size"] : "medium"
            );

            $prizes = BHG_Prizes::get_prizes($args);

            if (empty($prizes)) {
                return '<div class="bhg-prizes-shortcode"><p>' .
                    esc_html(bhg_t("no_prizes_yet", "No prizes found.")) .
                    "</p></div>";
            }
            ...
            $section_options = class_exists("BHG_Prizes")
                ? BHG_Prizes::prepare_section_options(["layout" => $layout])
                : ["layout" => $layout];
            $display_overrides = [];

            if ("" !== $atts["visible"]) {
                $section_options["carousel_visible"] = max(
                    1,
                    absint($atts["visible"])
                );
            }
            ...
            if ("" !== $atts["interval"]) {
                $section_options["carousel_interval"] = max(
                    1000,
                    absint($atts["interval"])
                );
            }

            if ("" !== $atts["hide_heading"]) {
                $hide = $this->normalize_yes_no_attr(
                    $atts["hide_heading"],
                    false
                );
            ...
            $heading_attr =
                "" !== $atts["heading"]
                    ...
                    : $atts["heading_text"];
            if ("" !== $heading_attr) {
                $section_options["heading_text"] = sanitize_text_field(
                    wp_unslash($heading_attr)
                );
            }

            $summary_attr = $this->normalize_yes_no_attr(
                $atts["show_summary"],
                false
            );
            ...
            if ("" !== $atts["summary_title"]) {
                $section_options["summary_heading"] = sanitize_text_field(
                    wp_unslash($atts["summary_title"])
                );
            }
            ...
            foreach (
                ...
                if ("" !== $atts[$key]) {
                    $display_overrides[$key] = $this->normalize_yes_no_attr(
                        $atts[$key]
                    );
                }
            }
            ...
            if ("" !== $atts["click_action"]) {
                $display_overrides[
                    "click_action"
                ] = $this->normalize_click_action_attr($atts["click_action"]);
            }

            if ("" !== $atts["link_target"]) {
                $display_overrides[
                    "link_target"
                ] = $this->normalize_link_target_attr($atts["link_target"]);
            }

            if ("" !== $atts["category_target"]) {
                $display_overrides[
                    "category_target"
                ] = $this->normalize_link_target_attr($atts["category_target"]);
            }
        ...
        public function winner_notifications_shortcode($atts)
        {
            ...
            $show_affiliate = !in_array(
                strtolower((string) $a["affiliate_site"]),
                ["no", "false", "0"],
                true
            );
            ...
            // db call ok; no-cache ok.
            $select =
                "SELECT h.id, h.title, h.starting_balance, h.final_balance, h.winners_count, h.status, h.created_at, h.closed_at, h.affiliate_site_id";
            if ($aff_table) {
            ...
            $sql = $wpdb->prepare(
                $select .
                    ...
                    ($aff_table
                        ? " h LEFT JOIN " .
                            $aff_table .
                            " aff ON aff.id = h.affiliate_site_id"
                        : " h") .
                    " WHERE h.status = 'closed' ORDER BY h.closed_at DESC LIMIT %d",
                (int) $a["limit"]
            ...
            if (!$hunts) {
                return "<p>" .
                    esc_html(
                        bhg_t("notice_no_closed_hunts", "No closed hunts yet.")
                    ) .
                    "</p>";
            }
            ...
            foreach ($hunts as $hunt) {
                $winners = function_exists("bhg_get_top_winners_for_hunt")
                    ? bhg_get_top_winners_for_hunt(
                        $hunt->id,
                        (int) $hunt->winners_count
                    )
                    : [];

                $start_balance = isset($hunt->starting_balance)
                    ? bhg_format_money((float) $hunt->starting_balance)
                    : bhg_t("label_emdash", "—");
                $opened = $hunt->created_at
                    ? mysql2date(get_option("date_format"), $hunt->created_at)
                    : "";
                $closed = $hunt->closed_at
                    ? mysql2date(get_option("date_format"), $hunt->closed_at)
                    : "";
                $affiliate = "";
                if ($show_affiliate) {
                    if (
                        isset($hunt->affiliate_site_name) &&
                        "" !== $hunt->affiliate_site_name
                    ) {
                        $affiliate = $hunt->affiliate_site_name;
                    } elseif (
                        isset($hunt->affiliate_site_id) &&
                        (int) $hunt->affiliate_site_id > 0
                    ) {
                        // translators: %d: affiliate site ID.
                        $affiliate = sprintf(
                            bhg_t("label_affiliate_id", "Site #%d"),
                            (int) $hunt->affiliate_site_id
                        );
                    }
                }

                echo '<div class="bhg-winner-card bhg-info-block">';
                echo '<h3 class="bhg-winner-card__title">' .
                    esc_html($hunt->title) .
                    "</h3>";
                ...
                echo "<li><strong>" .
                    esc_html(bhg_t("label_opened", "Opened")) .
                    ":</strong> " .
                    esc_html($opened ? $opened : bhg_t("label_emdash", "—")) .
                    "</li>";
                echo "<li><strong>" .
                    esc_html(bhg_t("label_closed", "Closed")) .
                    ":</strong> " .
                    esc_html($closed ? $closed : bhg_t("label_emdash", "—")) .
                    "</li>";
                if ($show_affiliate) {
                    echo "<li><strong>" .
                        esc_html(
                            bhg_t("label_affiliate_site", "Affiliate Site")
                        ) .
                        ":</strong> " .
                        esc_html(
                            "" !== $affiliate
                ...
                if (null !== $hunt->final_balance) {
                    echo "<li><strong>" .
                        ...
                        ":</strong> " .
                        esc_html(
                            bhg_format_money((float) $hunt->final_balance)
                        ) .
                        "</li>";
                }
                echo "</ul>";
                ...
                if ($winners) {
                    ...
                    echo "<h4>" .
                        esc_html(bhg_t("label_winners", "Winners")) .
                        "</h4>";
                    ...
                    foreach ($winners as $w) {
                        ...
                        echo '<li><span class="bhg-winner-name">' .
                            esc_html($nm) .
                            "</span> " .
                            esc_html(bhg_t("label_emdash", "—")) .
                            ' <span class="bhg-winner-guess">' .
                            esc_html(bhg_format_money((float) $w->guess)) .
                            '</span> <span class="bhg-winner-diff">(' .
                            esc_html(bhg_format_money((float) $w->diff)) .
                            ")</span></li>";
                    }
                    echo "</ul>";
                    echo "</div>";
        ...
        public function my_bonushunts_shortcode($atts)
        {
            ...
            $currency_callback = function_exists("bhg_format_money")
                ? "bhg_format_money"
            ...
            echo "<h2>" .
                esc_html(
                    bhg_t("profile_section_my_bonushunts", "My Bonus Hunts")
                ) .
                "</h2>";
            ...
            if (empty($rows)) {
                echo "<p>" .
                    esc_html(
                        bhg_t(
                            "notice_no_hunts_user",
                            "You have not participated in any bonus hunts yet."
                        )
                    ) .
                    "</p>";
            } else {
                ...
                echo "<th>" .
                    esc_html(bhg_t("label_bonus_hunt", "Bonus Hunt")) .
                    "</th>";
                ...
                echo "<th>" .
                    ...
                    "</th>";
                echo "<th>" .
                    esc_html(bhg_t("label_closed_at", "Closed At")) .
                    "</th>";
                echo "</tr></thead><tbody>";

                foreach ($rows as $row) {
                    $guess_display = call_user_func(
                        $currency_callback,
                        $row["guess"]
                    );
                    $final_display =
                    ...
                    $final_cell =
                        null === $final_display
                            ? "&mdash;"
                            : esc_html($final_display);
                    $diff_cell =
                        null === $diff_display
                            ? "&mdash;"
                            : esc_html($diff_display);
                    $position_cell =
                        $row["position"] > 0
                            ? esc_html((string) (int) $row["position"])
                            : "&mdash;";
                    $closed_cell = $closed_at
                        ? esc_html($closed_at)
                        : "&mdash;";
                    ...
                    echo '<tr class="' .
                        ...
                        '">';
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_bonus_hunt", "Bonus Hunt")) .
                        '">' .
                        esc_html($row["title"]) .
                        "</td>";
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_guess", "Guess")) .
                        '">' .
                        esc_html($guess_display) .
                        "</td>";
                    echo '<td data-label="' .
                        esc_attr(
                            bhg_t("label_final_balance", "Final Balance")
                        ) .
                        '">' .
                        $final_cell .
                        "</td>";
                    ...
                    echo '<td data-label="' .
                        ...
                        "</td>";
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_closed_at", "Closed At")) .
                        '">' .
                        $closed_cell .
                        "</td>";
                    echo "</tr>";
        ...
        public function my_tournaments_shortcode($atts)
        {
            ...
            echo "<h2>" .
                esc_html(
                    bhg_t("profile_section_my_tournaments", "My Tournaments")
                ) .
                "</h2>";
            ...
            if (empty($tournaments)) {
                ...
                echo "<th>" .
                    ...
                    "</th>";
                echo "<th>" .
                    esc_html(bhg_t("label_points", "Points")) .
                    "</th>";
                echo "<th>" .
                    esc_html(bhg_t("label_wins", "Times Won")) .
                    "</th>";
                echo "<th>" . esc_html(bhg_t("label_rank", "Rank")) . "</th>";
                echo "<th>" .
                    esc_html(bhg_t("label_last_win", "Last win")) .
                    "</th>";
                echo "</tr></thead><tbody>";
                ...
                foreach ($tournaments as $row) {
                    ...
                    $rank_cell =
                        null === $row["rank"]
                            ? "&mdash;"
                            : esc_html((string) (int) $row["rank"]);
                    $last_win_cell = $last_win
                        ? esc_html($last_win)
                        : "&mdash;";
                    ...
                    echo "<tr>";
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_tournament", "Tournament")) .
                        '">' .
                        esc_html($row["title"]) .
                        "</td>";
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_points", "Points")) .
                        '">' .
                        esc_html((int) $points_cell) .
                        "</td>";
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_wins", "Times Won")) .
                        '">' .
                        esc_html((int) $row["wins"]) .
                        "</td>";
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_rank", "Rank")) .
                        '">' .
                        $rank_cell .
                        "</td>";
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_last_win", "Last win")) .
                        '">' .
                        $last_win_cell .
                        "</td>";
                    echo "</tr>";
        ...
        public function my_prizes_shortcode($atts)
        {
            ...
            echo "<h2>" .
                esc_html(bhg_t("profile_section_my_prizes", "My Prizes")) .
                "</h2>";
            ...
            if (empty($prizes)) {
                echo "<p>" .
                    esc_html(
                        bhg_t(
                            "notice_no_prizes_user",
                            "You have not won any prizes yet."
                        )
                    ) .
                    "</p>";
            } else {
                ...
                echo "<th>" .
                    esc_html(bhg_t("label_category", "Category")) .
                    "</th>";
                echo "<th>" .
                    esc_html(bhg_t("label_bonus_hunt", "Bonus Hunt")) .
                    "</th>";
                ...
                foreach ($prizes as $row) {
                    $closed_at = $row["closed_at"]
                        ...
                        : "";
                    $category_label = BHG_Prizes::get_category_label(
                    ...
                    echo "<tr>";
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_prize", "Prize")) .
                        '">' .
                        esc_html($row["title"]) .
                        "</td>";
                    echo '<td data-label="' .
                        esc_attr(bhg_t("label_category", "Category")) .
                        '">' .
        ...
        public function my_rankings_shortcode($atts)
        {
            ...
            $wins = array_filter($hunts, static function ($row) {
            ...
            foreach ($tournaments as $row) {
                ...
                if (
                    isset($row["rank"]) &&
                    $row["rank"] &&
                    (null === $best_rank || (int) $row["rank"] < $best_rank)
                ) {
            ...
            echo "<h2>" .
                esc_html(bhg_t("profile_section_my_rankings", "My Rankings")) .
                "</h2>";

            echo '<ul class="bhg-profile-summary">';
            echo "<li>" .
                esc_html(bhg_t("label_total_hunt_wins", "Total hunt wins")) .
                ": " .
                esc_html((string) count($wins)) .
                "</li>";
            echo "<li>" .
                esc_html(
                    bhg_t(
                        "label_total_tournament_points",
                        "Total tournament points"
                    )
                ) .
                ": " .
                esc_html((string) $total_points) .
                "</li>";
            echo "<li>" .
                esc_html(
                    bhg_t("label_best_tournament_rank", "Best tournament rank")
                ) .
                ": " .
                (null === $best_rank
                    ? "&mdash;"
                    : esc_html((string) $best_rank)) .
                "</li>";
            echo "<li>" .
                esc_html(
                    bhg_t("label_tournaments_played", "Tournaments played")
                ) .
                ": " .
                esc_html((string) count($tournaments)) .
                "</li>";
            echo "</ul>";
            ...
            if (empty($wins) && empty($tournaments)) {
                ...
                if (!empty($wins)) {
                    echo "<h3>" .
                        esc_html(bhg_t("label_bonus_hunts", "Bonus Hunts")) .
                        "</h3>";
                    echo '<div class="bhg-table-wrapper">';
                    echo '<table class="bhg-profile-table"><thead><tr>';
                    echo "<th>" .
                ...
                if (!empty($tournaments)) {
                    ...
                    echo "<th>" .
                        ...
                        "</th>";
                    echo "<th>" .
                        esc_html(bhg_t("label_rank", "Rank")) .
                        "</th>";
                    echo "<th>" .
                        esc_html(bhg_t("label_last_win", "Last win")) .
                        "</th>";
                    ...
                    foreach ($tournaments as $row) {
                        ...
                        echo '<td data-label="' .
                            esc_attr(bhg_t("label_points", "Points")) .
                            '">' .
                            esc_html((string) (int) $points_cell) .
                            "</td>";
                        echo '<td data-label="' .
                            esc_attr(bhg_t("label_wins", "Times Won")) .
                            '">' .
                            esc_html((string) (int) $row["wins"]) .
                            "</td>";
                        echo '<td data-label="' .
                            esc_attr(bhg_t("label_rank", "Rank")) .
                            '">' .
        ...
        public function user_profile_shortcode($atts)
        {
            ...
            $user_id = $user->ID;
            $real_name = trim(
                (string) get_user_meta($user_id, "bhg_real_name", true)
            );
            if ("" === $real_name) {
                $real_name = trim(
            ...
            $username_display = $this->format_username_with_badges(
                $username,
                $user_id
            );
            $email = $user->user_email;
            $is_affiliate = (int) get_user_meta(
                $user_id,
                "bhg_is_affiliate",
                true
            );
            ...
            $aff_text = $is_affiliate
                ? bhg_t("label_affiliate", "Affiliate")
                : bhg_t("label_not_affiliate", "Not Affiliate");
            $edit_link = "";
            if (current_user_can("edit_user", $user_id)) {
                $edit_link = get_edit_user_link($user_id);
            }
            $output =
                '<div class="bhg-user-profile"><div class="bhg-table-wrapper"><table class="bhg-user-profile-table">';
            $output .=
                "<tr><th>" .
                esc_html(bhg_t("label_name", "Name")) .
                "</th><td>" .
                esc_html($real_name) .
                "</td></tr>";
            $output .=
                ...
                esc_html(bhg_t("label_username", "User")) .
                "</th><td>" .
                wp_kses_post($username_display) .
                "</td></tr>";
            $output .=
                "<tr><th>" .
                esc_html(bhg_t("label_email", "Email")) .
                "</th><td>" .
                esc_html($email) .
                "</td></tr>";
            $output .=
                "<tr><th>" .
                esc_html(bhg_t("label_affiliate_status", "Affiliate Status")) .
                "</th><td>" .
                wp_kses_post($badge) .
                " " .
                esc_html($aff_text) .
            ...
            if (function_exists("bhg_get_user_affiliate_websites")) {
                $site_ids = array_filter(
                    array_map(
                        "absint",
                        (array) bhg_get_user_affiliate_websites((int) $user_id)
                    )
                );
                if ($site_ids) {
                    global $wpdb;
            ...
            if ($site_rows) {
                ...
                foreach ($site_rows as $site_row) {
                    $label = sprintf(
                        bhg_t(
                            "label_affiliate_website_number",
                            "Affiliate Website %d"
                        ),
                        $count
                    );
            ...
            if ($edit_link) {
                $output .=
                    ...
                    esc_url($edit_link) .
                    '">' .
                    esc_html(bhg_t("link_edit_profile", "Edit Profile")) .
                    "</a></p>";
            }
            $output .= "</div>";
            return $output;
        ...
        public function jackpot_current_shortcode($atts)
        {
            if (!class_exists("BHG_Jackpots")) {
                return $this->shortcode_admin_notice(
                    "BHG jackpot current suppressed: jackpots module missing."
                );
            }
            ...
            if ($jackpot_id <= 0) {
                return $this->shortcode_admin_notice(
                    'BHG jackpot current suppressed: provide id="" attribute.'
                );
            }

            $amount = BHG_Jackpots::instance()->get_formatted_amount(
                $jackpot_id
            );

            if ("" === $amount) {
                return $this->shortcode_admin_notice(
                    "BHG jackpot current suppressed: jackpot not found."
                );
            }

            return '<span class="bhg-jackpot-amount" data-jackpot-id="' .
                esc_attr($jackpot_id) .
                '">' .
                esc_html($amount) .
                "</span>";
        ...
        public function jackpot_ticker_shortcode($atts)
        {
            ...
            $status = in_array(
                ...
                : "active";
            $design = in_array(
                sanitize_key($atts["design"]),
                ["fade", "scroll"],
                true
            )
                ? sanitize_key($atts["design"])
                : "fade";
            $jackets = BHG_Jackpots::instance()->get_ticker_items(
                $mode,
                $status
            );
            ...
            $fade_interval = max(
                1,
                (int) get_option("bhg_jackpot_ticker_interval", 5)
            );
            $scroll_speed = max(
                1,
                (int) get_option("bhg_jackpot_ticker_scroll_speed", 25)
            );
            $separator = (string) get_option(
                "bhg_jackpot_ticker_separator",
                "-"
            );
            $padding = max(
                0,
                (int) get_option("bhg_jackpot_ticker_padding", 0)
            );
            ...
            $style_attr =
                $padding > 0
                    ? sprintf(' style="--bhg-ticker-padding:%dpx;"', $padding)
                    : "";
            echo '<div class="' .
                esc_attr(implode(" ", $classes)) .
                '" data-interval="' .
                esc_attr($fade_interval) .
                '" data-speed="' .
                ...
                '"' .
                $style_attr .
                '><ul class="bhg-jackpot-ticker__list">';
            if ("winners" === $mode) {
                $total = count($jackets);
                $index = 0;
                foreach ($jackets as $row) {
                    ...
                    $amount = isset($row["amount_after"])
                        ? (float) $row["amount_after"]
                        : 0.0;
                    $title = isset($row["jackpot_title"])
                        ? $row["jackpot_title"]
                        : "";
                    echo '<li class="bhg-jackpot-ticker__item">';
                    echo '<span class="bhg-ticker-winner-name">' .
                        wp_kses_post($user_label) .
                        "</span> ";
                    echo '<span class="bhg-ticker-amount">' .
                        esc_html($format_amount($amount)) .
                        "</span> ";
                    if ($title) {
                        echo '<span class="bhg-ticker-jackpot">' .
                            esc_html($title) .
                            "</span>";
                    }
                    if ("scroll" === $design && $separator && $index < $total) {
                        echo '<span class="bhg-ticker-separator" aria-hidden="true">' .
                            esc_html($separator) .
                            "</span>";
                    }
                    echo "</li>";
                ...
                foreach ($jackets as $row) {
                    ...
                    $amount = isset($row["current_amount"])
        ...
        public function jackpot_winners_shortcode($atts)
        {
            if (!class_exists("BHG_Jackpots")) {
                return $this->shortcode_admin_notice(
                    "BHG jackpot winners suppressed: jackpots module missing."
                );
            }
            ...
            $args = ["limit" => max(1, absint($atts["limit"]))];
            ...
            if (empty($rows)) {
                if ($atts["empty"]) {
                    return '<div class="bhg-jackpot-winners-empty">' .
                        esc_html($atts["empty"]) .
                        "</div>";
                }

                return $this->shortcode_admin_notice(
                    "BHG jackpot winners suppressed: no winners found for filters."
                );
            }
            ...
            $visibility = static function ($value) {
                ...
                return !in_array($value, ["hide", "0", "false", "no"], true);
            };
            $format_amount = static function ($amount) {
                if (function_exists("bhg_format_money")) {
                    return bhg_format_money($amount);
            ...
            $field_config = $this->parse_field_tokens(
                $atts["fields"],
                $allowed_fields,
                ["username", "amount", "title", "affiliate", "date"]
            );
            $field_tokens = $field_config["tokens"];
            ...
            $field_tokens = array_values(
                array_filter(
                    array_map(static function ($token) use ($visibility_flags) {
                        if (isset($token["field"])) {
                            $field = (string) $token["field"];
                            if (
                                isset($visibility_flags[$field]) &&
                                false === $visibility_flags[$field]
                            ) {
                                return null;
                            }
                        }
            ...
            $strong_fields = array_filter(
                array_map(
                    "sanitize_key",
                    array_map("trim", explode(",", (string) $atts["strong"]))
                )
            );
            $field_tokens = array_map(static function ($token) use (
                $strong_fields
            ) {
                if (
                    isset($token["field"]) &&
                    in_array($token["field"], $strong_fields, true)
                ) {
                    $token["bold"] = true;
                }
            ...
            $header_labels = [
                ...
                "affiliate" => bhg_t(
                    "label_affiliate_website",
                    "Affiliate Website"
                ),
                "date" => bhg_t("label_date", "Date"),
            ];

            $render_token_value = static function (
                $token,
                $value_map,
                $placeholder = ""
            ) {
            ...
            if ("list" === $layout) {
                ...
                foreach ($rows as $row) {
                    $title = isset($row["jackpot_title"])
                        ? $this->format_title_label(
                            (string) $row["jackpot_title"]
                        )
                        : "";
                    $amount = isset($row["amount_after"])
                        ? (float) $row["amount_after"]
                    ...
                    $user_id = isset($row["user_id"])
                        ? (int) $row["user_id"]
                        : 0;
                    $affiliate = isset($row["affiliate_site_name"])
                        ? $this->format_title_label(
                            (string) $row["affiliate_site_name"]
                        )
                        : "";
                    $user_name = "";
                    if ($user_id) {
                        $user = get_userdata($user_id);
                        if ($user) {
                            $user_name = $this->format_title_label(
                                $user->display_name
                            );
                        }
                    }
                    ...
                    $field_values = [
                        "username" =>
                            "" !== $user_name
                                ? '<span class="bhg-jackpot-winner-name">' .
                                    esc_html($user_name) .
                                    "</span>"
                                : "",
                        "amount" =>
                            '<span class="bhg-jackpot-amount">' .
                            esc_html($format_amount($amount)) .
                            "</span>",
                        "title" =>
                            "" !== $title
                                ? '<span class="bhg-jackpot-name">' .
                                    esc_html($title) .
                                    "</span>"
                                : "",
                        "affiliate" =>
                            "" !== $affiliate
                                ? '<span class="bhg-jackpot-affiliate">' .
                                    esc_html($affiliate) .
                                    "</span>"
                                : "",
                        "date" => $created_at
            ...
            foreach ($field_tokens as $token) {
                ...
                if (isset($token["literal"])) {
                    ...
                    isset($token["field"]) &&
                    isset($header_labels[$token["field"]])
                ) {
            ...
            foreach ($rows as $row) {
                $title = isset($row["jackpot_title"])
                    ? $this->format_title_label((string) $row["jackpot_title"])
                    : "";
                $amount = isset($row["amount_after"])
                    ? (float) $row["amount_after"]
                    : 0.0;
        ...
        public function best_guessers_shortcode($atts)
        {
            ...
            $periods = [
                ...
                "monthly" => [
                    ...
                    "end" => wp_date(
                        "Y-m-t",
                        strtotime($current_month . "-01", $now_ts)
                    ),
                ],
                "yearly" => [
            ...
            foreach ($periods as $key => $info) {
                ...
                if (!empty($info["start"]) && !empty($info["end"])) {
                    ...
                    $where_clauses[] =
                        '(t.start_date IS NULL OR t.start_date = "0000-00-00")';
                    $where_clauses[] =
                        '(t.end_date IS NULL OR t.end_date = "0000-00-00")';
                }

                $sql =
                    "SELECT u.ID as user_id, u.user_login, SUM(r.wins) as total_wins" .
                    " FROM {$wins_tbl} r" .
                ...
                $sql .=
                    " ORDER BY total_wins DESC, u.user_login ASC, u.ID ASC LIMIT 50";
            ...
            foreach ($periods as $key => $info) {
                if ($first) {
                    echo '<li class="active"><a href="#bhg-tab-' .
                        esc_html($key) .
                        '">' .
                        esc_html($info["label"]) .
                        "</a></li>";
            ...
            foreach ($periods as $key => $info) {
                ...
                echo '<div id="bhg-tab-' .
                    ...
                    esc_attr($classes) .
                    '">';
                $rows = isset($results[$key]) ? $results[$key] : [];
                if (!$rows) {
                    echo "<p>" .
                        esc_html(bhg_t("notice_no_data_yet", "No data yet.")) .
                        "</p>";
                } else {
                    echo '<div class="bhg-table-wrapper">';
                    echo '<table class="bhg-leaderboard"><thead><tr><th>' .
                        esc_html(bhg_t("label_position", "Position")) .
                        "</th><th>" .
                        esc_html(bhg_t("sc_user", "User")) .
                        "</th><th>" .
                        esc_html(bhg_t("label_points", "Points")) .
                        "</th><th>" .
                        esc_html(bhg_t("sc_wins", "Times Won")) .
                        "</th></tr></thead><tbody>";
                    $pos = 1;
                    foreach ($rows as $r) {
                        /* translators: %d: user ID. */
                        ...
                        $points = isset($r->points)
                            ? (int) $r->points
                            : (isset($r->total_points)
                                ? (int) $r->total_points
                                : 0);
                        echo "<tr><td>" .
                            (int) $pos .
            ...
            if ($hunts) {
                ...
                $base = esc_url_raw(
                    remove_query_arg(
                        "hunt_id",
                        wp_validate_redirect($raw, home_url("/"))
                    )
                );
...
add_shortcode( 'bonushunt-list', array( $this, 'bonushunt_list_shortcode' ) );
	...
	private function shortcode_admin_notice( $message ) {
		if ( current_user_can( 'manage_options' ) ) {
			return '<div class="bhg-shortcode-note">' . esc_html( $message ) . '</div><!-- ' . esc_html( $message ) . ' -->';
		}
...
private function normalize_prize_layout( $layout ) {
         ...
         * @return string HTML markup.
         */
        private function render_prize_summary_columns( $prize_sets, $section_options = array(), $enabled = true ) {
                if ( ! $enabled || empty( $prize_sets ) ) {
                        return '';
                }
                ...
                $regular_summary = $this->build_prize_summary_entries( $regular_prizes, $section_options );
                $premium_summary = $this->build_prize_summary_entries( $premium_prizes, $section_options );
                ...
                if ( ! empty( $regular_summary ) ) {
                        echo '<div class="bhg-prize-summary-column">';
                        echo '<h5 class="bhg-prize-summary-title">' . esc_html( bhg_t( 'regular_prize_summary_heading', 'Regular Prize Summary' ) ) . '</h5>';
                        echo '<ol class="bhg-prize-summary-list">';
                        foreach ( $regular_summary as $entry ) {
                ...
                if ( ! empty( $premium_summary ) ) {
                        echo '<div class="bhg-prize-summary-column">';
                        echo '<h5 class="bhg-prize-summary-title">' . esc_html( bhg_t( 'premium_prize_summary_heading', 'Premium Prize Summary' ) ) . '</h5>';
                        echo '<ol class="bhg-prize-summary-list">';
                        foreach ( $premium_summary as $entry ) {
                                $text = isset( $entry['text'] ) ? (string) $entry['text'] : '';
                                if ( '' === $text ) {
...
if ( is_user_logged_in() ) {
return $this->shortcode_admin_notice( 'BHG login notice suppressed: user already logged in.' );
}
...
if ( $tournaments_table ) {
               ...
               if ( '' !== $section_html ) {
                               ...
                               $leaderboard_prizes_markup .= '</div>';
                                      ...
                                      $leaderboard_summary_markup = $this->render_prize_summary_columns( $prizes, $section_options, $show_prize_summary );
...
echo '<div class="bhg-leaderboard-headings">' . implode( '', $heading_parts ) . '</div>';
                                                ...
                                                if ( $tournament_filter_active && '' !== $leaderboard_summary_markup ) {
                                                                echo '<div class="bhg-prize-summary-wrap">' . wp_kses_post( $leaderboard_summary_markup ) . '</div>';
                                                }
...
$site_filter = isset( $_GET['bhg_site'] ) ? $this->resolve_affiliate_site_id( wp_unslash( $_GET['bhg_site'] ) ) : 0;
...
case 'points':
$order_parts[] = 'r.points ' . $order;
$order_parts[] = 'r.wins ' . ( 'DESC' === $order ? 'DESC' : 'ASC' );
$order_parts[] = 'r.last_win_date ' . ( 'DESC' === $order ? 'ASC' : 'DESC' );
break;
case 'username':
$order_parts[] = 'u.user_login ' . $order;
...
$heading = $tournament->title ? $tournament->title : bhg_t( 'label_tournament', 'Tournament' );
                                                                ...
                                                                echo '<div class="bhg-tournament-header">';
                                                                echo '<h3 class="bhg-tournament-title">' . esc_html( $heading ) . '</h3>';
                                                                echo '<div class="bhg-tournament-meta">';
                                                                echo '<div class="bhg-tournament-meta__item"><span class="bhg-tournament-meta__label">' . esc_html( bhg_t( 'sc_start', 'Start' ) ) . '</span><span class="bhg-tournament-meta__value">' . esc_html( mysql2date( get_option( 'date_format' ), $tournament->start_date ) ) . '</span></div>';
                                                                echo '<div class="bhg-tournament-meta__item"><span class="bhg-tournament-meta__label">' . esc_html( bhg_t( 'sc_end', 'End' ) ) . '</span><span class="bhg-tournament-meta__value">' . esc_html( mysql2date( get_option( 'date_format' ), $tournament->end_date ) ) . '</span></div>';
                                                                echo '<div class="bhg-tournament-meta__item"><span class="bhg-tournament-meta__label">' . esc_html( bhg_t( 'sc_status', 'Status' ) ) . '</span><span class="bhg-tournament-meta__value">' . esc_html( bhg_t( $status_key, ucfirst( $status_key ) ) ) . '</span></div>';
echo '</div>';
...
echo '</p>';
                ...
                if ( $show_prizes ) {
                        ...
                        $prize_summary_markup = $this->render_prize_summary_columns( $prizes, $section_options, $show_prize_summary_detail );
                        if ( '' !== $prize_summary_markup ) {
                                echo '<div class="bhg-prize-summary-wrap">' . wp_kses_post( $prize_summary_markup ) . '</div>';
                        }
                }
...
echo '<th scope="col" class="sortable"><a href="' . esc_url( $toggle( 'points' ) ) . '">' . esc_html( $points_label ) . $sort_icon_markup( 'points', $points_label ) . '</a></th>';
...
return ob_get_clean();
					   ...
					   if ( '' === $search ) {
							   $base_url = remove_query_arg( 'bhg_search', $base_url );
                                           ...
                                           echo '<div class="bhg-filter-controls">';
                                           echo '<label class="bhg-filter-label">' . esc_html( bhg_t( 'label_timeline_colon', 'Timeline:' ) ) . ' ';
                                           echo '<select name="bhg_timeline" class="bhg-filter-select">';
                                               $timeline_options = array(
                                                                 'all_time'     => bhg_t( 'label_all_time', 'Alltime' ),
                                                                 'today'        => bhg_t( 'label_today', 'Today' ),
                        ...
                        echo '<label class="bhg-filter-label">' . esc_html( bhg_t( 'status', 'Status:' ) ) . ' ';
                        echo '<select name="bhg_status" class="bhg-filter-select">';
                        $statuses   = array(
                                'active' => bhg_t( 'label_active', 'Active' ),
                                'closed' => bhg_t( 'label_closed', 'Closed' ),
                        ...
                        foreach ( $statuses as $key => $label ) {
                                echo '<option value="' . esc_attr( $key ) . '"' . selected( $status_key, $key, false ) . '>' . esc_html( $label ) . '</option>';
                                           ...
                                           echo '<button class="bhg-filter-button" type="submit">' . esc_html( bhg_t( 'button_filter', 'Filter' ) ) . '</button>';
                                           echo '</div>';

                                           if ( $show_search_control ) {
                                                           echo '<div class="bhg-search-control">';
                                                           echo '<label class="bhg-filter-label">' . esc_html( bhg_t( 'label_search', 'Search' ) ) . ' <input type="text" name="bhg_search" value="' . esc_attr( $search ) . '"></label>';
                                                           echo '<button class="bhg-filter-button" type="submit">' . esc_html( bhg_t( 'button_search', 'Search' ) ) . '</button>';
                                                           echo '</div>';
                                           }
...
public function jackpot_current_shortcode( $atts ) {
if ( ! class_exists( 'BHG_Jackpots' ) ) {
return $this->shortcode_admin_notice( 'BHG jackpot current suppressed: jackpots module missing.' );
}
...
if ( $jackpot_id <= 0 ) {
return $this->shortcode_admin_notice( 'BHG jackpot current suppressed: provide id="" attribute.' );
}

$amount = BHG_Jackpots::instance()->get_formatted_amount( $jackpot_id );

if ( '' === $amount ) {
return $this->shortcode_admin_notice( 'BHG jackpot current suppressed: jackpot not found.' );
}
...
public function jackpot_winners_shortcode( $atts ) {
if ( ! class_exists( 'BHG_Jackpots' ) ) {
return $this->shortcode_admin_notice( 'BHG jackpot winners suppressed: jackpots module missing.' );
}
...
return $this->shortcode_admin_notice( 'BHG jackpot winners suppressed: no winners found for filters.' );
}
