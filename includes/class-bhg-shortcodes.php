...
$wpdb->usermeta,
	...
	*/
        ...
        public function login_hint_shortcode( $atts = array() ) {
                ...
                /**
                 * Enqueue default frontend assets used by user dashboards.
                 *
                 * @return void
                 */
                private function enqueue_user_assets() {
                        wp_enqueue_style(
                                'bhg-shortcodes',
                ...
                /**
                 * Fetch hunts the current user has participated in.
                 *
                 * @param int $user_id User ID.
                 * @return array[]
                 */
                private function get_user_hunts( $user_id ) {
                        global $wpdb;
                        ...
                        $latest_query = $wpdb->prepare(
                                "SELECT MAX(id) AS last_id, hunt_id FROM {$guesses_table} WHERE user_id = %d GROUP BY hunt_id",
                                (int) $user_id
                        );

                        $sql = "SELECT h.id, h.title, h.status, h.final_balance, h.closed_at, h.created_at, h.winners_count, h.starting_balance, h.num_bonuses, h.affiliate_site_id, g.id AS guess_id, g.guess, g.created_at AS guessed_at, g.updated_at FROM {$hunts_table} h INNER JOIN ({$latest_query}) latest ON latest.hunt_id = h.id INNER JOIN {$guesses_table} g ON g.id = latest.last_id ORDER BY h.created_at DESC";

                        $rows = $wpdb->get_results( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                        ...
                        foreach ( $rows as $row ) {
                                $final_balance   = isset( $row->final_balance ) && '' !== $row->final_balance ? (float) $row->final_balance : null;
                                ...
                                if ( null !== $final_balance ) {
                                        $absolute_diff = abs( $final_balance - $guess_value );

                                        $rank_sql = $wpdb->prepare(
                                                "SELECT COUNT(*) FROM {$guesses_table} WHERE hunt_id = %d AND (ABS(%f - guess) < %f OR (ABS(%f - guess) = %f AND id < %d))",
                                                (int) $row->id,
                                                $final_balance,
                                                $absolute_diff,
                                                $final_balance,
                                        ...
                                        $better = (int) $wpdb->get_var( $rank_sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                                        $rank   = $better + 1;
                                }
                                ...
                                $results[] = array(
                                        ...
                                        'guess'              => $guess_value,
                                        'guess_id'           => $guess_id,
                                        'guessed_at'         => isset( $row->guessed_at ) ? (string) $row->guessed_at : '',
                                        'updated_at'         => isset( $row->updated_at ) ? (string) $row->updated_at : '',
                                        'rank'               => $rank,
                                        'absolute_diff'      => $absolute_diff,
                                        'winners_count'      => isset( $row->winners_count ) ? (int) $row->winners_count : 0,
                                        'affiliate_site_id'  => isset( $row->affiliate_site_id ) ? (int) $row->affiliate_site_id : 0,
                                );
                        }
                ...
                /**
                 * Fetch tournament results for a user.
                 *
                 * @param int $user_id User ID.
                 * @return array[]
                 */
                private function get_user_tournaments( $user_id ) {
                        ...
                        $sql = $wpdb->prepare(
                                "SELECT t.id, t.title, t.type, t.status, t.start_date, t.end_date, t.affiliate_site_id, r.wins, r.points, r.last_win_date FROM {$tournaments_table} t INNER JOIN {$results_table} r ON r.tournament_id = t.id WHERE r.user_id = %d ORDER BY COALESCE(t.end_date, t.start_date) DESC, t.id DESC",
                                (int) $user_id
                        );
                        ...
                        foreach ( $rows as $row ) {
                                $ranking_rows = $wpdb->get_results(
                                        $wpdb->prepare(
                                                "SELECT user_id FROM {$results_table} WHERE tournament_id = %d ORDER BY points DESC, wins DESC, last_win_date ASC, user_id ASC",
                                                (int) $row->id
                                        )
                                ...
                                if ( ! empty( $ranking_rows ) ) {
                                        ...
                                        foreach ( $ranking_rows as $ranking_row ) {
                                                if ( (int) $ranking_row->user_id === (int) $user_id ) {
                                                        $rank = $position;
                                                        break;
                                                }
                                                ++$position;
                ...
                /**
                 * Fetch prizes the user has won.
                 *
                 * @param int $user_id User ID.
                 * @return array[]
                 */
                private function get_user_prizes( $user_id ) {
                        global $wpdb;
                        ...
                        $sql = $wpdb->prepare(
                                "SELECT w.hunt_id, w.position, w.guess, w.diff, w.created_at, h.title, h.final_balance, h.closed_at, h.status FROM {$winners_table} w INNER JOIN {$hunts_table} h ON h.id = w.hunt_id WHERE w.user_id = %d ORDER BY w.created_at DESC",
                                (int) $user_id
                        );
                        ...
                        foreach ( $rows as $row ) {
                                ...
                                if ( $hunt_id && ! array_key_exists( $hunt_id, $prize_cache ) ) {
                                        if ( class_exists( 'BHG_Prizes' ) ) {
                                ...
                                if ( $hunt_id && ! empty( $prize_cache[ $hunt_id ] ) ) {
                                        $index = max( 0, (int) $row->position - 1 );
                                        if ( isset( $prize_cache[ $hunt_id ][ $index ] ) && isset( $prize_cache[ $hunt_id ][ $index ]->title ) ) {
                                                $prize_title = (string) $prize_cache[ $hunt_id ][ $index ]->title;
                                        }
                                }
                                ...
                                $results[] = array(
                                        ...
                                        'guess'         => isset( $row->guess ) ? (float) $row->guess : 0,
                                        'diff'          => isset( $row->diff ) ? abs( (float) $row->diff ) : null,
                ...
                /**
                 * Render the My Bonus Hunts shortcode.
                 *
                ...
                public function my_bonushunts_shortcode( $atts ) {
                        ...
                        if ( ! is_user_logged_in() ) {
                                return $this->login_hint_shortcode();
                        }
                        ...
                        $hunts = $this->get_user_hunts( (int) $user_id );

                        $this->enqueue_user_assets();
                        ...
                        if ( empty( $hunts ) ) {
                                ...
                                foreach ( $hunts as $hunt ) {
                                        $title        = $hunt['title'] ? $hunt['title'] : bhg_t( 'label_untitled', '(untitled)' );
                                        $guess_display = bhg_format_currency( (float) $hunt['guess'] );
                                        $diff_display  = null !== $hunt['absolute_diff'] ? bhg_format_currency( (float) $hunt['absolute_diff'] ) : bhg_t( 'label_dash', '-' );
                                        $rank_display  = null !== $hunt['rank'] ? (int) $hunt['rank'] : bhg_t( 'label_dash', '-' );
                                        $final_display = null !== $hunt['final_balance'] ? bhg_format_currency( (float) $hunt['final_balance'] ) : bhg_t( 'label_dash', '-' );

                                        $status = isset( $hunt['status'] ) ? $hunt['status'] : '';
                                        switch ( $status ) {
                                                case 'closed':
                                                        $status_label = bhg_t( 'label_closed', 'Closed' );
                                                        break;
                                                case 'open':
                                                        $status_label = bhg_t( 'label_active', 'Active' );
                                                        break;
                                                default:
                                                        $status_label = $status ? ucfirst( $status ) : bhg_t( 'label_dash', '-' );
                                                        break;
                                        }
                ...
                /**
                 * Render the My Tournaments shortcode.
                 *
                 * @param array $atts Shortcode attributes.
                 * @return string
                 */
                public function my_tournaments_shortcode( $atts ) {
                        ...
                        $tournaments = $this->get_user_tournaments( (int) $user_id );
                        ...
                        if ( empty( $tournaments ) ) {
                                ...
                                foreach ( $tournaments as $tournament ) {
                                        $title        = $tournament['title'] ? $tournament['title'] : bhg_t( 'label_untitled', '(untitled)' );
                                        $points_value = isset( $tournament['points'] ) ? (int) $tournament['points'] : 0;
                                        $wins_display = (int) $tournament['wins'];
                                        $rank_display = null !== $tournament['rank'] ? (int) $tournament['rank'] : bhg_t( 'label_dash', '-' );
                                        $type_key     = $tournament['type'] ? sanitize_key( $tournament['type'] ) : '';
                                        ...
                                        $type_labels = array(
                                                ...
                                                'alltime'   => bhg_t( 'label_alltime', 'All-time' ),
                                        );

                                        $type_label = isset( $type_labels[ $type_key ] ) ? $type_labels[ $type_key ] : ( $type_key ? ucfirst( $type_key ) : bhg_t( 'label_dash', '-' ) );

                                        $status = isset( $tournament['status'] ) ? $tournament['status'] : '';
                                        switch ( $status ) {
                                                case 'active':
                                                        $status_label = bhg_t( 'label_active', 'Active' );
                                                        break;
                                        ...
                                        $row_classes = array();
                                        if ( is_int( $rank_display ) && $rank_display <= 3 ) {
                                                $row_classes[] = 'bhg-top-three';
                                        }

                                        $row_attr = $row_classes ? ' class="' . esc_attr( implode( ' ', $row_classes ) ) . '"' : '';

                                        echo '<tr' . wp_kses_post( $row_attr ) . '>';
                                        echo '<td>' . esc_html( $title ) . '</td>';
                                        echo '<td>' . esc_html( $points_value ) . '</td>';
                                        echo '<td>' . esc_html( $wins_display ) . '</td>';
                                        echo '<td>' . esc_html( $rank_display ) . '</td>';
                ...
                /**
                 * Render the My Prizes shortcode.
                 *
                 * @param array $atts Shortcode attributes.
                 * @return string
                 */
                public function my_prizes_shortcode( $atts ) {
                        ...
                        $wins = $this->get_user_prizes( (int) $user_id );
                        ...
                        if ( empty( $wins ) ) {
                                ...
                                foreach ( $wins as $win ) {
                                        $title        = $win['title'] ? $win['title'] : bhg_t( 'label_untitled', '(untitled)' );
                                        $prize_title  = $win['prize'] ? $win['prize'] : bhg_t( 'label_dash', '-' );
                                        $position     = (int) $win['position'];
                                        $guess_display = bhg_format_currency( (float) $win['guess'] );
                                        $diff_display  = null !== $win['diff'] ? bhg_format_currency( (float) $win['diff'] ) : bhg_t( 'label_dash', '-' );
                ...
                /**
                 * Render the combined rankings shortcode.
                 *
                 * @param array $atts Shortcode attributes.
                 * @return string
                 */
		...
		$atts = shortcode_atts(
			array(
				'prize_layout' => $default_prize_layout,
		...
		$prize_layout = $this->normalize_prize_layout( isset( $atts['prize_layout'] ) ? $atts['prize_layout'] : $default_prize_layout );
		$prize_size   = $this->normalize_prize_size( isset( $atts['prize_size'] ) ? $atts['prize_size'] : $default_prize_size );
          ...
          return ob_get_clean();
			...
			foreach ( $rows as $row ) {
                ...
                public function leaderboards_shortcode( $atts ) {
                       $a = shortcode_atts(
                               array(
                                       'fields'     => 'pos,user,points,wins,avg_hunt,avg_tournament',
                                       'ranking'    => 1,
                                       'timeline'   => '',
                                       'orderby'    => 'points',
                                       'order'      => 'DESC',
                       ...
                       if ( empty( $fields_arr ) ) {
                               $fields_arr = array( 'pos', 'user', 'points', 'wins', 'avg_hunt', 'avg_tournament' );
                        }
			...
			$aff_yes_list = implode( ',', $aff_yes_sql );
                        ...
                        $points_map       = function_exists( 'bhg_get_tournament_points_map' ) ? bhg_get_tournament_points_map() : array();
                        $points_case_bits = array();
                        foreach ( $points_map as $position => $points_value ) {
                                $position     = (int) $position;
                                $points_value = (int) $points_value;
                                if ( $position <= 0 || $points_value <= 0 ) {
                                        continue;
                                }
                                $points_case_bits[] = 'WHEN ' . $position . ' THEN ' . $points_value;
                        }

                        if ( $points_case_bits ) {
                                $select_parts[] = 'SUM(CASE hw.position ' . implode( ' ', $points_case_bits ) . ' ELSE 0 END) AS total_points';
                        } else {
                                $select_parts[] = 'SUM(0) AS total_points';
                        }
                        ...
                        $orderby       = isset( $orderby_map[ $orderby_key ] ) ? $orderby_map[ $orderby_key ] : $orderby_map['points'];
                                ...
                                if ( $need_aff ) {
                                        ...
                                        if ( $pos <= 3 ) {
                                                $row_classes[] = 'bhg-top-three';
                                                                                ...
                                                                                echo '<tr' . ( $row_classes ? ' class="' . esc_attr( implode( ' ', $row_classes ) ) . '"' : '' ) . '>';
                                foreach ( $fields_arr as $field ) {
                                        if ( 'pos' === $field ) {
                                                echo '<td>' . (int) $pos . '</td>';
                                        } elseif ( 'user' === $field ) {
                                                ...
                                                echo '<td>' . (int) ( isset( $row->total_points ) ? $row->total_points : 0 ) . '</td>';
                                        } elseif ( 'wins' === $field ) {
		...
		public function tournaments_shortcode( $atts ) {
			...
			wp_enqueue_style(
				...
				$details_id = isset( $_GET['bhg_tournament_id'] ) ? absint( wp_unslash( $_GET['bhg_tournament_id'] ) ) : 0;
                        if ( $details_id > 0 ) {
                                                                ...
                                                                $r  = esc_sql( $this->sanitize_table( $wpdb->prefix . 'bhg_tournament_results' ) );
                                                                $u  = esc_sql( $this->sanitize_table( $wpdb->users ) );
                                                                $aw = esc_sql( $this->sanitize_table( $wpdb->prefix . 'bhg_affiliate_websites' ) );
                                if ( ! $t || ! $r || ! $u ) {
                                        return '';
                                }

                                        $select = "SELECT t.id, t.title, t.description, t.type, t.start_date, t.end_date, t.status, t.affiliate_site_id, t.affiliate_url_visible, t.prizes";
                                        $from   = " FROM {$t} t";

                                        if ( $aw ) {
                                                $select .= ', aw.name AS affiliate_name, aw.url AS affiliate_url';
                                                $from   .= " LEFT JOIN {$aw} aw ON aw.id = t.affiliate_site_id";
                                        }

                                        // db call ok; no-cache ok.
                                        $tournament = $wpdb->get_row(
                                                $wpdb->prepare(
                                                        $select . $from . ' WHERE t.id = %d',
                                                        $details_id
                                                )
                                        );
                                if ( ! $tournament ) {
                                        ...
                                        $normalized_layout = $this->normalize_prize_layout( $default_prize_layout );
                                        $normalized_size   = $this->normalize_prize_size( $default_prize_size );

                                        $prizes_markup = '';
                                        if ( class_exists( 'BHG_Prizes' ) ) {
                                                $prize_ids = array();
                                                if ( isset( $tournament->prizes ) && function_exists( 'bhg_parse_tournament_prizes_field' ) ) {
                                                        $prize_ids = bhg_parse_tournament_prizes_field( $tournament->prizes );
                                                }

                                                if ( ! $prize_ids && function_exists( 'bhg_get_tournament_prize_ids' ) ) {
                                                        $prize_ids = bhg_get_tournament_prize_ids( (int) $tournament->id );
                                                }

                                                if ( $prize_ids ) {
                                                        $prizes = BHG_Prizes::get_prizes_by_ids( $prize_ids, array( 'active_only' => true ) );
                                                        if ( $prizes ) {
                                                                if ( 'carousel' === $normalized_layout ) {
                                                                        wp_enqueue_script(
                                                                                'bhg-shortcodes-js',
                                                                                ( defined( 'BHG_PLUGIN_URL' ) ? BHG_PLUGIN_URL : plugins_url( '/', __FILE__ ) ) . 'assets/js/bhg-shortcodes.js',
                                                                                array(),
                                                                ...
                                                                $prizes_markup = $this->render_prize_section( $prizes, $normalized_layout, $normalized_size );
                                                        }
                                                }
                                        }
					...
					return esc_url(
						add_query_arg(
							...
							$base
                                        ...
                                        $title = isset( $tournament->title ) && '' !== $tournament->title ? $tournament->title : bhg_t( 'label_untitled', '(untitled)' );
                                        echo $this->render_title_block( $title, 'h2', array( 'bhg-tournament-title' ) );

                                        if ( ! empty( $tournament->description ) ) {
                                                echo '<div class="bhg-description bhg-tournament-description">' . wp_kses_post( wpautop( $tournament->description ) ) . '</div>';
                                        }

                                        $type_key    = isset( $tournament->type ) ? sanitize_key( (string) $tournament->type ) : '';
                                        $type_labels = array(
                                                'weekly'    => bhg_t( 'label_weekly', 'Weekly' ),
                                                'monthly'   => bhg_t( 'label_monthly', 'Monthly' ),
                                                'quarterly' => bhg_t( 'label_quarterly', 'Quarterly' ),
                                        ...
                                        echo '<ul class="bhg-hunt-meta">';
                                        echo '<li><strong>' . esc_html( bhg_t( 'label_type', 'Type' ) ) . ':</strong> ' . esc_html( $type_label ) . '</li>';

                                        $start_date = isset( $tournament->start_date ) ? (string) $tournament->start_date : '';
                                        if ( '' !== $start_date && '0000-00-00' !== $start_date ) {
                                                echo '<li><strong>' . esc_html( bhg_t( 'sc_start', 'Start' ) ) . ':</strong> ' . esc_html( mysql2date( get_option( 'date_format' ), $start_date ) ) . '</li>';
                                        }

                                        $end_date = isset( $tournament->end_date ) ? (string) $tournament->end_date : '';
                                        if ( '' !== $end_date && '0000-00-00' !== $end_date ) {
                                                echo '<li><strong>' . esc_html( bhg_t( 'sc_end', 'End' ) ) . ':</strong> ' . esc_html( mysql2date( get_option( 'date_format' ), $end_date ) ) . '</li>';
                                        }

                                        $status_key = strtolower( (string) $tournament->status );
                                        echo '<li><strong>' . esc_html( bhg_t( 'sc_status', 'Status' ) ) . ':</strong> ' . esc_html( bhg_t( $status_key, ucfirst( $status_key ) ) ) . '</li>';

                                        $affiliate_name = isset( $tournament->affiliate_name ) ? $tournament->affiliate_name : '';
                                        $affiliate_id   = isset( $tournament->affiliate_site_id ) ? (int) $tournament->affiliate_site_id : 0;
                                        if ( $affiliate_id > 0 && '' !== $affiliate_name ) {
                                                $show_url = isset( $tournament->affiliate_url_visible ) ? (int) $tournament->affiliate_url_visible : 1;
                                                $url      = isset( $tournament->affiliate_url ) ? $tournament->affiliate_url : '';
                                                echo '<li><strong>' . esc_html( bhg_t( 'label_affiliate_website', 'Affiliate Website' ) ) . ':</strong> ';
                                                if ( $show_url && '' !== $url ) {
                                                        echo '<a href="' . esc_url( $url ) . '" target="_blank" rel="nofollow noopener">' . esc_html( $affiliate_name ) . '</a>';
                                                } else {
			...
			foreach ( $rows as $row ) {
                ...
                public function prizes_shortcode( $atts ) {
                        ...
                        $layout = $this->normalize_prize_layout( isset( $atts['design'] ) && '' !== $atts['design'] ? $atts['design'] : $default_prize_layout );
                        $size   = $this->normalize_prize_size( isset( $atts['size'] ) && '' !== $atts['size'] ? $atts['size'] : $default_prize_size );
				...
				if ( $info['type'] ) {
					...
					if ( ! empty( $info['start'] ) && ! empty( $info['end'] ) ) {
						...
						$params[] = $info['end'];
										...
										$sql = 'SELECT u.ID as user_id, u.user_login, SUM(r.points) as total_points, SUM(r.wins) as total_wins'
										. " FROM {$wins_tbl} r"
										...
										// db call ok; no-cache ok.
																				$sql = $wpdb->prepare( $sql, ...$params );
										$sql                                        .= ' ORDER BY total_points DESC, total_wins DESC, u.user_login ASC LIMIT 50';
			...
			foreach ( $periods as $key => $info ) {
				...
				if ( ! $rows ) {
						echo '<p>' . esc_html( bhg_t( 'notice_no_data_yet', 'No data yet.' ) ) . '</p>';
                                        ...
                                        echo '<table class="bhg-leaderboard"><thead><tr><th>' . esc_html( bhg_t( 'label_hash', '#' ) ) . '</th><th>' . esc_html( bhg_t( 'sc_user', 'User' ) ) . '</th><th>' . esc_html( bhg_t( 'label_points', 'Points' ) ) . '</th><th>' . esc_html( bhg_t( 'sc_wins', 'Wins' ) ) . '</th></tr></thead><tbody>';
                                                $pos = 1;
                                        foreach ( $rows as $r ) {
                                                        /* translators: %d: user ID. */
                                                        $user_label = $r->user_login ? $r->user_login : sprintf( bhg_t( 'label_user_hash', 'user#%d' ), (int) $r->user_id );
                                                        ...
                                                        echo '<tr' . $class_attr . '><td>' . (int) $pos . '</td><td>' . esc_html( $user_label ) . '</td><td>' . $points . '</td><td>' . (int) $r->total_wins . '</td></tr>';
                                                        ++$pos;
                                        }
                                                echo '</tbody></table>';
				}
