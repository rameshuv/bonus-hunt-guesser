...
if ( ! function_exists( 'bhg_dispatch_hunt_notifications' ) ) {
        ...
        function bhg_dispatch_hunt_notifications( $hunt_id, $final_balance, array $winner_ids = array() ) {
                ...
                if ( '' === $hunt_title ) {
                        ...
                        $title_query = $wpdb->get_var( $wpdb->prepare( "SELECT title FROM {$hunts_table} WHERE id = %d", $hunt_id ) );
                        if ( $title_query ) {
                ...
                if ( ! empty( $winners ) ) {
                        foreach ( $winners as $winner ) {
                                ...
                                $guess_display = bhg_format_currency( $guess_value );
                                $diff_display  = bhg_format_currency( $diff_value );
                ...
                $final_balance_safe = esc_html( bhg_format_currency( $final_balance ) );
                ...
                if ( ! empty( $settings['bonus_hunt']['enabled'] ) ) {
                        ...
                        $participants  = $wpdb->get_col( $wpdb->prepare( "SELECT DISTINCT user_id FROM {$guesses_table} WHERE hunt_id = %d", $hunt_id ) );
...
if ( ! function_exists( 'bhg_dispatch_tournament_notifications' ) ) {
        ...
        function bhg_dispatch_tournament_notifications( $tournament_id ) {
                ...
                $tournament_title = $wpdb->get_var( $wpdb->prepare( "SELECT title FROM {$tournaments_table} WHERE id = %d", $tournament_id ) );
                ...
                $results = $wpdb->get_results( $wpdb->prepare( "SELECT user_id, wins FROM {$results_table} WHERE tournament_id = %d ORDER BY wins DESC, user_id ASC", $tournament_id ) );
...
if ( ! function_exists( 'bhg_send_hunt_results_email' ) ) {
        ...
        function bhg_send_hunt_results_email( $hunt_id, $winner_ids = array(), $final_balance = null ) {
                ...
                if ( null === $final_balance ) {
                        global $wpdb;
                        $hunts_table   = esc_sql( $wpdb->prefix . 'bhg_bonus_hunts' );
                        $final_balance = $wpdb->get_var( $wpdb->prepare( "SELECT final_balance FROM {$hunts_table} WHERE id = %d", $hunt_id ) );
                }
