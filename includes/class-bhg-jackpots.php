...
class BHG_Jackpots {
    ...
    protected function get_jackpots_for_hunt( $hunt_id, array $context ) {
        ...
        $closed_timestamp = $closed_at ? mysql2date( 'U', $closed_at, false ) : current_time( 'timestamp', true );
            ...
            switch ( $link_mode ) {
                ...
                case 'period':
                    ...
                    if ( 'all_time' === $period ) {
                        ...
                        $applies = gmdate( 'Y', $closed_timestamp ) === gmdate( 'Y', current_time( 'timestamp', true ) );
                    } else {
                        $applies = gmdate( 'Ym', $closed_timestamp ) === gmdate( 'Ym', current_time( 'timestamp', true ) );
                    }
                    break;
                case 'all':
                default:
    ...
    protected function record_hit( array $jackpot, $user_id, $hunt_id, $guess_id, $final_balance ) {
        ...
        $this->log_event( $jackpot_id, 'hit', $current_amount, $current_amount, $user_id, $hunt_id, array( 'guess_id' => $guess_id, 'final_balance' => $final_balance ) );
    }
