	...
	public function handle_save_hunt() {
		...
		if ( $id ) {
						$existing_row = $wpdb->get_row(
							$wpdb->prepare(
                                        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is built from the sanitized prefix.
	...
	public function handle_delete_ad() {
		...
		if ( $ad_id ) {
						...
						$placeholders = implode( ', ', array_fill( 0, count( $bulk_ad_ids ), '%d' ) );
												$wpdb->query(
													$wpdb->prepare(
                                                                // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber,WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- Table name is built from the sanitized prefix and placeholders generated programmatically.
	...
	private function get_hunt_ids_within_range( $start_date, $end_date ) {
		...
		if ( is_string( $end_date ) && preg_match( '/^\d{4}-\d{2}-\d{2}$/', $end_date ) ) {
				...
				$params[] = $end_date . ' 23:59:59';
						...
						$sql = 'SELECT id FROM `' . $table . '`'; // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is built from the sanitized prefix.
		if ( $where ) {
				$sql .= ' WHERE ' . implode( ' AND ', $where );
						...
						$query = $sql;
		if ( ! empty( $params ) ) {
						$query = $wpdb->prepare( $sql, ...$params ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- SQL fragments use sanitized table names and validated WHERE clauses.
		}

						$ids = $wpdb->get_col(
                                // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Query is built from sanitized fragments and prepared when parameters are present.
							$query
						);
