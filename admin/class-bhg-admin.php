...
class BHG_Admin {
	...
	public function __construct() {
		...
		// Handlers.
				...
				add_action( 'admin_post_bhg_save_points', array( $this, 'handle_save_points_settings' ) );
	}
	...
	public function handle_save_hunt() {
		...
		if ( $is_new_hunt && 'open' === $status && function_exists( 'bhg_send_notifications' ) ) {
				$all_users = get_users(
					array(
			...
			foreach ( $all_users as $user_obj ) {
				...
				$recipients[] = array(
					...
					'username' => $user_obj->display_name ? $user_obj->display_name : $user_obj->user_login,
				);
			}
			...
			if ( ! empty( $recipients ) ) {
										$tokens = array(
											'{{hunt}}' => $title,
											'{{start_balance}}' => bhg_format_currency( $starting ),
											'{{num_bonuses}}' => (string) $num_bonuses,
										);
										bhg_send_notifications( 'hunt', $recipients, $tokens );
			}
		}
		...
		if ( $should_close ) {
			...
			if ( function_exists( 'bhg_send_notifications' ) ) {
										$guesses_table = esc_sql( $wpdb->prefix . 'bhg_guesses' );
				...
				foreach ( (array) $winners as $winner_id ) {
					...
					if ( $wu ) {
										...
										$winner_first = $winner_names ? $winner_names[0] : bhg_t( 'label_emdash', '—' );
				...
				if ( ! empty( $recipients ) ) {
					$tokens = array(
						'{{hunt}}'    => $hunt_title ? $hunt_title : bhg_t( 'bonus_hunt', 'Bonus Hunt' ),
						'{{final}}'   => bhg_format_currency( $final_balance ),
						'{{winner}}'  => $winner_first,
						'{{winners}}' => $winner_list,
					);
					bhg_send_notifications( 'winner', $recipients, $tokens );
				}
			}
		}
	...
	public function handle_delete_prize() {
		...
		/**
		 * Save notification templates and preferences.
		 */
	public function handle_save_notifications() {
		if ( ! current_user_can( 'manage_options' ) ) {
			...
			check_admin_referer( 'bhg_save_notifications', 'bhg_save_notifications_nonce' );

			$submitted = isset( $_POST['notifications'] ) && is_array( $_POST['notifications'] ) ? wp_unslash( $_POST['notifications'] ) : array();
			$defaults  = function_exists( 'bhg_get_default_notification_settings' ) ? bhg_get_default_notification_settings() : array();
			$sanitized = array();

		foreach ( $defaults as $type => $default ) {
				$raw = isset( $submitted[ $type ] ) && is_array( $submitted[ $type ] ) ? $submitted[ $type ] : array();

				$enabled = isset( $raw['enabled'] ) ? (int) ( '1' === (string) $raw['enabled'] || 1 === (int) $raw['enabled'] ) : 0;
				$subject = isset( $raw['subject'] ) ? sanitize_text_field( $raw['subject'] ) : ( isset( $default['subject'] ) ? $default['subject'] : '' );
				$desc    = isset( $raw['description'] ) ? wp_kses_post( $raw['description'] ) : ( isset( $default['description'] ) ? $default['description'] : '' );
				$bcc     = '';

			if ( isset( $raw['bcc'] ) ) {
					$bcc_string = sanitize_text_field( $raw['bcc'] );
					$bcc_string = str_replace( ';', ',', $bcc_string );
					$emails     = array_filter( array_map( 'trim', explode( ',', $bcc_string ) ), 'is_email' );
				if ( ! empty( $emails ) ) {
					$bcc = implode( ', ', $emails );
				}
			}
			...
			update_option( 'bhg_notification_settings', $sanitized );
		...
		if ( ! in_array( $participants_mode, array( 'winners', 'all' ), true ) ) {
                ...
                $data             = array(
					...
					if ( 'auto' === $hunt_link_mode ) {
						...
						if ( $is_new_tournament && $saved_id > 0 && function_exists( 'bhg_send_notifications' ) ) {
							$users = get_users(
								array(
									'fields' => array( 'ID', 'user_email', 'display_name', 'user_login' ),
								)
							...
							if ( ! empty( $recipients ) ) {
								...
								if ( ! empty( $data['start_date'] ) ) {
									$start_label = mysql2date( get_option( 'date_format' ), $data['start_date'] );
								}

								if ( ! empty( $data['end_date'] ) ) {
									$end_label = mysql2date( get_option( 'date_format' ), $data['end_date'] );
								}
								...
								bhg_send_notifications( 'tournament', $recipients, $tokens );
							}
						}
	...
	public function handle_save_user_meta() {
		...
		/**
		 * Save tournament points configuration from the admin form.
		 */
	public function handle_save_points_settings() {
		if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( esc_html( bhg_t( 'no_permission', 'No permission' ) ) );
		}

			check_admin_referer( 'bhg_save_points', 'bhg_save_points_nonce' );

			$context    = isset( $_POST['points_context'] ) ? sanitize_key( wp_unslash( $_POST['points_context'] ) ) : 'all';
			$valid_keys = array( 'all', 'active', 'closed' );
		if ( ! in_array( $context, $valid_keys, true ) ) {
				$context = 'all';
		}

			$submitted_points = isset( $_POST['points'] ) && is_array( $_POST['points'] ) ? wp_unslash( $_POST['points'] ) : array();
			$map              = bhg_get_default_points_map();

		foreach ( $map as $position => $default_points ) {
			if ( isset( $submitted_points[ $position ] ) ) {
					$map[ $position ] = max( 0, (int) $submitted_points[ $position ] );
			}
		}

			$current             = bhg_get_points_settings();
			$current[ $context ] = $map;
			update_option( 'bhg_points_settings', $current );

			$recalculate = ! empty( $_POST['recalculate_points'] );
		if ( $recalculate && method_exists( 'BHG_Models', 'recalculate_tournament_results' ) ) {
				global $wpdb;
				$tournament_ids = $wpdb->get_col( 'SELECT id FROM ' . $wpdb->prefix . 'bhg_tournaments' );
				$scope          = isset( $_POST['recalculate_scope'] ) ? sanitize_key( wp_unslash( $_POST['recalculate_scope'] ) ) : 'all';
			if ( ! in_array( $scope, $valid_keys, true ) ) {
					$scope = 'all';
			}
				BHG_Models::recalculate_tournament_results( (array) $tournament_ids, $scope );
		}

			wp_safe_redirect( add_query_arg( 'bhg_msg', 'points_saved', BHG_Utils::admin_url( 'admin.php?page=bhg-tournaments' ) ) );
			exit;
	}
	...
	public function admin_notices() {
		...
		$map   = array(
			...
			'points_saved'             => bhg_t( 'points_saved', 'Points settings saved.' ),
		);
