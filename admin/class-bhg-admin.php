...
class BHG_Admin {
	...
	public function menu() {
		...
		// Note: Do not remove the default submenu duplication—doing so previously hid our Dashboard submenu.
	}
	...
	public function handle_delete_guess() {
		...
		if ( $guess_id ) {
			$wpdb->delete( $table, array( 'id' => $guess_id ), array( '%d' ) );
		}
	...
	public function handle_save_hunt() {
		...
		if ( $id ) {
			$existing_row = $wpdb->get_row(
				$wpdb->prepare(
					'SELECT status FROM ' . $hunts_table_sql . ' WHERE id = %d',
					(int) $id
				)
			);
		...
		if ( $should_close ) {
			...
			if ( $emails_enabled ) {
				...
				$hunt_title = (string) $wpdb->get_var(
					$wpdb->prepare(
						"SELECT title FROM {$hunts_table_sql} WHERE id = %d",
						$id
					)
				);
	...
	public function handle_delete_hunt() {
		...
		if ( $hunt_id ) {
			$hunt_row = $wpdb->get_row(
				$wpdb->prepare(
					'SELECT tournament_id FROM ' . esc_sql( $hunts_table ) . ' WHERE id = %d',
					(int) $hunt_id
				)
			);
			...
			$winners = $wpdb->get_results(
				$wpdb->prepare(
					'SELECT * FROM ' . esc_sql( $winners_table ) . ' WHERE hunt_id = %d',
					(int) $hunt_id
				)
			);
			...
			$wpdb->delete( $wpdb->prefix . 'bhg_hunt_tournaments', array( 'hunt_id' => $hunt_id ), array( '%d' ) );
				...
				foreach ( $winner_map as $tournament_id => $user_ids ) {
					foreach ( $user_ids as $user_id ) {
						$result_row = $wpdb->get_row(
							$wpdb->prepare(
								'SELECT id, wins FROM ' . esc_sql( $results_table ) . ' WHERE tournament_id = %d AND user_id = %d',
								(int) $tournament_id,
								(int) $user_id
							)
						);
	...
	public function handle_delete_ad() {
		...
		if ( $ad_id ) {
			$wpdb->query(
				$wpdb->prepare(
					'DELETE FROM `' . esc_sql( $ads_table ) . '` WHERE id = %d',
					$ad_id
				)
			);
			...
			$wpdb->query(
				$wpdb->prepare(
					'DELETE FROM `' . esc_sql( $ads_table ) . '` WHERE id IN (' . $placeholders . ')',
					...$bulk_ad_ids
				)
			);
		}
	...
	/** Create or update an advertising entry. */
	...
	public function handle_delete_tournament() {
		...
		if ( $id ) {
			...
			$wpdb->delete( $wpdb->prefix . 'bhg_hunt_tournaments', array( 'tournament_id' => $id ), array( '%d' ) );
	...
	/** Delete an affiliate website record (and unlink from users). */
	...
	/**
	 * Display admin notices for various actions.
	 */
	public function admin_notices() {
		...
		$map = array(
			...
			'invalid_starting_balance' => bhg_t( 'invalid_starting_balance', 'Starting balance could not be parsed. Please enter a numeric amount.' ),
			'invalid_final_balance'    => bhg_t( 'invalid_final_balance', 'Final balance could not be parsed. Please enter a numeric amount.' ),
			...
			'jackpot_created'          => bhg_t( 'jackpot_created', 'Jackpot created.' ),
			'jackpot_updated'          => bhg_t( 'jackpot_updated', 'Jackpot updated.' ),
			'jackpot_deleted'          => bhg_t( 'jackpot_deleted', 'Jackpot deleted.' ),
			'jackpot_reset'            => bhg_t( 'jackpot_reset', 'Jackpot has been reset.' ),
			'jackpot_error'            => bhg_t( 'jackpot_error', 'Unable to save jackpot.' ),
			'hunt_deleted'             => bhg_t( 'hunt_deleted', 'Bonus hunt deleted.' ),
			'close_failed'             => bhg_t( 'close_failed', 'Unable to close the hunt.' ),
		);
