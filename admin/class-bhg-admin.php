...
class BHG_Admin {
	...
	public function __construct() {
		...
		add_action( 'admin_post_bhg_delete_prize_category', array( $this, 'handle_delete_prize_category' ) );
	...
	public function affiliates() {
        ...
        public function advertising() {
                ...
                /**
                 * Render the Buttons admin page.
                 */
                public function buttons() {
                        require BHG_PLUGIN_DIR . 'admin/views/buttons.php';
                }
...
$prize_sets['regular'] = $extract_prize_map( 'prize_ids' );
	...
	public function handle_delete_ad() {
        ...
        public function handle_save_ad() {
			...
			if ( $id ) {
                ...
                /**
                 * Save or update a configured frontend button.
                 */
                public function handle_save_button() {
                        ...
                        check_admin_referer( 'bhg_save_button', 'bhg_save_button_nonce' );
                        ...
                        $data = array(
                                ...
                                'visible_when'     => isset( $_POST['visible_when'] ) ? sanitize_key( wp_unslash( $_POST['visible_when'] ) ) : 'always',
                                'link_url'         => isset( $_POST['link_url'] ) ? esc_url_raw( wp_unslash( $_POST['link_url'] ) ) : '',
                                'link_target'      => isset( $_POST['link_target'] ) ? sanitize_text_field( wp_unslash( $_POST['link_target'] ) ) : '_self',
                                'background'       => isset( $_POST['background'] ) ? sanitize_text_field( wp_unslash( $_POST['background'] ) ) : '',
                                'background_hover' => isset( $_POST['background_hover'] ) ? sanitize_text_field( wp_unslash( $_POST['background_hover'] ) ) : '',
                                'text_color'       => isset( $_POST['text_color'] ) ? sanitize_text_field( wp_unslash( $_POST['text_color'] ) ) : '',
                                'text_hover'       => isset( $_POST['text_hover'] ) ? sanitize_text_field( wp_unslash( $_POST['text_hover'] ) ) : '',
                                'border_color'     => isset( $_POST['border_color'] ) ? sanitize_text_field( wp_unslash( $_POST['border_color'] ) ) : '',
                                'text_size'        => isset( $_POST['text_size'] ) ? absint( wp_unslash( $_POST['text_size'] ) ) : null,
                                'size'             => isset( $_POST['size'] ) ? sanitize_text_field( wp_unslash( $_POST['size'] ) ) : 'medium',
                                'active'           => isset( $_POST['active'] ) ? 1 : 0,
                        );

                        BHG_Buttons::save_button( $data );
                ...
                public function handle_delete_button() {
                        ...
                        check_admin_referer( 'bhg_delete_button', 'bhg_delete_button_nonce' );

                        $id = isset( $_POST['id'] ) ? absint( wp_unslash( $_POST['id'] ) ) : 0;
                        if ( ! $id ) {
                                wp_safe_redirect( wp_get_referer() ? wp_get_referer() : BHG_Utils::admin_url( 'admin.php?page=bhg-buttons' ) );
                                exit;
                        }
	...
	public function handle_delete_prize() {
		...
		/**
		 * Handle creation or update of prize categories.
		 */
	public function handle_save_prize_category() {
		...
		if ( ! check_admin_referer( 'bhg_save_prize_category', 'bhg_prize_category_nonce' ) ) {
				wp_safe_redirect( add_query_arg( 'bhg_msg', 'nonce', BHG_Utils::admin_url( 'admin.php?page=bhg-prizes' ) ) );
				exit;
		}

			$category_id = isset( $_POST['category_id'] ) ? absint( wp_unslash( $_POST['category_id'] ) ) : 0;
			$name        = isset( $_POST['category_name'] ) ? wp_unslash( $_POST['category_name'] ) : '';
			$link_url    = isset( $_POST['category_link_url'] ) ? wp_unslash( $_POST['category_link_url'] ) : '';
			$link_target = isset( $_POST['category_link_target'] ) ? wp_unslash( $_POST['category_link_target'] ) : '_self';
			$show_link   = ! empty( $_POST['category_show_link'] ) ? 1 : 0;

			$result = BHG_Prizes::save_category(
				array(
					'id'          => $category_id,
					'name'        => $name,
					'link_url'    => $link_url,
			...
			$message     = $category_id ? 'cat_updated' : 'cat_saved';
			$redirect    = BHG_Utils::admin_url( 'admin.php?page=bhg-prizes' );
			$stay_edited = $category_id;

		if ( is_wp_error( $result ) ) {
				$message = 'cat_error';
				$code    = $result->get_error_code();

			if ( in_array( $code, array( 'bhg_category_table_missing', 'bhg_category_save_failed', 'bhg_category_name_required', 'bhg_category_not_found' ), true ) ) {
				$message = 'cat_error';
			}

			if ( $stay_edited ) {
					$redirect = add_query_arg( 'edit_category', $stay_edited, $redirect );
			}
		}
		...
		/**
		 * Handle removal of a prize category.
		 */
	public function handle_delete_prize_category() {
			$capability = apply_filters( 'bhg_manage_prizes_capability', 'manage_options' );
		...
		if ( ! check_admin_referer( 'bhg_delete_prize_category', 'bhg_prize_category_delete_nonce' ) ) {
				wp_safe_redirect( add_query_arg( 'bhg_msg', 'nonce', BHG_Utils::admin_url( 'admin.php?page=bhg-prizes' ) ) );
				exit;
		}
		...
		if ( $category_id ) {
				$result = BHG_Prizes::delete_category( $category_id );

			if ( is_wp_error( $result ) ) {
					$code = $result->get_error_code();

				if ( 'bhg_category_delete_last' === $code ) {
					$message = 'cat_last';
				} else {
						$message = 'cat_delete_error';
				}
			}
		} else {
				$message = 'cat_delete_error';
			...
			wp_safe_redirect( add_query_arg( 'bhg_msg', $message, BHG_Utils::admin_url( 'admin.php?page=bhg-prizes' ) ) );
			exit;
	}
