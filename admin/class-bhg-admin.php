...
class BHG_Admin {
	...
	public function menu() {
		...
		if ( isset( $submenu[ $slug ][0] ) ) {
				// phpcs:ignore WordPress.WP.GlobalVariablesOverride -- Adjust default submenu label for clarity.
	...
	public function handle_save_hunt() {
		...
		$tournament_ids_input   = isset( $_POST['tournament_ids'] ) ? array_map( 'sanitize_text_field', (array) wp_unslash( $_POST['tournament_ids'] ) ) : array();
		$tournament_ids         = bhg_sanitize_tournament_ids( $tournament_ids_input );
		$prize_ids_input        = isset( $_POST['prize_ids'] ) ? array_map( 'absint', (array) wp_unslash( $_POST['prize_ids'] ) ) : array();
		$prize_ids              = array();
		...
		if ( $id ) {
			$existing_row = $wpdb->get_row(
				$wpdb->prepare(
		/* phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name sanitized via esc_sql above. */
...
/* phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name sanitized via esc_sql above. */
	...
	public function handle_delete_ad() {
		...
		/* phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name sanitized via esc_sql above. */
			...
			$wpdb->query(
				$wpdb->prepare(
/* phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber -- Dynamic placeholder list generated safely above. */
	...
	private function get_hunt_ids_within_range( $start_date, $end_date ) {
		...
		if ( ! empty( $params ) ) {
			$query = $wpdb->prepare(
		/* phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Dynamic WHERE clause constructed with sanitized fragments. */
				$sql,
				$params
			);
		}

		$ids = $wpdb->get_col( /* phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Query prepared or sanitized above. */ $query );
	...
	public function handle_delete_affiliate() {
		...
		if ( $id ) {
			...
			if ( function_exists( 'get_users' ) && function_exists( 'bhg_get_user_affiliate_websites' ) && function_exists( 'bhg_set_user_affiliate_websites' ) ) {
						$user_ids = get_users(
							array(
								'fields'       => 'ID',
				...
				foreach ( (array) $user_ids as $user_id ) {
					...
					if ( empty( $sites ) ) {
							...
							$filtered = array_values( array_diff( array_map( 'absint', (array) $sites ), array( $id ) ) );
	...
	public function handle_save_user_meta() {
		...
		if ( $user_id ) {
			...
			if ( isset( $_POST['bhg_affiliate_sites'] ) ) {
				$site_ids = array_filter(
					array_map(
						'absint',
						(array) wp_unslash( $_POST['bhg_affiliate_sites'] )
					)
				);
			}
