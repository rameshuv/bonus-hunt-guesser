...
$hunts_sql      = 'SELECT id, title FROM ' . $hunts_table . ' ORDER BY title ASC';
$all_hunts      = $wpdb->get_results( $hunts_sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name assembled from the known $wpdb->prefix.
...
<input type="hidden" name="id" value="<?php echo esc_attr( (int) $row->id ); ?>" /><?php endif; ?>
	<table class="form-table">
		...
		<td><input id="bhg_t_end" type="date" name="end_date" value="<?php echo esc_attr( $row->end_date ?? '' ); ?>" /></td>
				...
				<tr id="bhg_t_hunts_row"<?php echo wp_kses_post( $hunts_row_attr ); ?>>
