...
$hunt_id = absint( filter_input( INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT ) );
...
$tournament_sql    = "SELECT id, title FROM {$t_table} WHERE status = %s";
$tournament_params = array( 'active' );

if ( ! empty( $selected_tournaments ) ) {
		$placeholders      = implode( ', ', array_fill( 0, count( $selected_tournaments ), '%d' ) );
		$tournament_sql   .= " OR id IN ( {$placeholders} )";
		$tournament_params = array_merge( $tournament_params, $selected_tournaments );
}

$tournament_sql .= ' ORDER BY title ASC';

$tournament_prepare_args = array_merge( array( $tournament_sql ), $tournament_params );
$prepared_tournament_sql = call_user_func_array( array( $wpdb, 'prepare' ), $tournament_prepare_args );
$tournaments             = $wpdb->get_results( $prepared_tournament_sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Query prepared above.

$current_page   = max( 1, absint( filter_input( INPUT_GET, 'ppaged', FILTER_SANITIZE_NUMBER_INT ) ) );
$items_per_page = 30;
