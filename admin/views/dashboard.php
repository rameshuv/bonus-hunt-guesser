...
$final_balance = isset( $h->final_balance ) && null !== $h->final_balance ? (float) $h->final_balance : null;
...
if ( '' !== $closed_at ) {
        ...
        $closed_display = false !== $timestamp ? date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $timestamp ) : $closed_at;
}

if ( empty( $winners ) ) :
?>
<tr>
        <td class="bhg-latest-hunts-title"><?php echo '' !== $hunt_title ? esc_html( $hunt_title ) : esc_html( bhg_t( 'label_untitled', '(untitled)' ) ); ?></td>
        <td class="bhg-latest-hunts-winner-cell"><?php echo esc_html( bhg_t( 'no_winners_yet', 'No winners yet' ) ); ?></td>
        <td class="bhg-latest-hunts-money"><?php echo esc_html( bhg_format_money( $start_balance ) ); ?></td>
        <td class="bhg-latest-hunts-money"><?php echo esc_html( $final_display ); ?></td>
        <td><?php echo esc_html( $closed_display ); ?></td>
</tr>
<?php else :
...
<tr>
        <?php if ( 0 === $index ) : ?>
        <td class="bhg-latest-hunts-title" rowspan="<?php echo esc_attr( $winner_total ); ?>"><?php echo '' !== $hunt_title ? esc_html( $hunt_title ) : esc_html( bhg_t( 'label_untitled', '(untitled)' ) ); ?></td>
        <?php endif; ?>
        <td class="bhg-latest-hunts-winner-cell"><?php echo wp_kses_post( $winner_line ); ?></td>
        <?php if ( 0 === $index ) : ?>
        <td class="bhg-latest-hunts-money" rowspan="<?php echo esc_attr( $winner_total ); ?>"><?php echo esc_html( bhg_format_money( $start_balance ) ); ?></td>
        <td class="bhg-latest-hunts-money" rowspan="<?php echo esc_attr( $winner_total ); ?>"><?php echo esc_html( $final_display ); ?></td>
        <td rowspan="<?php echo esc_attr( $winner_total ); ?>"><?php echo esc_html( $closed_display ); ?></td>
        <?php endif; ?>
</tr>
