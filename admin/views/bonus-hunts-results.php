...
$timeframe        = isset( $_GET['timeframe'] ) ? sanitize_key( wp_unslash( $_GET['timeframe'] ) ) : 'month'; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
...
$requested_type = isset( $_GET['type'] ) ? sanitize_key( wp_unslash( $_GET['type'] ) ) : 'hunt'; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
$requested_id   = isset( $_GET['id'] ) ? absint( wp_unslash( $_GET['id'] ) ) : 0; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
...
if ( 'tournament' === $view_type && $tournament_id ) {
	...
	$users_table       = esc_sql( $wpdb->users );
		...
		$tournament = $wpdb->get_row( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$wpdb->prepare(
...
<?php if ( $hunts_map ) : ?>
	...
	foreach ( $hunts_map as $hunt_row ) :
		...
		?>
						<option value="<?php echo esc_attr( $value ); ?>"<?php selected( $selected_control_value, $value ); ?>><?php echo esc_html( $label ); ?></option>
<?php endforeach; ?>
			</optgroup>
<?php endif; ?>
