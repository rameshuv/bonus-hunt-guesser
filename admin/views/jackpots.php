						...
						<?php foreach ( $jackpot_items as $item ) : ?>
							...
							if ( 'hit' === $status && $hit_user ) {
								...
								if ( $user ) {
									...
									$status_label = sprintf(
										bhg_t( 'jackpot_hit_status', 'Hit by %1$s on %2$s' ),
										$user->display_name,
										$hit_at ? esc_html( mysql2date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $hit_at ) ) : ''
									);
								}
							}
							?>
							<tr>
								...
								<td class="bhg-jackpot-actions">
									<a class="button" href="<?php echo esc_url( add_query_arg( array( 'page' => 'bhg-jackpots', 'action' => 'edit', 'jackpot_id' => $item_id ), admin_url( 'admin.php' ) ) ); ?>">
										<?php echo esc_html( bhg_t( 'edit', 'Edit' ) ); ?>
									</a>
									<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="display:inline;">
										...
										<button type="submit" class="button button-link-delete" onclick="return confirm('<?php echo esc_js( bhg_t( 'confirm_delete_jackpot', 'Delete this jackpot? This cannot be undone.' ) ); ?>');">
