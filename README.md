...
- `timeline`: limit tournaments by `day`, `week`, `month`, `quarter`, `year`, `last_year`, or the legacy aliases `this_week`, `this_month`, `this_year`. Compatibility keywords (`weekly`, `monthly`, `yearly`, `quarterly`, `alltime`, `all_time`) map to the corresponding ranges.
...
- **Tournament timeline filtering**
  ...
  3. Edit the same tournament (adjusting a field such as the title or description) and save.
  4. Refresh the shortcode output and verify the tournament still appears in the monthly view, confirming the filter relies on the saved dates.
  5. Repeat with a weekly-length tournament to confirm timeline filters continue working after edits.
