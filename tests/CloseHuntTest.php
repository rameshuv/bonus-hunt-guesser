<?php
/**
 * Tests for closing hunts and recalculating tournament results.
 *
 * @package Bonus_Hunt_Guesser\Tests
 */

use PHPUnit\Framework\TestCase;

/**
 * Ensures hunt closing logic behaves as expected.
 */
final class CloseHuntTest extends TestCase {
	/**
	 * Mocked WordPress database object.
	 *
	 * @var MockWPDB
	 */
	private $wpdb;
	...
	/**
	 * Set up the mocked database instance.
	 */
	...
	/**
	 * Ensure closing a hunt twice does not duplicate winners or wins.
	 */
	public function test_closing_hunt_twice_does_not_duplicate_winners_or_wins(): void {
		...
		$this->assertSame( 3, count( $this->collect_tournament_wins( $tournament_id ) ) );
		...
		$wins_after_second_call = $this->collect_tournament_wins( $tournament_id );
	...
	/**
	 * Ensure "all" participant mode records every guesser position.
	 */
	public function test_close_hunt_all_mode_records_all_positions(): void {
		...
		$wins = $this->collect_tournament_wins( $tournament_id );
	...
	/**
	 * Ensure recalculation honours the participant mode on tournaments.
	 */
	public function test_recalculate_tournament_results_respects_participants_mode(): void {
		...
		$winners_wins = $this->collect_tournament_wins( $winners_tournament );
		$all_wins     = $this->collect_tournament_wins( $all_tournament );
		...
		$recalc_winners = $this->collect_tournament_wins( $winners_tournament );
		$recalc_all     = $this->collect_tournament_wins( $all_tournament );
	 ...
	 *
	 * @return array<int, int> Map of user ID to win count.
	 */
	private function collect_tournament_wins( $tournament_id ) {
		$wins = array();
